...
@Component({
  ...
  template: `
    <div [ngClass]="[store.isPhone ? 'h5-carousel' : 'carousel']" #carousel>
      <div class="item" *ngFor="let img of imgList;let i=index">
      <ng-container *ngIf="i!==imgList.length-2">
          <div class="title">{{img.title}}</div>
          <img [src]="store.isPhone ? img.h5Src : img.src" [alt]="img.alt" />
      </ng-container>
        <ng-container *ngIf="i===imgList.length-2">
            <a class='divbest' href="http://www.hzfactoring.com" target="_blank"></a>
           <div class="title">{{img.title}}</div>
           <img [src]="store.isPhone ? img.h5Src : img.src" [alt]="img.alt" />
        </ng-container>
...
export class CarouselComponent implements AfterViewInit {
  ...
  setImageStyle(imgDomList: any[]) {
    ...
    const setStyleArr = [];
    // const mLen: number = Math.floor(len / 2);
    let leftImgIndex = 0;
    let rightImgIndex = 0;
    for (let i = 0; i < 2; i++) {
