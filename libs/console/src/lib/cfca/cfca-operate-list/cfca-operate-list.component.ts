 ...
 * 1.0                 wangqing          CFCA审核待办         2020-10-30
...
declare const $: any
@Component({
  selector: 'cfca-operate-list',
  templateUrl: `./cfca-operate-list.component.html`,
  styles: [`
        .title {
            width: 100px;
        }
...
export class CfcaOperateListComponent implements OnInit {
  tabConfig: any;
  ...
  public viewFiles(paramFile) {
    XnModalUtils.openInViewContainer(this.xn, this.vcr, FileViewModalComponent, JSON.parse(paramFile)).subscribe();
  }
  ...
  viewRecord(recordId: string) {
    this.xn.router.navigate([`/console/record/todo/view/${recordId}`]);
  }
  ...
  public onPage(e?) {
    ...
    this.heads = CommUtils.getListFields(this.currentTab.headText);
  ...
  downLoad() {
    const params = this.buildParams();
    this.xn.dragon.download('/cfca/down_review_list', params).subscribe(
      (v: any) => {
        this.xn.loading.close();
        this.xn.api.dragon.save(v._body, 'cfca审核列表附件.xlsx');
      }
    );
  }
  /**
    ...
    if (this.searches.length > 0) {
      for (const search of this.searches) {
        if (!XnUtils.isEmpty(this.arrObjs[search.checkerId])) {
          ...
          } else if (search.checkerId === 'updateTime') {
            const date = JSON.parse(this.arrObjs[search.checkerId]);
            params.updateTimeStart = date.beginTime;
            params.updateTimeEnd = date.endTime;
