 ...
 * 1.0                   yu           中介机构用户管理      2019-09-19
 * **********************************************************************
 */

import { Component, OnInit, ViewContainerRef, ElementRef, AfterViewInit, OnDestroy } from '@angular/core';
...
@Component({
    selector: 'app-intermediary-user-management-component',
    templateUrl: `./intermediary-user-management-list.componment.html`,
    styleUrls: ['../intermediary-management-list.componment.css']
})
export class DragonIntermediaryUserManagementComponent implements OnInit, AfterViewInit, OnDestroy {
    tabConfig: any;
  ...
  */
    ...
    public checkAll() {
        if (!this.isAllChecked()) {
            ...
            this.selectedItems = XnUtils.distinctArray2([...this.selectedItems, ...this.listInfo], 'userId');
        } else {
    ...
    public singleChecked(paramItem, index) {
        if (paramItem.checked && paramItem.checked === true) {
            ...
            this.selectedItems = this.selectedItems.filter((x: any) => x.userId !== paramItem.userId);
        } else {
    ...
    public handleHeadClick(paramBtnOperate: ButtonConfigModel) {
        if (paramBtnOperate.operate === 'sub_agency_user_add'){
            paramBtnOperate.click(this.xn, [this.configParams.appId]);
        }else if (paramBtnOperate.operate === 'sub_agency_user_delete'){
            if (this.selectedItems && this.selectedItems.length > 0){
                const userList = this.selectedItems.map(x => {
                    return {userId: x.userId, appId: x.appId};
                });
                paramBtnOperate.click(this.xn, JSON.stringify(userList));
            }else{
    ...
    public handleRowClick(item, btn: ButtonConfigModel, i: number) {
        switch (btn.operate) {
            ...
            case 'sub_agency_user_modify':
                btn.click(this.xn, JSON.stringify([{userId: item.userId, appId: item.appId}]));
                break;
        }
    }
     ...
     */
    navBack() {
        this.xn.router.navigate(['/console/manage/intermediary/intermediary-list'], {
            queryParams: {
