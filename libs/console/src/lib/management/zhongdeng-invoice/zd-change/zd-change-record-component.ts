...
@Component({
    selector: 'zd-record',
    templateUrl: './zd-change-record-component.html'
})
export class ZdRecordComponent implements OnInit {
    ...
    editAction(val: any) {
        ...
        if ((data.status !== 1 && data.status !== 0) || this.xn.user.roles.indexOf(data.nowRoleId) < 0) {
            this.xn.router.navigate([`/console/manage/invoice-search/record/${this.flowId}/view/${data.recordId}`]);
        } else {
            this.xn.router.navigate([`/console/manage/invoice-search/record/${this.flowId}/edit/${data.recordId}`]);
        }
    }
