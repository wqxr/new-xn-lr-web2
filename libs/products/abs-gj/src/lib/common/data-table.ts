 ...
 * All rights reserved.
 *
 * @file：libs\products\abs-gj\src\lib\common\data-table.ts
 * @summary：data-table.ts
 * @version: 1.0
 * *************************************************************************
...
import { SortType } from 'libs/shared/src/lib/config/enum';
...
export class DragonDataTable {
  ...
  protected bindEditorEvent(): void {
    ...
    this.editor.on('open', (e, mode, action) => {
      if (!(action === ActionType.Create || (action === ActionType.Edit && !this.isViewAction))) {
        return;
      }
      ...
      for (const field of this.config.fields) {
        ...
        $(`#DTE_Field_${field.options.memoLink.field}`).change((ev) => {
          const select = $(ev.target).val();
          if (select === null) {
  ...
  preSubmit(e, data) {
    let params;
    if (data.action === ActionType.Edit || data.action === ActionType.Create) {
  ...
  protected getAction(data: any, callback: (data: any) => void, oSettings: any, url: string): void {
  ...
  protected modifyAction(
    ...
    error: (params: any, msg: any, code: any) => void
  ): void {
    let url: string;
    const req: any = {} as any;
    if (data.action === ActionType.Put) {
      url = DataTableUtils.buildUrl(this.config, HttpMethod.Put);
      for (const key in data.data) {
        if (data.data.hasOwnProperty(key)) {
          ...
          if (this.options.where) {
            for (const k in this.options.where) {
              if (this.options.where.hasOwnProperty(k)) {
                req.where[k] = this.options.where[k];
              }
            }
          }
          break;
    ...
    } else if (data.action === ActionType.Remove) {
      url = DataTableUtils.buildUrl(this.config, HttpMethod.Del);
      for (const key in data.data) {
        if (data.data.hasOwnProperty(key)) {
          req.where = DataTableUtils.parseRowId(this.config, key);
          break;
    ...
    } else if (data.action === ActionType.Create) {
      url = DataTableUtils.buildUrl(this.config, HttpMethod.Post);
      for (const key in data.data) {
        if (data.data.hasOwnProperty(key)) {
          req.value = data.data[key];
          for (const field of this.config.fields) {
          ...
          if (this.options.where) {
            for (const k in this.options.where) {
              if (this.options.where.hasOwnProperty(k)) {
                req.value[k] = this.options.where[k];
              }
            }
          }
          break;
