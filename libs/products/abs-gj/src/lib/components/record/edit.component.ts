 ...
 * All rights reserved.
 *
 * @file：libs\products\abs-gj\src\lib\components\record\edit.component.ts
 * @summary：edit.component.ts
 * @version: 1.0
 * *************************************************************************
...
import { HtmlModalComponent } from '../../../../../../shared/src/lib/public/modal/html-modal.component';
import {
  VankeViewChangeAccountComponent
} from '../../../../../../shared/src/lib/public/dragon-vanke/modal/vanke-change-account.component';
...
import { FlowId, OperateType, ProgressStep, GjAgencyType } from '../../../../../../shared/src/lib/config/enum';
import { CheckersOutputModel } from '../../../../../../shared/src/lib/config/checkers';
import {
  RejectModalComponent
} from '../../../../../../shared/src/lib/public/dragon-vanke/modal/reject-modal.component';
import {
...
import { ModalComponent } from '../../../../../../shared/src/lib/common/modal/components/modal';
...
export class GjEditComponent implements AfterViewChecked, OnInit, OnDestroy {
  ...
  GjPlatformVerify = FlowId.GjPlatformVerify;
  GjFinancing = FlowId.GjFinancing;
  ...
  ngOnInit() {
    this.mainFlowId =
      ...
      '';
    this.updatedata = [new CheckerData(), new CheckerData()];
  ...
  private showSubmit(recordId, fresh?) {
    // fresh 为可选，true的时候则刷新，不拉取actions的最后一项，拉checker
    this.xn.dragon
      ...
      .subscribe((json) => {
        ...
        // 拷贝对象
        this.newSvrConfig = XnUtils.deepCopy(this.svrConfig);
      });
  }
  ...
  showRejectIcon() {
    return (
      (this.svrConfig.record.flowId === FlowId.GjFinancing ||
        this.svrConfig.record.flowId === FlowId.GjPlatformVerify) &&
      this.svrConfig.procedure.procedureId === ProgressStep.Operate
    );
  }
  ...
  flowClick() {
    ...
    this.xn.router.navigate([`/abs-gj/record/new/${FlowId.CMNChangeAccount}`], {
      queryParams: {
        id: FlowId.CMNChangeAccount,
        relate: 'mainFlowId',
  ...
  showTempClick() {
    return (
      (this.svrConfig.flow.flowId === FlowId.GjFinancing &&
        this.svrConfig.record.nowProcedureId === ProgressStep.Operate) ||
      (this.svrConfig.flow.flowId === FlowId.GjPlatformVerify &&
        this.svrConfig.record.nowProcedureId === ProgressStep.Operate) ||
      (this.svrConfig.flow.flowId === 'sub_law_manager_survey' &&
        this.svrConfig.record.nowProcedureId === ProgressStep.Review)
    );
  }
  ...
  private buildRows(fresh?): void {
    ...
    if (this.svrConfig.procedure.procedureId === ProgressStep.Begin) {
      // 起始步骤要处理下标题
  ...
  private copyDraft(shows: any[], hides: any[], fresh: boolean): void {
    // 如果有草稿，就把草稿内容复制过来
    const actions = this.svrConfig.actions.slice(
      0,
    ...
    for (const action of actions) {
      // 判断是否为草稿
      if (
        action.operator === OperateType.NONE &&
        action.procedureId === this.svrConfig.procedure.procedureId
      ) {
        for (const checker of action.checkers) {
          // sms, password, text-rate字段不采用草稿
          if (checker.type === 'sms' || checker.type === 'password' || checker.type === 'text-rate') {
            continue;
          }
    ...
    if (
      [FlowId.GjPlatformVerify].includes(
        this.svrConfig.flow.flowId
      ) &&
      [ProgressStep.Operate, ProgressStep.Begin].includes(this.svrConfig.procedure.procedureId)
    ) {
      for (const item of shows) {
  ...
  private onTerminate(): void {
    ...
    this.xn.api.dragon.post('/flow/cancel', params).subscribe(() => {
      this.cancelModal.dismiss();
      if (this.svrConfig.record.flowId === 'sub_law_manager_survey') {
        let msg = [];
        if (Number(agencyInfo.agencyType) === GjAgencyType.PALNMANAGER) {
          msg = [`已取消尽调。`];
          this.xn.msgBox.open(false, msg, () => {
            // 点击【确定】返回上一页
            this.xn.user.navigateBack();
          });
        } else if (Number(agencyInfo.agencyType) === GjAgencyType.LAWFIRM) {
          msg = [`已取消尽调。`];
          this.xn.msgBox.open(false, msg, () => {
            // 2、点击【确定】返回上一页
            this.xn.user.navigateBack();
  ...
  presubmitSpecial(params, x) {
    if (
      ...
      params.procedureId === ProgressStep.Begin
    ) {
      if (x.ret === 0) {
        this.xn.msgBox.open(
          false,
      ...
      (this.svrConfig.flow.flowId === 'sub_change_start' ||
        this.svrConfig.flow.flowId === 'sub_change_capital') &&
      params.procedureId === ProgressStep.Review
    ) {
      if (x.ret === 0) {
        this.xn.msgBox.open(
          false,
      ...
      this.svrConfig.flow.flowId === 'sub_change_start' &&
      params.procedureId === ProgressStep.WindReview
    ) {
      if (x.ret === 0) {
        this.xn.msgBox.open(
          false,
      ...
      params.procedureId === ProgressStep.Operate
    ) {
      if (x.ret === 0) {
    ...
    fn().subscribe((v) => {
      ...
      if (v.action === 'modal') {
        XnModalUtils.openInViewContainer(
          ...
          v.modal,
          v.params
        ).subscribe(() => this.xn.user.navigateBack());
      }
    });
  }
   ...
   * @param res 接口返回数据
   */
  private afterSubmit(flowId, mainFlowId, nowProcedureId, res) {
  ...
  openRejectModal() {
    ...
    XnModalUtils.openInViewContainer(
      ...
      if (v.action === 'ok' && v.type === RejectBranch.Reject) {
        const params: any = {
          recordId: this.recordId,
          procedureId: this.svrConfig.procedure.procedureId,
          memo: this.mainForm.value.cancelChecker,
        ...
        this.xn.api.dragon.post(`/flow/reject`, params).subscribe(() => window.history.back());
      } else if (v.action === 'ok' && v.type === RejectBranch.Stop) {
        this.onTerminate();
      }
    });
  }
  ...
  public showAuditStandard() {
    ...
    if (this.svrConfig.record.nowProcedureId === ProgressStep.Operate) {
      step = 'value';
      invCheckers = this.svrConfig.checkers;
      contractObj = this.localStorageService.caCheMap.get('VankecontractFile');
    } else if (this.svrConfig.record.nowProcedureId === ProgressStep.Review) {
      step = 'data';
      invCheckers = this.svrConfig.actions
        .filter((action) => action.recordSeq === this.svrConfig.record.recordSeq)[0]
        .checkers;
      const contractTemp = invCheckers.filter((c) => c.checkerId === 'dealContract');
      if (contractTemp.length === 1
        && contractTemp[0][step]
        && JSON.parse(contractTemp[0][step])
        && XnUtils.isArray(JSON.parse(contractTemp[0][step]))
      ) {
        contractObj = JSON.parse(contractTemp[0][step]);
      }
    }

    const invoiceObj = invCheckers.filter((item) => item.checkerId === 'invoice');
    const invoiceArray = [];
    if (invoiceObj.length === 1
      && invoiceObj[0][step]
      && JSON.parse(invoiceObj[0][step])
      && XnUtils.isArray(JSON.parse(invoiceObj[0][step]))
    ) {
      ...
      invoiceArr.forEach((invoice) => {
        invoiceArray.push({
          ...
          isHistory: !!(invoice.mainFlowId && XnUtils.isArray(invoice.mainFlowId) && invoice.mainFlowId.length)
        });
      });
    }
    const params: any = {
    ...
    this.xn.dragon.post('/list/main/checker_list_box', params)
      .subscribe(x => {
        if (x.ret === 0 && x.data && x.data.length > 0) {
          const params1 = Object.assign({flowId: this.flowId}, {value: '', checkers: x.data});
          XnModalUtils.openInViewContainer(this.xn, this.vcr, NewVankeAuditStandardModalComponent, params1)
            .subscribe();
        }
      });
  }
