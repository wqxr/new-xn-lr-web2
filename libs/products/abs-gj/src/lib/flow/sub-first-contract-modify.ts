 ...
 * All rights reserved.
 *
 * @file：libs\products\abs-gj\src\lib\flow\sub_first_contract_modify.ts
 * @summary：sub_first_contract_modify.ts
 * @version: 1.0
 * *************************************************************************
...
export class OnceContractGroupModify implements IFlowCustom {
  ...
  afterSubmitandGettip(svrConfig: any): Observable<any> {
    if (svrConfig.procedure.procedureId === ProgressStep.Begin) {
      ...
      const msg = svrConfig.actions
        .find((action) => action.recordSeq === svrConfig.record.recordSeq).checkers
        .find((checker) => checker.checkerId === 'contractListName').data;
