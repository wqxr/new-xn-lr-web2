 ...
 * All rights reserved.
 *
 * @file：libs\products\account-system\src\lib\pages\account-list\account-detail\account-detail.component.ts
 * @summary：查看账户信息页面
 * @version: 1.0
 * *************************************************************************
 * revision             author            reason             date
 * 1.0                 hucongying          init           2021-11-16
 ***************************************************************************/
import {
  Component,
  OnInit,
...
import { AccountStatusTypeEnum, BankTypeEnum, RetCodeEnum, SystemCodeEnum, TradeStatusEnum } from 'libs/shared/src/lib/config/enum';
import { XnTableComponent } from '@lr/ngx-table/lib/table.component';
import { AccountBankTypeOptions, AccountStatusOptions, TradeDirectionOptions, TradeStatusOptions, TranAmtTypeOptions } from 'libs/shared/src/lib/config/options';
...
export class AccountDetailComponent implements OnInit {
      ...
      buttons: [
        {
          ...
          click: (
            ...
            this.xn.router.navigate(['trade-detail'], {
  ...
  ngOnInit(): void {
    this.router.params.subscribe(params => {
      this.appId = params?.appId ? params.appId : null;
      this.getAccountInfo();
    })
  ...
  */
  ...
  downLoadTrade(accountId: string, detailId: string) {
    this.xn.middle.getFileDownload('/account/trade/download/electronic',
      { accountId, detailId }
    ).subscribe(
      res => {
  ...
  */
  viewAccountInfo() {
    this.xn.router.navigate(['account-info', this.accountInfo.accountId], {
      relativeTo: this.router.parent,
    }).then();
  }
   ...
   */
  viewOperatorInfo() {
    this.xn.router.navigate(['operator-info/view', this.accountInfo.accountId], {
      relativeTo: this.router.parent,
    }).then();
  }
   ...
   */
  viewBusinessInfo() {
    this.xn.router.navigate(['business-info/view', this.accountInfo.accountId], {
      relativeTo: this.router.parent,
    }).then();
  }
