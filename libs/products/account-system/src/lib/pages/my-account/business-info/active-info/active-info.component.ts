 ...
 * All rights reserved.
 *
 * @file：libs\products\account-system\src\lib\pages\my-account\business-info\active-account\active-account.component.ts
 * @summary：激活工商信息
 * @version: 1.0
 * *************************************************************************
...
export class ActiveBusinessInfoComponent implements OnInit {
  // 账户id
  accountId: string = '';
  // 打款账户信息
  ...
  getAccountInfo() {
    ...
    this.xn.middle
      .post2('/account/info/business/activate', { accountId: this.accountId })
      .subscribe(
        (x) => {
  ...
  activeAccount() {
    this.$modal.confirm({
      ...
      nzOnOk: () => {
        this.xn.loading.open();
        this.xn.middle
          .post2('/account/info/business/activate', {
          ...
          .subscribe((v) => {
            ...
            if (v.code === RetCodeEnum.OK) {
              if (XnUtils.isEmptys(v.data) || v.data.isActivate === InfoActiveEnum.ACTIVATED) {
                this.message.success('工商信息激活成功！');
                setTimeout(() => {
                  this.goBack();
                }, 1500);
              } else {
                this.message.error(
                  '暂未查到款项信息,请稍后再试.如已超过1小时,可联系平台客服！'
                );
