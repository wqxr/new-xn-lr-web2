 ...
 * All rights reserved.
 *
 * @file：AvengerListComponent
 * @summary：雅居乐-恒泽审批放款列表
 * @version: 1.0
 * **********************************************************************
 * revision             author            reason             date
 * 1.0                 hucongying      雅居乐-恒泽数据对接      2020-12-03
 * **********************************************************************
 */
...
export class AvengerListComponent implements OnInit {
  ...
  public initData(paramTabValue: string, init?: boolean, ) {
    this.businessinvoice = paramTabValue === 'E'; // 商票匹配
    ...
    if (this.tabConfig.title.includes('审批放款-雅居乐-恒泽') && (this.defaultValue === 'A' || this.defaultValue === 'C')) {
      this.xn.api.post('/tool/is_jindie', {}).subscribe(x => {
        if (x.data) {
          this.is_jindie = x.data.isOpen;
          if (this.is_jindie === 1) {
            this.tabConfig.title = '审批放款-雅居乐-恒泽(已开启金蝶云审批)';
          } else {
            this.tabConfig.title = '审批放款-雅居乐-恒泽(线下人工审批)';
          }
        }
      });
    }
    this.subDefaultValue = 'DOING'; // 重置子标签默认
    if (this.defaultValue === 'C' && this.tabConfig.title.includes('审批放款-雅居乐-恒泽') && !!!init) {
      this.pushApproval(1);
    } else if (this.defaultValue === 'A' && this.tabConfig.title.includes('审批放款-雅居乐-恒泽')) {
  ...
  public onPage(e?: { page: number, first?: number, pageSize?: number, pageCount?: number }, types?: number) {
    if (this.tabConfig.title.includes('审批放款-雅居乐-恒泽') && this.defaultValue === 'A') {
  ...
  watchApproval(type?: number) {
    ...
    this.xn.avenger.post2('/jd/approval_progress', {}).subscribe(data => {
      if (data.ret !== 0) {
        if (this.subDefaultValue === 'SPECIAL') {
          this.cancelSpecial(this.subDefaultValue);
        } else {
  ...
  pushApproval(type?: number) {
    const that = this;
    this.xn.avenger.post2('/jd/push_progress', {}).subscribe(data => {
      if (data.ret !== 0) {
    ...
    this.xn.dragon.post('/trade/get_mainflowid_pool_info', params).subscribe(x => {
      if (x.ret === 0 && x.data.capitalPoolId !== '') {
        this.xn.router.navigate(['/agile-hz/assets-management/capital-pool'], {
          queryParams: {
            title: '项目管理-雅居乐-恒泽>' + (x.data.type === 1 ? 'ABS储架项目' : '再保理银行项目') + '>'
