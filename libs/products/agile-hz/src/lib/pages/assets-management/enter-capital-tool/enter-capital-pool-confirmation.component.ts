...
export class EnterpoolComponent implements OnInit {
    tabConfig: any;
    ...
    public isFirst = true;
    public headquarters = HeadquartersTypeEnum[4];
    fitProject = '';
    ...
    public handleHeadClick(paramBtnOperate: ButtonConfigModel) {
        ...
        switch (paramBtnOperate.operate) {
            case 'enter-capitalpool':
                // 移入资产池
                this.enterCapitalpool(1, this.headquarters);
                break;
            case 'delete-capitalpool':
                // 移出资产池
                this.enterCapitalpool(2);
                break;
            case 'batch-information':
                // 批量补充信息
                this.batchModify();
                break;
            case 'download-file':
    ...
    /**
     *  移入/移出资产池
     * @param paramType 1-移入资产池 2-移出资产池
     * @param headquarters 总部公司
     */
    ...
    public exportCapital() {
        ...
        XnModalUtils.openInViewContainer(this.xn, this.vcr, CapitalPoolExportListModalComponent, params).subscribe(x => {
            ...
            this.xn.api.dragon.download('/sub_system/yjl_system/yjl_pool_excel_download', param).subscribe((v: any) => {
                this.xn.loading.close();
                this.xn.api.dragon.save(v._body, '交易清单.xlsx');
            }, () => {
                this.xn.loading.close();
            });
        });
