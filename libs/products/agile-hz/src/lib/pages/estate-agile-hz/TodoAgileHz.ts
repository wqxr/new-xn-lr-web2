...
import { applyFactoringTtype, HeadquartersTypeEnum, SelectOptions } from 'libs/shared/src/lib/config/select-options';
import { ProxyType } from 'libs/shared/src/lib/config/enum/common-enum';

/**
 *  首页代办
 */
export default class TodoAgileHz {
    ...
    static headquarters = HeadquartersTypeEnum[4];
    static factoringAppId = `${applyFactoringTtype['广州恒泽商业保理有限公司']}`;
        ...
        rowButtons: [
            {
                ...
                click: (base: DragonCommBase, record, xn, vcr) => {
                    // 代办的拆分，避免有采购融资相关的业务 2 表示为采购融资相关业务
                    record.proxyType = record.proxyType === ProxyType.AVENGER ? ProxyType.AVENGER : ProxyType.AGILE_HZ;
