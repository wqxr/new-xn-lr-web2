...
import { IsProxyDef } from 'libs/shared/src/lib/config/enum/index';
...
export class MachineListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // 自定义筛选条件status传值范围定义
  public customSearchNumber: number = CustomSearchNumber['AGILE_HZ']
  ...
  getCustomlist(): void {
    ...
    XnModalUtils.openInViewContainer(this.xn, this.vcr, DragongetCustomListComponent, params).subscribe((x: any[]) => {
      this.onPage({page: this.paging});
    });
  }
  ...
  onScroll($event: any) {
    ...
    while (headNumber >= 2) { // 获取冻结列Dom对象
      const Column = $('.height')
        .find(`.head-height tr th:nth-child(${headNumber}), .table-height tr td:nth-child(${headNumber})`);
    ...
    XnModalUtils.openInViewContainer(this.xn, this.vcr, EditModalComponent, params)
      .subscribe((v) => {
        if (v) {
          ...
          this.xn.loading.open()
          this.xn.dragon.post(paramBtnOperate.post_url, {mainFlowIdList: this.selectedItems.map(v => v.mainFlowId), ...v,})
            .subscribe(x => {
              this.xn.loading.close()
              if (x.ret === 0) {
  ...
  downloadFile(paramBtnOperate: ButtonConfigModel): void {
    ...
    XnModalUtils.openInViewContainer(this.xn, this.vcr, DownloadAttachmentsModalComponent, params).subscribe(data => {
      if (data !== '') {
        this.xn.loading.open();
        this.newSelectItems = this.selectedItems.filter((x: any) => x.isProxy === IsProxyDef.YJL_ABS);
        const newparam = {
          mainFlowIdList: [],
          fileTypeKey: {},
        ...
        if (data.downloadRange === 'all') {
          newparam.mainFlowIdList = this.listInfo.map(c => c.mainFlowId);
        ...
        if ((data.downloadRange === 'selected' && this.oldSelectItems.length > 0)) {
  ...
  downloadApprovallist(paramItem: any) {
    ...
    XnModalUtils.openInViewContainer(
      ...
      if (newParam.type === 'all' || (newParam.type === 'selected' && this.newSelectItems.length > 0)) {
        const paramsTotal = Object.assign({}, newParam, this.buildParams(this.currentSubTab.params));
  ...
  subZhongdengRegister() {
    ...
    this.oldSelectItems = this.selectedItems.filter(x => (x.isProxy !== IsProxyDef.YJL_ABS)); // 老业务
  ...
  handmore(item: any) {
    ...
    if (item.nowProcedureId === '@end') {
      this.xn.router.navigate([`/agile-hz/record/todo/view/${item.recordId}`]);
    }
    ...
    if (!isOk) {
      ...
      if (item.nowProcedureId === 'operate' || item.nowProcedureId === 'review') {
        if (item.flowId === 'dragon_platform_verify' && item.zhongdengStatus === 1) {
          ...
          this.xn.router.navigate([`/agile-hz/record/todo/edit/${item.recordId}`]);
        }
      } else {
        this.xn.router.navigate([`/agile-hz/record/todo/view/${item.recordId}`]);
      }
...
export const FlowIdObj = {
  DOING: ['yjl_financing_pre', 'yjl_financing', 'yjl_platform_verify', 'yjl_supplier_sign'],
  TODO: ['yjl_financing_sign', 'yjl_factoring_passback', 'yjl_factoring_risk', 'verificating_500', 'wait_verification_500'],
  DONE: ['loaded_500'],
  SPECIAL: [],
  stop: ['99', '100']
};
