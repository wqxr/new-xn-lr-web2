...
export class AgileFinancingSign implements IFlowCustom {
    ...
    afterSubmitandGettip(svrConfig: any): Observable<any> {
        if (svrConfig.procedure.procedureId === 'operate') {
            ...
            if (this.localStorageService.caCheMap.get('yjl_financing_sign').onLineStatus) {
                // 线上签署
            ...
            this.localStorageService.caCheMap.delete('yjl_financing_sign');
        ...
        return this.xn.dragon.post('/flow/preSubmit', params)
            .pipe(map(json => {
                if (json.data.length === 0) {
                    ...
                    json.data.flowId = 'yjl_financing_sign';
                    json.data.isProxy = 57;
                    const contracts = json.data;
                    this.localStorageService.setCacheValue('yjl_financing_sign', { onLineStatus: !contracts[0].readonly });
