 ...
 * All rights reserved.
 *
 * @file：AgileHzFinancing
 * @summary：雅居乐-恒泽 供应商提交资料
 * @version: 1.0
 * **********************************************************************
...
export class AgileHzFinancing implements IFlowCustom {
    ...
    preSubmit(svrConfig: any, formValue: any): Observable<any> {
        if (svrConfig.procedure.procedureId === 'operate') {
            ...
            let invoiceFile = formValue.invoice; // 发票文件
            const { dealContract, certificateFile, accountInfo, businessLicense } = formValue;
            ...
            try {
                ...
                if (businessLicense === '') {
                    alert.push(`${alert.length + 1}、营业执照文件未上传`);
                }
                if (invoiceFile === '') {
                ...
                if (alert.length) {
                    ...
                    return of({
                        ...
                        params: this.confirmAccountInfo(formValue, svrConfig.record.mainFlowId)
                    });
                }
            } catch (e) {
                console.log('msg:', e);
    ...
    /**
     * a）是否已有内容【合同扫描件、资质证明、营业执照】
     * b）是否已有内容【发票信息（发票列表所有信息）】
     * c）是否满足【发票含税金额总计大于等于应收账款金额】
     * 以上校验成功后，显示账号确认modal
     * @param formValue
     */
    confirmAccountInfo(formValue: any, mainFlowId: string): { checker: any[], title: string, buttons: string[] } {
