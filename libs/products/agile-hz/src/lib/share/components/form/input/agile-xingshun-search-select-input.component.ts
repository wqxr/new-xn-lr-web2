...
@Component({
    selector: 'agile-xingshun-search-select-input',
    template: `
    <div [formGroup]="form">
...
@DynamicForm({ type: 'search-select-yjl', formModule: 'dragon-input' })
export class SearchSelectInputComponent implements OnInit {
    ...
    config: any = {
        post_url: '/contract/first_contract_info/get_project', // 雅居乐提单 监听保理商值 获取对应合同组
    ...
    ngOnInit() {
        ...
        // 雅居乐一次转让合同 适用项目/适用收款单位
        if (['fitProject', 'fitDebtUnit'].includes(this.row.checkerId) && !this.ctrlFactor) {
            this.onInitOptions({ isProxy: 57 }, this.config.project_url);
        }
        // 雅居乐提单 监听保理商值 获取对应合同组
        if (this.ctrlFactor) {
            if (this.ctrlFactor?.value) {
                this.onInitOptions({ isProxy: 57, factoringAppId: applyFactoringTtype[this.ctrlFactor.value] }, this.config.post_url);
            }
            this.ctrlFactor.valueChanges.subscribe(v => {
                this.row.selectOptions = [];
                this.ctrl.setValue('')
                if (v) {
                    this.onInitOptions({ isProxy: 57, factoringAppId: applyFactoringTtype[v] }, this.config.post_url);
                }
            })
        }
