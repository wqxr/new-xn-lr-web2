...
export class EditComponent implements AfterViewInit, AfterViewChecked, OnInit {
    ...
    private showSubmit(recordId, fresh?) {  // fresh 为可选，true的时候则刷新，不拉取actions的最后一项，拉checker
        this.xn.dragon.post('/flow/showSubmit', {
            ...
            if (isfresh.length !== 0 && this.flowId !== 'yjl_factoring_risk') {
                this.buildRows(true);
            } else {
    ...
    // 是否显示退回原因图标
    showRejectIcon() {
        return ((this.svrConfig.record.flowId === 'yjl_financing' || this.svrConfig.record.flowId === 'yjl_platform_verify')
    ...
    public onSubmit() {
        this.innerSubmit(() => {
            for (const checker of this.svrConfig.checkers) {
                ...
                // 雅居乐-恒泽-发起提单 渠道处理
                if (checker.checkerId === 'productType' && this.flowId === 'yjl_financing_pre') {
    ...
    public otherJudge(row): boolean {
        ...
        let flag = false;
        if (row.checkerId === 'dealContract' && row.flowId === 'yjl_platform_verify') {
            ...
            return flag;
        } else if (row.checkerId === 'dealContract' && row.flowId === 'yjl_financing') {
            // 供应商上传资料-交易合同必填
            return !rowVal;
        } else if (row.checkerId === 'performanceFile' && row.flowId === 'yjl_platform_verify') {
            // 平台审核上传资料-履约证明必填
            flag = rowVal ? JSON.parse(rowVal).some(x => !(x.percentOutputValue && x.payType)) : true;
            return flag;
        } else if (row.checkerId === 'performanceFile' && row.flowId === 'yjl_financing') {
            // 供应商上传资料-履约证明必填
            return !rowVal;
    ...
    // 显示暂存按钮
    showTempClick() {
        return (this.svrConfig.flow.flowId === 'yjl_platform_verify' && this.svrConfig.record.nowProcedureId === 'operate')
    ...
    private copyDraft(shows: any[], hides: any[], fresh: boolean): void {
        ...
        // memo字段赋值-雅居乐-恒泽数据对接 平台审核
        if (['yjl_platform_verify'].includes(this.svrConfig.flow.flowId)
    ...
    rejectBtnShow(flowId: string): boolean {
        const notShowFlow = ['yjl_financing', 'yjl_platform_verify', 'yjl_factoring_risk']; // 雅居乐-恒泽主流程 隐藏中止按钮
