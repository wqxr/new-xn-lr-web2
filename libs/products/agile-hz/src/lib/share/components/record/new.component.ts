...
export class NewComponent implements OnInit, AfterViewInit {
    ...
    private doShow() {
        ...
        if (this.flowId === 'yjl_financing_pre') { // 发起提单
        ...
        postObj.factoringAppId = `${applyFactoringTtype['广州恒泽商业保理有限公司']}`;
    ...
    private request(method: string) {
        ...
        for (const checker of this.svrConfig.checkers) {
            ...
            if (this.mainForm.value[checker.checkerId] &&
                this.flowId === 'sub_dragon_book_change' &&
                (checker.checkerId === 'receive' ||
                    checker.checkerId === 'changePrice')) {
                // 特殊过滤：雅居乐台账修改预录入应收账款金额
                let receive = '';
                this.mainForm.value[checker.checkerId].split(',').forEach(v => {
                    receive += v;
                });
                this.mainForm.value[checker.checkerId] = receive;
            }
        ...
        this.xn.api.dragon.post(`/flow/${method}`, params).subscribe(json => {
            ...
            this.xn.router.navigate([`/agile-hz/record/record/${this.flowId}`]);
        });
    }
