...
export class RecordViewComponent implements OnInit {
    ...
    editAction(val: any) {
        ...
        if ((data.status !== 1 && data.status !== 0) || !XnUtils.getRoleExist(data.nowRoleId, this.xn.user.roles, 1)) {
            this.xn.router.navigate([`/agile-hz/record/${this.flowId}/view/${data.recordId}`]);
        } else {
            this.xn.router.navigate([`/agile-hz/record/${this.flowId}/edit/${data.recordId}`]);
        }
    }

    onNewRecord() {
        this.xn.router.navigate([`/agile-hz/record/new/${this.flowId}`]);
    }
}
