...
@Component({
    ...
    <modal #modal [backdrop]="'static'" [keyboard]="false" [animation]="false">
        <form class="form-horizontal" (ngSubmit)="handleSubmit()" [formGroup]="mainForm" *ngIf="!!mainForm">
            ...
            <modal-body>
                <div *ngIf="params?.type===999;else block" class="clearfix" style="max-height: calc( 100vh - 400px);overflow-y: auto;overflow-x:hidden;">
                ...
                <ng-template #block>
                 <div class="clearfix" style="max-height: calc( 100vh - 400px);overflow-y: auto;overflow-x:hidden;" #block>
                  <div class="form-group" *ngFor="let row of params.checker">
            ...
            <modal-footer>
                ...
                <button type="primary" *ngIf="params?.changeAccount" class="btn btn-success"
                (click)="changeAccount()"
                >申请变更账号
                </button>
        ...
        this.xn.router.navigate([`/agile-hz/record/new/`],
            {
                queryParams: {
                    id: 'sub_yjl_change',
                    relate: 'mainFlowId',
                    relateValue: this.params.mainFlowId,
                }
            });
    }
