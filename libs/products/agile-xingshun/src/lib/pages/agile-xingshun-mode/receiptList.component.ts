 ...
 * 1.0                hucongying        雅居乐-星顺数据对接       2021-01-05
 * **********************************************************************
 */
...
import { EditModalComponent, EditParamInputModel } from '../../share/modal/edit-modal.component';
...
export class RecepitListComponent implements OnInit {
   ...
   * @param head 表头配置
   */
    public uploadContract(row: any, head: any) {
        const params = {
            title: `上传${head.label}`,
            checker: [
                {
        ...
        XnModalUtils.openInViewContainer(this.xn, this.vcr, NewFileModalComponent, params).subscribe(v => {
            ...
            this.xn.api.dragon.post(url, param).subscribe((x) => {
                this.xn.loading.close();
                if (x.ret === 0) {
    ...
    */
    ...
    /**
     *  下载合同
     * @param btn 表头按钮组 {label:string,operate:string,value:string,value2?:string,disabled:boolean}
     */
    downloadContract(btn: { label: string, operate: string, value: string, value2?: string, disabled: boolean }) {
        const oldSelectItems = this.selectedItems.filter(x => (x.isProxy === 18)); // 老业务
        if (oldSelectItems.length > 0) {
            return this.xn.msgBox.open(false,
                [
                    '您选择的交易，存在' + oldSelectItems.length + '笔旧的交易',
                    ...oldSelectItems.map(o => o.mainFlowId),
                    '不能进行"下载合同"操作'
                ]);
        }
        const checkers = [
            {
                checkerId: 'contentType', name: 'contentType', required: true,
                type: 'checkbox', title: '下载内容',
                selectOptions: [
                    {
                    ...
                    },
                    {
                        ...
                        memo: '项目公司需要线下用印文件包括：付款确认书、两次应收账款通知书回执'
                    },
                    {
                        label: '《应收账款转让通知书（适用于保理商向雅居乐下属公司出具）》',
                        value: 'second_yjl_common_05_contract',
                        memo: '预计在向供应商集中付款后15个工作日左右可下载保理商已签署通知书'
                    },
                ]
            }
        ];
        ...
        XnModalUtils.openInViewContainer(this.xn, this.vcr, EditModalComponent, params)
            .subscribe((v) => {
                if (v === null) {
                    ...
                    this.xn.loading.open();
                    const param = {
                        mainFlowIdList: this.selectedItems.map(c => c.mainFlowId),
                        contentType: v.contentType.split(',').filter(c => c !== '').map(c => c),
                    };
                    this.xn.api.dragon.download('/trade/down_file', param).subscribe((v: any) => {
                        this.xn.loading.close();
                        this.xn.api.dragon.save(v._body, '合同文件.zip');
                    });
                }
            });
    }
