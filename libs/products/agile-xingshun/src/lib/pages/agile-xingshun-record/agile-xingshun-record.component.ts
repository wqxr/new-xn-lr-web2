...
@Component({
    templateUrl: './agile-xingshun-record.component.html',
    styleUrls: ['./agile-xingshun-record.component.css']
})
export class AgileXingshunRecordComponent implements OnInit, AfterViewInit {

    flowId1 = 'yjl_financing_pre_common';  // 新万科流程
    ...
    doProcess(record: any) {
        ...
        if ((record.status !== 1 && record.status !== 0)
            ...
            this.xn.router.navigate([`/agile-xingshun/record/${this.flowId1}/view/${record.recordId}`]);
        } else {
            this.xn.router.navigate([`/agile-xingshun/record/${this.flowId1}/edit/${record.recordId}`]);
        }
    }
    ...
    // 发起提单
    newProcess() {
        this.xn.router.navigate([`/agile-xingshun/record/new/yjl_financing_pre_common/${HeadquartersTypeEnum[4]}`]);
    }
