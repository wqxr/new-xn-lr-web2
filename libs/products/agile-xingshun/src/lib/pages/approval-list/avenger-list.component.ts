 ...
 * All rights reserved.
 *
 * @file：AgileXingshunAvengerListComponent
 * @summary：雅居乐-星顺审批放款列表
 * @version: 1.0
 * **********************************************************************
...
export class AgileXingshunAvengerListComponent implements OnInit {
    ...
    */
    public initData(paramTabValue: string, init?: boolean,) {
        ...
        if (this.tabConfig.title.includes('审批放款-雅居乐-星顺') && (this.defaultValue === 'A' || this.defaultValue === 'C')) {
            this.xn.api.post('/tool/is_jindie', {}).subscribe(x => {
                if (x.data) {
                    this.is_jindie = x.data.isOpen;
                    if (this.is_jindie === 1) {
                        this.tabConfig.title = '审批放款-雅居乐-星顺(已开启金蝶云审批)';
                    } else {
                        this.tabConfig.title = '审批放款-雅居乐-星顺(线下人工审批)';
                    }
                }
            });
        }
        this.subDefaultValue = 'DOING'; // 重置子标签默认
        if (this.defaultValue === 'C' && this.tabConfig.title.includes('审批放款-雅居乐-星顺') && !!!init) {
            this.pushApproval(1);
        } else if (this.defaultValue === 'A' && this.tabConfig.title.includes('审批放款-雅居乐-星顺')) {
    ...
    public onPage(e?: { page: number, first?: number, pageSize?: number, pageCount?: number }, types?: number) {
        if (this.tabConfig.title.includes('审批放款-雅居乐-星顺') && this.defaultValue === 'A') {
   ...
   */
    ...
    private buildParams(addparams: number) {
        ...
        if (addparams !== undefined) {
            ...
            // jdLoanFailStatus  雅居乐-星顺放款异常状态  0=放款成功 1=放款异常  addparams=5 已放款
            if (addparams === 5) { params.jdLoanFailStatus = this.currentTab.value === 'G' ? 1 : 0 }
