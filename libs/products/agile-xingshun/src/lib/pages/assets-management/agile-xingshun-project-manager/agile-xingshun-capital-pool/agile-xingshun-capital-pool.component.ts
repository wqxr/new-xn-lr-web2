 ...
 * All rights reserved.
 *
 * @file：AgileXingshunCapitalpoolComponent
 * @summary：雅居乐-星顺-项目管理-基础资产列表
 * @version: 1.0
 * **********************************************************************
...
import { AgileXingshunSingleSearchListModalComponent, SingleListParamInputModel } from '../../../../share/modal/single-searchList-modal.component';
import ProjectManagerCapitalList from './agile-xingshun-capital-pool';
declare const moment: any;
declare const $: any;

@Component({
    selector: 'agile-xingshun-capital-management',
    templateUrl: `./agile-xingshun-capital-pool.component.html`,
    styleUrls: ['./agile-xingshun-capital-pool.component.css']
})
export class AgileXingshunCapitalpoolComponent implements OnInit, AfterViewInit, OnDestroy {
    //表格配置
    tabConfig: any;
    // 当前标签页tabList
    currentTab: any;
    ...
    ngOnInit(): void {
        ...
        this.router.data.subscribe(x => {
            ...
            console.log("cpaitalTadeList==", this.tabConfig);
    ...
    */
            ...
            for (const search of this.searches) {
                if (!XnUtils.isEmpty(this.arrObjs[search.checkerId])) {
                    if (search.checkerId === 'isHeadPreDate') {
                        ...
                        console.log("surveyStatus===", this.arrObjs[search.checkerId], this.searches);
                        if (arr && arr.length) {
    ...
    navBack() {
        if (this.isMachineenter === true) {
            ...
            this.xn.router.navigate(['/agile-xingshun/assets-management/projectPlan-management'], {
                queryParams: {
    ...
    */
    ...
    tradeChange() {
        ...
        XnModalUtils.openInViewContainer(this.xn, this.vcr, AgileXingshunSingleSearchListModalComponent, params).subscribe(v => {
        });
                ...
                if (x.data.isCapitalPoolSample) {
                    ...
                    XnModalUtils.openInViewContainer(this.xn, this.vcr, EditModalComponent, params).subscribe(v => {
                        if (v === null) {
                            ...
                            //抽样页面
                            this.xn.router.navigate(['/agile-xingshun/assets-management/capital-sample'], {
                                queryParams: {
    ...
    */
    dataAnalyse() {
        this.xn.router.navigate(['/agile-xingshun/assets-management/capital-data-analyse'], {
            queryParams: {
                totalCount: this.pageConfig.total,
                sumReceive: this.sumReceive,
                ...this.queryParams
    ...
    */
    ...
    viewProgress(mainFlowId) {
        this.xn.router.navigate([`agile-xingshun/main-list/detail/${mainFlowId}`]);
    }
    ...
    public handleRowClick(item: any, btn: ButtonConfigModel) {
        if (btn.operate === 'sub_start_survey') {
            ...
            if (item.flowId === 'yjl_financing_pre_common') {
     ...
     */
    addTransaction() {
        this.xn.router.navigate([`/agile-xingshun/assets-management/enter-pool`], {
            queryParams: {
