 ...
 * All rights reserved.
 *
 * @file：CountryGradenProjectmanagePlanComponent
 * @summary：雅居乐-星顺-项目管理-专项计划列表
 * @version: 1.0
 * **********************************************************************
 * revision             author            reason             date
 * 1.0                hucongying        雅居乐-星顺数据对接     2020-11-03
 * **********************************************************************
 */
...
import ProjectManagerplanList from './agile-xingshun-project-plan';

declare const moment: any;
declare const $: any;

@Component({
    selector: 'agile-xingshun-project-plan-list',
    templateUrl: `./agile-xingshun-project-plan-list.component.html`,
    styles: [`
    .item-box {
        position: relative;
        display: flex;
...
export class AgileXingshunProjectmanagePlanComponent implements OnInit {
    tabConfig: any;
    ...
    public isPlatForm: boolean = this.xn.user.orgType === 99; // 是否平台角色（平台角色只能有只读权限）
    ...
    ngOnInit(): void {
        this.router.queryParams.subscribe(x => {
            this.title = '项目管理-雅居乐-星顺>' + x.title + '>' + x.projectName + '-' + x.headquarters;
    ...
    getPlan(paramInfo: any) {
        ...
        if (paramInfo.poolFlag === 1) {
            ...
            this.xn.router.navigate(['/agile-xingshun/capital-pool/trading-list'], {
                queryParams: {
                    capitalId: paramInfo.capitalPoolId,
                    capitalPoolName: paramInfo.capitalPoolName || '',
                    headquarters: HeadquartersTypeEnum[4],
                    isProxy: paramInfo.isProxy,
                    type: 1,
                    currentPage: 1,
                    isLocking: paramInfo.isLocking
                }
    ...
    addTransaction(item: any, btn: ButtonConfigModel) {
        ...
        if (item.poolFlag === 1) {
            ...
            this.xn.router.navigate(['/agile-xingshun/capital-pool/main-list'], {
                queryParams: {
    ...
    changeInfo(paramInfo: string) {
        XnModalUtils.openInViewContainer(
            ...
            { project: paramInfo, type: 1, isVanke: true }
        ).subscribe(v => {
            if (v === 'ok') {
                this.onPage({ page: this.paging });
            }
  ...
  */
     ...
     */
    navBack() {
        this.xn.router.navigate(['/agile-xingshun/assets-management/project-list'], {
            queryParams: {
    ...
    setPlan() {
        XnModalUtils.openInViewContainer(
            ...
            this.vcr,
            CapitalPoolsetProjectPlanModalComponent,
            { project: this.projectId, type: 2, isVanke: true }
        ).subscribe(v => {
            if (v === 'ok') {
                this.onPage({ page: this.paging });
            }
    ...
    */
    public capitalItemForm(params?: { id: string; name: string }) {
        ...
        const checkers = () => {
            ...
            return params
                ? [capitalPoolName]
                : [capitalPoolName, headquarters, storageRack];
        };
        XnModalUtils.openInViewContainer(
            this.xn,
            this.vcr,
                    ...
                    if (action === 'rename') {
                        this.xn.api
                            .post(
                                ...
                                Object.assign({ capitalPoolId, headquarters: HeadquartersTypeEnum[4] }, data)
                            )
                            .subscribe(() => {
                                this.onPage({ page: this.paging });
                            });
                        ...
                        this.xn.api
                            .post(
                                ...
                                Object.assign(
                                    ...
                                    { headquarters: HeadquartersTypeEnum[4], isProxy: data.isProxy, capitalPoolName: data.capitalPoolName })
                            )
                            .subscribe(() => {
                                this.onPage({ page: this.paging });
                            });
