...
import { GeneratingContractModalComponent } from 'libs/products/new-agile/src/lib/share/modal/generating-contract-modal.component';
import { GeneratingContractStampModalComponent } from 'libs/products/new-agile/src/lib/share/modal/generating-contract-stamp-modal.component';
import { DownloadAttachmentsModalComponent } from 'libs/products/new-agile/src/lib/share/modal/download-attachments-modal.component';
import { BulkUploadModalComponent } from 'libs/products/new-agile/src/lib/share/modal/bulk-upload-modal.component';
import { MfilesViewModalComponent } from 'libs/products/new-agile/src/lib/share/modal/mfiles-view-modal.component';
...
export class CapitalPoolCommListComponent extends CommonPage implements OnInit {
    ...
    // 影印件对应关系
    private PhotoCopy = {
        雅居乐集团控股有限公司: ['photoCopy01', 'photoCopy02', 'photoCopy03', 'photoCopy04', 'photoCopy05', 'photoCopy06'],
    };
    // 选中集合
            ...
            if (this.headquarters === HeadquartersTypeEnum[4]) {
                ...
                for (let i = 0; i < this.heads.length; i++) {
                    if (this.heads[i].checkerId === 'pdfProjectFiles') {
                        this.heads[i].title = '《付款确认书（适用于雅居乐控股向供应商出具）》';
                        break;
                    }
                }
            }
    ...
    public viewProcess(item: any) {
        this.xn.router.navigate([`agile-xingshun/main-list/detail/${item}`]);
    }
    ...
    public generateAndSign() {
        ...
        // 接口
        const urls = {
            capital01: '/llz/capital_list/capital01', // 应收账款转让通知书（适用于保理商向雅居乐控股出具）
            update_capital01: '/llz/capital_list/update_capital01',
            exp_capital01: { noStamp: false },
            capital02: '/llz/capital_list/capital02', // 应收账款转让通知书（适用于保理商向雅居乐下属公司出具）
    ...
    public exportPayList() {
        ...
        if (param !== HeadquartersTypeEnum[4]) {
            this.xn.msgBox.open(false, '总部公司不是雅居乐集团控股有限公司！');
            return;
        }
