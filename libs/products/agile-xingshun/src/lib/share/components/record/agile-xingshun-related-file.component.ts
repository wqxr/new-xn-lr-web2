...
@Component({
  selector: 'agile-xingshun-file-related',
  templateUrl: './agile-xingshun-related-file.component.html',
  styles: [
    `
      .table {
        font-size: 13px;
...
export class AgileXingshunRelatedFileComponent implements OnInit {
  /** 列表数据 */
  data: any[] = [];
  heads: any[];
  ...
  public download() {
    ...
    if (this.xn.user.orgType === 1) {
      ...
      if (this.params.logs.length) {
        logsData = this.params.logs.filter(
