...
export class NewComponent implements OnInit, AfterViewInit {
    ...
    private doShow() {
        ...
        if (this.flowId === 'yjl_financing_pre_common') { // 发起提单
        ...
        postObj.factoringAppId = `${applyFactoringTtype['深圳市星顺商业保理有限公司']}`;
    ...
    private request(method: string) {
        ...
        this.xn.api.dragon.post(`/flow/${method}`, params).subscribe(json => {
            ...
            this.xn.router.navigate([`/agile-xingshun/record/record/${this.flowId}`]);
        });
    }
