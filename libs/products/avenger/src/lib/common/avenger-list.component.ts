...
import { EditParamInputModel, EditModalComponent } from 'libs/shared/src/lib/public/avenger/modal/edit-modal.component';
...
export class AvengerListComponent implements OnInit {
    ...
    */
    public initData(paramTabValue: string, init?: boolean, ) {
        ...
        if (this.defaultValue === 'C' && this.tabConfig.title.includes('审批放款列表') && !!!init) {
            this.pushApproval(1);
    ...
    public handleHeadClick(paramBtnOperate: ButtonConfigModel) {
        ...
        if (paramBtnOperate.operate === 'initiate_examination') {    //
            // let freezeOne = this.selectedItems.map((x: any) => x.freezeOne);
            ...
            this.xn.router.navigate([`/avenger/record/avenger/new/sub_approval_again_530`],
                {
                    queryParams: {
   ...
   */
        ...
        this.xn.dragon.post('/trade/get_mainflowid_pool_info', params).subscribe(x => {
            if (x.ret === 0 && x.data.capitalPoolId !== '') {
                if (x.data.headquarters ===HeadquartersTypeEnum[5]) {
                    ...
                    this.xn.router.navigate(['/vanke/vanke/capital-pool'], {
                        queryParams: {
    ...
    public handleClick(type: number) {
        if (type === 1) {
            this.xn.router.navigate([`/avenger/record/avenger/new/sub_approval_honor_530`]);
        } else if (type === 2) {
            this.xn.router.navigate([`/avenger/record/avenger/new/sub_approval_return_530`]);
        } else if (type === 3) {
