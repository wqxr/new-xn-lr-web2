...
export class Financing500 implements IFlowCustom {
    ...
    preSubmit(svrConfig: any, formValue: any): Observable<any> {// 供应商
        if (svrConfig.procedure.procedureId === '@begin') {
            const datalist = Number(formValue.receiveDate.replace(/-/g, ''));
            const addday = Number(JSON.parse(svrConfig.checkers[0].value)[4].value);
            const controlday = this.calcDate(datalist, moment().format('YYYY-MM-DD'), addday);
            if (controlday) {
                ...
                if (svrConfig.record && svrConfig.record.mainFlowId !== '') {
                    ...
                    return this.xn.avenger.post('/apply/applyInfo/compare', params).pipe(map(x => {
                        if (x.data) {
