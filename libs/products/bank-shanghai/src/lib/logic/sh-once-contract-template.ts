...
export default class ShOnceContractTemplateTab {
    static contractGroupList = { // 合同组列表界面页面配置
        heads: [
            {label: '备注', value: 'contractListName', width: '14'},    // 合同组名称
            {label: '适用总部公司', value: 'headquarters', width: '14'},
            {label: '合同组', value: 'fitProject', type: 'fit', width: '10'},  // 适用项目
            {label: '适用收款单位', value: 'fitDebtUnit', type: 'fit', width: '12'},
            {label: '合同模板', value: 'contractTemplate', type: 'contract', width: '25'},
            {label: '合同组状态', value: 'contractStatus', type: 'contractStatus', width: '8'},
            {label: '最后修改时间', value: 'updateTime', type: 'date', width: '12'},
        ],
        searches: [
            {
                title: '合同组名称',
    ...
    static contractTemplateList = {  // 合同模板列表templateName
        heads: [
            {label: '合同模板名称', value: 'templateFile', type: 'contract', width: '35'},  // templateName
            {label: '合同模板类型', value: 'templateType', type: 'templateType', width: '11'},
            {label: '签署方', value: 'signer', type: 'signer', width: '11'},
            {label: '特定条件', value: 'specialCondition', type: 'specialCondition', width: '12'},
            {label: '合同模板状态', value: 'templateStatus', type: 'templateStatus', width: '11'},
            {label: '最后修改时间', value: 'updateTime', type: 'date', width: '15'},
        ],
        searches: [
            {
                title: '合同模板名称',
                    ...
                    subTabList: [
                        {
                            ...
                            edit: {
                                headButtons: [
                                    {
                                        label: '新增',
                                        operate: 'sub_sh_first_contract_add',
                                        post_url: '/',
                                        disabled: false,
                                ...
                                rowButtons: [
                                    {
                                        label: '修改',
                                        operate: 'sub_sh_first_contract_modify',
                                        post_url: '',
                                        click: (xn: XnService, params) => {
                                            const rolesArr = xn.user.roles.filter((x) => {
                                                return x === 'operator';
                                            ...
                                            if (!(rolesArr && rolesArr.length)) {
                                                ...
                                                {
                                                    queryParams: {
                                                        id: 'sub_sh_first_contract_modify',
                                                        relate: 'mainIds',
                                                        relateValue: [params],
                                                    }
                                                });
                                    ...
                                    {
                                        label: '删除',
                                        operate: 'sub_sh_first_contract_delete',
                                        post_url: '',
                                        click: (xn: XnService, params) => {
                                            const rolesArr = xn.user.roles.filter((x) => {
                                                return x === 'operator';
                                            ...
                                            if (!(rolesArr && rolesArr.length)) {
                                                ...
                                                {
                                                    queryParams: {
                                                        id: 'sub_sh_first_contract_delete',
                                                        relate: 'mainIds',
                                                        relateValue: [params],
                                                    }
                                                });
                            ...
                            searches: ShOnceContractTemplateTab.contractGroupList.searches,
                            params: 0,
                            headText: [...ShOnceContractTemplateTab.contractGroupList.heads],
                        }
                    ],
                    ...
                    subTabList: [
                        {
                            ...
                            searches: ShOnceContractTemplateTab.contractTemplateList.searches,
                            params: 1,
                            headText: [...ShOnceContractTemplateTab.contractTemplateList.heads],
                        }
                    ],
