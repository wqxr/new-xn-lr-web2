...
@Component({
  selector: 'lib-drawing-list',
  templateUrl: './drawing-list.component.html',
  styleUrls: ['./drawing-list.component.css']
})
export class ShanghaiSupplierDrawingListComponent implements OnInit {
  tabConfig: any;
  ...
  toCallInterfaceAgain(item: any) {
    // console.log('toCallInterfaceAgain----', item);
    // 打开弹框
    const params: ShSingleListParamInputModel = {
      title: '上海银行接口状态',
      ...
      multiple: null,
      heads: [
        { label: '异常类型', value: 'status', type: 'interfaceErrType' },
        { label: '接口名称', value: 'taskType', type: 'shBankTaskTypes' },
        { label: '接口异常时间', value: 'updateTime', type: 'date' },
        { label: '异常内容', value: 'response', type: 'text' }
      ],
      searches: [],
      key: 'id',
      data: [],
      ...
      rightButtons: [{ label: '取消', value: 'cancel' }, { label: '重新调用接口', value: 'repeatRequst' }],
      options: {
        paramsType: 3
      }
    };
