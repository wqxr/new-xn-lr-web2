...
@Component({
  selector: 'lib-files-archive',
  templateUrl: './files-archive.component.html',
  styleUrls: ['./files-archive.component.css']
})
export class FilesArchiveComponent implements OnInit {
  ...
  constructor(private fileSignService: FileSignService, private fileAdapter: FileAdapterService, private xn: XnService,
              private vcr: ViewContainerRef, private router: Router, private showModalService: ShowModalService) {}
  ...
  onPage(pageConfig: { pageIndex: number, pageSize: number, total?: number }, searchModel: {[key: string]: any} = {}) {
    ...
    this.xn.dragon.postMapCatch('/shanghai_bank/sh_supplier_archive/getArchiveList', params).subscribe({
      next: (res) => {
  ...
  antSignFiles(item: {filesList: any[], appId: string}, type: number) {
    ...
    const param = {
      ...
      postUrl: '/shanghai_bank/sh_supplier_archive/updateArchive',
    };
