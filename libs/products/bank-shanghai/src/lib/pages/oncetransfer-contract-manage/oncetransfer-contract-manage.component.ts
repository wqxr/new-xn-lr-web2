 ...
 * All rights reserved.
 *
 * @file：oncetransfer-contract-manage.component
 * @summary：多标签页列表项 根据tab-pane.ts中的配置
 * @version: 1.0
 * **********************************************************************
 * revision             author            reason             date
 * 1.0                   yu           一次转让合同管理      2019-09-19
 * **********************************************************************
 */
...
@Component({
  selector: 'lib-oncetransfer-contract-manage',
  templateUrl: './oncetransfer-contract-manage.component.html',
  styleUrls: ['./oncetransfer-contract-manage.component.css']
})
export class OncetransferContractManageComponent implements OnInit {
  tabConfig: any;
  ...
  constructor(private xn: XnService, private vcr: ViewContainerRef, public bankManagementService: BankManagementService,
              private router: ActivatedRoute, public hwModeService: HwModeService, private localStorageService: LocalStorageService) {
  }
  ...
  public onPage(e?: { page: number, first?: number, pageSize?: number, pageCount?: number }) {
    ...
    if (this.tabConfig.value === 'once_contract_manage' && this.defaultValue === 'B') {
      this.xn.dragon.post('/contract/first_contract_info/init_first_template', {}).subscribe(x => {
  ...
  */
   ...
   * 构建参数
   */
  private buildParams(flag: any, type: any) {
    // 分页处理
    const params: any = {
  ...
  public handleRowClick(item, btn: ButtonConfigModel, i: number) {
    ...
    switch (btn.operate) {
      // 修改
      case 'sub_sh_first_contract_modify':
        btn.click(this.xn, mainFlowIds);
        break;
      // 删除
      case 'sub_sh_first_contract_delete':
        btn.click(this.xn, mainFlowIds);
        break;
    }
  }
