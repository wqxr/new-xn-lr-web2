...
export class PuhuiCompanyListComponent implements OnInit {
  ...
  public onPage(e?: { pageIndex: number, pageSize?: number, total?: number }, searchModel?: { [key: string]: any }) {
    ...
    this.xn.dragon.post2('/shanghai_bank/sh_general/shEAccountApplyList', params).subscribe(x => {
      this.loading = false
  ...
  updateCheckStatus(appId: string) {
    this.loading = true;
    this.xn.dragon.post2('/shanghai_bank/sh_general/shPHBankVerifyStatusRefresh', { appId }).subscribe(x => {
      this.loading = false;
      if (x.ret === RetCodeEnum.OK) {
  ...
  updateActiveStatus(appId: string) {
    this.loading = true;
    this.xn.dragon.post2('/shanghai_bank/sh_general/shPHAcctActiveStatusRefresh', { appId }).subscribe(x => {
      this.loading = false;
      if (x.ret === RetCodeEnum.OK) {
        this.message.success('更新成功！');
        this.onPage(this.pageConfig, {});
  ...
  viewAccountDetail(appId: string) {
    this.xn.router.navigate(['/bank-shanghai/puhui-account-detail'],
      {
        queryParams: {
          appId: appId || ''
        }
  ...
  */
  exportData() {
    ...
    this.xn.dragon.download('/shanghai_bank/sh_general/shEAccountApplyListDownload', params)
      .subscribe(x => {
