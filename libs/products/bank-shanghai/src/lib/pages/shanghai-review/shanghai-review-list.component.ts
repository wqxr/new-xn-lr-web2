...
import { MFileViewerService } from '../../share/components/mfile-viewer/mfile-viewer.service';
import { Router } from '@angular/router';
import { SelectItemsModel } from 'libs/shared/src/lib/config/checkers';

@Component({
  selector: 'lib-shanghai-review-list',
  templateUrl: './shanghai-review-list.component.html',
  styleUrls: ['./shanghai-review-list.component.css']
})
export class ShanghaiReviewListComponent implements OnInit {
    ...
    this.xn.dragon.postMapCatch('/trade/get_column', params).subscribe({
      next: (res) => {
        if (res && res.ret !== 0) {
          ...
          const customSearches = res.data.data.find((x: any) => x.status.toString() === '69');
          const customTableHeads = res.data.data.find((x: any) => x.status.toString() === '70');
  ...
  onPage(pageConfig: { pageIndex: number, pageSize: number, total?: number }, searchModel: { [key: string]: any }) {
    ...
    this.xn.dragon.postMapCatch('/sh_bank_trade/list', params).subscribe({
      next: (res) => {
        if (res && res.ret !== 0) {
          this.showPostError(res);
        } else if (res && res.ret === 0 && res.data) {
  ...
  private buildParams(searchModel: { [key: string]: any }) {
    ...
    for (const sortKey of Object.keys(this.sortModels)) {
      params.order = [
        { name: SortFieldsEnum[sortKey], asc: Number(this.sortModels[sortKey]) }
      ];
    }
  ...
  verify(row: any) {
    if (!row.recordId){
      ...
      this.xn.router.navigate(['/bank-shanghai/record/bank-review'], {
        queryParams: {
   ...
   */
  viewRecord(row: any) {
    this.xn.router.navigate(['/bank-shanghai/record/bank-view'], {
      queryParams: {
        flowId: "sh_vanke_bank_verify",
