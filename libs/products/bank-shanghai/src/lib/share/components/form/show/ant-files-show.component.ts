...
@DynamicForm({ type: 'ant-files', formModule: 'dragon-show' })
export class AntFilesShowComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.files = XnUtils.parseObject(this.row.data, []);
    }
    ...
    downloadFiles(files: any[]) {
        ...
        if (['sh_vanke_bank_verify', 'so_bank_verify'].includes(this.row.flowId)) {
            if (files && files.length === 1){
                this.fileAdapter.saveFile(files[0], `${FilesName[this.row.checkerId]}-${files[0]?.fileName}`, 'dragon');
            } else {
                const param = {
                ...
                this.xn.dragon.download(`/shanghai_bank/${this.svrConfig.record.flowId?.substring(0, 2)}_supplier/shFlowFileDown`, param).subscribe({
                    next: (v: any) => {
                        if (!!v && !!v.ret && v.ret !== 0) {
                            this.showPostError(v);
                        } else if (!!v && !!v._body){
                            this.xn.api.save(v._body, `${this.svrConfig?.debtUnit}-${FilesName[this.row.checkerId]}.zip`);
                        }
                    },
                    error: (err: any) => {
