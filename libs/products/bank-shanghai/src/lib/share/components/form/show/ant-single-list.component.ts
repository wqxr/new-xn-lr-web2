...
@Component({
  selector: 'lib-ant-single-list-show',
  templateUrl: './ant-single-list.component.html',
  styleUrls: ['./ant-single-list.component.css']
})
@DynamicForm({ type: 'ant-list', formModule: 'dragon-show' })
export class AnTSingleListShowComponent implements OnInit {
  @Input() row: any;
  ...
  initList() {
    // 配置
    const { columns, tableHeadBtn } = AntTableListConfig.getConfig(this.row.checkerId);
    this.columns = [
      ...columns,
      {
        title: '操作', width: '10%', buttons: [
          {
            text: '查看', type: 'link', iif: (item: TableData, btn: ColumnButton, column: Column) =>
              ['invoice', 'baseFile'].includes(this.row.checkerId), iifBehavior: 'hide',
            click: (e: any) => {
              console.log('查看 click', e);
              const files = this.row.checkerId === 'baseFile' ? e.fileInfo : [e];
              this.viewFiles(XnUtils.parseObject(files, []), false);
            },
          },
          {
            text: '下载', type: 'link', iif: (item: TableData, btn: ColumnButton, column: Column) =>
              ['baseFile'].includes(this.row.checkerId), iifBehavior: 'hide',
            click: (e: any) => {
          ...
          {
            text: '查看文件', type: 'link', iif: (item: TableData, btn: ColumnButton, column: Column) =>
              ['dealContract'].includes(this.row.checkerId), iifBehavior: 'hide',
            click: (e: any) => {
              console.log('查看文件 click', e);
    ...
    if (['baseFile'].includes(this.row.checkerId) && ['sh_vanke_platform_verify'].includes(this.row.flowId)) {
      const resList = XnUtils.parseObject(this.row.data, []) || [];
      ...
      let data = XnUtils.parseObject(this.row.data, []);
      this.lists = typeof data === 'string' ? XnUtils.parseObject(data, []) : data;
    }
    const needStatistic = columns.some((x: any) => x.hasOwnProperty('statistical'));
    this.statisticColumn = this.lists && this.lists.length && needStatistic ? columns.map((x: any) => x.index) : [];
    this.calcAlertClass();
  }
  ...
  public handleHeadClick(btn: ButtonGroup) {
    if (btn.btnKey === 'basicInfoAllDownload') {
      ...
      this.xn.dragon.download('/shanghai_bank/sh_general/downContractFiles', param).subscribe((v: any) => {
        if (!!v && !!v.ret && v.ret !== 0) {
          this.showPostError(v);
        } else if (!!v && !!v._body){
          this.xn.api.save(v._body, `${this.svrConfig?.debtUnit}-交易合同.zip`);
        }
      });
      ...
      this.xn.dragon.download('/shanghai_bank/sh_general/downInvoiceFiles', param).subscribe((v: any) => {
        if (!!v && !!v.ret && v.ret !== 0) {
          this.showPostError(v);
        } else if (!!v && !!v._body){
  ...
  downloadFiles(files: any[], e: any) {
    ...
    if (files && files.length === 1){
      this.fileAdapter.saveFile(files[0], `${FilesName[e.fileType]}-${files[0]?.fileName}`, 'dragon');
    } else {
      const param = {
        appId: this.svrConfig?.debtUnitId,
        fileType: e.fileType
      ...
      this.xn.dragon.download(`/shanghai_bank/${this.svrConfig.record.flowId?.substring(0, 2)}_supplier/shBaseFileDown`, param).subscribe((v: any) => {
        if (!!v && !!v.ret && v.ret !== 0) {
          ...
          this.xn.api.save(v._body, `${this.svrConfig?.debtUnit}-${FilesName[e.fileType]}.zip`);
        }
      });
    }
  }
  ...
  private calcAlertClass() {
    if (this.row.checkerId === 'refEnterprise') {
      // 提示
      const isLimit = this.lists.some((x: any) => x.refLevel.toString() === '4');
      const tipStr = XnUtils.distinctArray(this.lists.map((x: any) => x.refContent));
