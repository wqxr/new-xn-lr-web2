...
@DynamicForm({ type: 'ant-text', formModule: 'dragon-show' })
export class AntTextShowComponent implements OnInit {
        ...
        if (['sh_vanke_bank_verify', 'so_bank_verify'].includes(this.row.flowId)){
            switch (this.row.name) {
                case 'linkageRadio':
                    ...
                    this.label = !!relation ? `该企业与${relation}有关联` : `该企业与${Relation[1]}、${Relation[2]}、${Relation[3]}无关联`;
                    break;
                case 'productType':
                    const {type, selectBank} = XnUtils.parseObject(this.row.data, {}) || {};
