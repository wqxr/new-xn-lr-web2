...
import { ShViewChangeAccountComponent } from '../../modal/vanke-change-account.component';
import { ShDataChangeModalComponent } from '../../modal/vanke-dataChange-modal.component';
import { ShAuditStandardModalComponent } from '../../modal/audit-standard-modal.component';
import { ShIntelligenceStandardModalComponent } from '../../modal/intelligence-standard-modal.component';
import { ShRejectModalComponent } from '../../modal/reject-modal.component';
...
export class EditComponent implements AfterViewInit, AfterViewChecked, OnInit, OnDestroy {
    ...
    private showSubmit(recordId, fresh?) {
        this.xn.dragon.post('/flow/showSubmit', { recordId }).subscribe(json => {
            ...
            if (this.flowId === 'sh_vanke_financing') {
                this.isVanke = 'vanke';
            }
    ...
    // 是否显示退回原因图标
    showRejectIcon() {
        return ((['sh_vanke_financing', 'sh_vanke_platform_verify'].includes(this.svrConfig.record.flowId))
            && this.svrConfig.procedure.procedureId === 'operate');
    }

    // 是否显示数据变动图标
    showVankeChangesIcon() {
        return this.svrConfig.record.flowId === 'sh_vanke_platform_verify' && this.svrConfig.wkType && this.svrConfig.wkType === 1;
    }
    ...
    public otherJudge(row): boolean {
        ...
        let flag = false;
        if (row.checkerId === 'dealContract' && row.flowId === 'sh_vanke_platform_verify') {
            ...
            return flag;
        } else if (row.checkerId === 'dealContract' && row.flowId === 'sh_vanke_financing') { // 供应商上传资料-交易合同必填
            return !rowVal;
        } else if (row.checkerId === 'performanceFile' && row.flowId === 'sh_vanke_platform_verify') {
            flag = JSON.parse(rowVal).some(x => !(x.percentOutputValue && x.payType));
            return flag;
        } else if (row.checkerId === 'performanceFile' && row.flowId === 'sh_vanke_financing') { // 供应商上传资料-履约证明必填
            return !rowVal;
    ...
    onRowOtherClick(value: string, row: any) {
        if (['accountInfo'].includes(row.checkerId)) {
            ...
            window.open(CommonEnum[row.checkerId], '_blank');
        }
    }
    ...
    // 显示暂存按钮
    showTempClick() {
        return (this.svrConfig.flow.flowId === 'sh_vanke_platform_verify' && this.svrConfig.record.nowProcedureId === 'operate')
            || (this.svrConfig.flow.flowId === 'sh_vanke_financing' && this.svrConfig.record.nowProcedureId === 'operate');
    }
    ...
    private copyDraft(shows: any[], hides: any[], fresh: boolean): void {
        ...
        if (['sh_vanke_platform_verify'].includes(this.svrConfig.flow.flowId)
    ...
    public showAuditStandard(type: number) {
        ...
        if (['sh_vanke_platform_verify'].includes(this.svrConfig.record.flowId)){
    ...
    getDivideLine(checker: CheckersOutputModel) {
        // 是否是上海银行业务
        const isSh = ['sh_vanke_platform_verify'].includes(checker.flowId);
        return {
            show: ['businessRelated', 'headquarters'].includes(checker.checkerId) && isSh,
            text: ['businessRelated', 'headquarters'].includes(checker.checkerId) ? CommonEnum[checker.checkerId] : '',
