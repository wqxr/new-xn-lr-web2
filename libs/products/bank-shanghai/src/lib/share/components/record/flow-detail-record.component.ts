...
@Component({
    selector: 'xn-sh-flow-detail-record-component',
...
export class FlowDetailRecordComponent implements OnInit {
    ...
    public showAuditingstandard(paramsRow): boolean {
        return (paramsRow.appId === this.xn.user.appId || this.xn.user.orgType === 3) &&
            paramsRow.flowId === 'sh_vanke_platform_verify';
    }
    ...
    public viewaduitStandard(row: any) {
        ...
        if (['sh_vanke_platform_verify'].includes(row.flowId)) {
    ...
    public checkSubprocess(paramRow: any) {
        this.xn.router.navigate([`/bank-shanghai/record/view/${paramRow.recordId}`]);
