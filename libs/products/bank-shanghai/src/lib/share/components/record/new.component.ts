...
export class NewComponent implements OnInit, AfterViewInit {
    ...
    private doShow() {
        ...
        if (['vanke_financing_pre', 'sh_vanke_financing_pre'].includes(this.flowId)) {
        ...
        'sub_system_match_qrs', 'sub_sh_platform_check_retreat'].includes(this.Relate.flowId)) {
    ...
    private request(method: string) {
        ...
        // 多保理-添加保理商
        if (['sub_sh_supplementaryinfo_input', 'sub_dragon_book_stop', 'sub_vanke_system_check_reject', 'sh_vanke_financing_pre',
        'sub_sh_platform_check_retreat'].includes(this.flowId)){
        ...
        this.xn.api.dragon.post(`/flow/${method}`, params).subscribe(json => {
            ...
            this.xn.router.navigate([`/bank-shanghai/record/record/${this.flowId}`]);
        });
    }
