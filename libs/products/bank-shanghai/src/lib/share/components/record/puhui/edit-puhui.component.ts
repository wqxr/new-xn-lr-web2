...
export class PuhuiEditComponent implements AfterViewInit, AfterViewChecked, OnInit, OnDestroy {
  ...
  private showSubmit(recordId: string, fresh?: boolean) {
    this.xn.dragon.post('/flow/showSubmit', { recordId }).subscribe(json => {
      ...
      if (this.flowId === ShPuhuiFlowIdEnum.SUB_SH_PRATTWHITNEY_INPUT) {
        // 供应商复核导航
  ...
  showSupplierStepForm() {
    // sub_sh_prattwhitney_input 指供应商发起开户流程
    // procedureId为经办步骤时，供应商需要补正资料
    return this.flowId === ShPuhuiFlowIdEnum.SUB_SH_PRATTWHITNEY_INPUT && this.svrConfig.procedure.procedureId === ProcduresIdEnum.BEGIN;
  }
   ...
   */
  canSubmit(): boolean {
    if (this.flowId === ShPuhuiFlowIdEnum.SUB_SH_PRATTWHITNEY_PLATFORM_VERIFY && this.svrConfig.procedure.procedureId === ProcduresIdEnum.OPERATE) {
      // 平台审核初审
  ...
  platAsyncCheckInfo() {
    ...
    if (action.operator === OperateType.REJECT
      && action.flowId === ShPuhuiFlowIdEnum.SUB_SH_PRATTWHITNEY_INPUT
   ...
   */
  fetchCheckInfo() {
    // 补正原因取actions数组的倒数第二项
    const action = this.newSvrConfig.actions[this.newSvrConfig.actions.length-2];
    if (action.operator === OperateType.REJECT && this.showPuhuiMemo(action)) {
      // 默认展开补正原因信息
