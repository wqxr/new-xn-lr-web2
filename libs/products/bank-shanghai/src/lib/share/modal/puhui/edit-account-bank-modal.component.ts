...
import { editShowFields, reActiveShowFields, resultShowFields } from './edit-account-bank-modal-config';
...
export class BankAccountEditModalComponent {
  ...
  // 修改对公账号
  public editShowFields: CheckersOutputModel[] = XnUtils.deepClone(editShowFields);
  // 确认电子账户
  ...
  activeAccount() {
    ...
    this.xn.dragon.post2('/shanghai_bank/sh_bank_gateway/shPHModifyBindAccount', { appId: this.xn.user.appId, ...actInfo }).subscribe(async res => {
  ...
  */
  sendCode() {
    ...
    this.xn.dragon.post('/shanghai_bank/sh_bank_gateway/shPHDynamicCode', { appId: this.xn.user.appId }).subscribe(res => { })
    // 60s倒计时
    this.time = 60;
