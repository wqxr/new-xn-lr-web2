...
@Component({
    templateUrl: `./push-vanke-modal.component.html`,
    selector: 'push-vanke-modal',
    styles: [`
    .modal-title{
        height:25px;
    }
...
export class ShangHaiPushVankeModalComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    onOk() {
        ...
        // 查验推送数据
        this.xn.dragon.post('/sub_system/sh_vanke_system/verify_realy_pay_batch', {
            mainFlowIdList,
            type: this.type
        }).subscribe(x => {
            if (x.ret === 0 && x.data && x.data.data.length) {
                ...
                this.xn.loading.open();
                // 批量向万科推送清单
                this.xn.dragon.post('/sub_system/sh_vanke_system/push_realy_pay_batch', {
                    mainFlowIdList,
                    type: this.type
                }).subscribe(x => {
