...
import { TabConfigModel, TabListOutputModel, SubTabListOutputModel } from 'libs/shared/src/lib/config/list-config-model';
import CommUtils from 'libs/shared/src/lib/public/component/comm-utils';
import { HwModeService } from 'libs/shared/src/lib/services/hw-mode.service';
import { XnService } from 'libs/shared/src/lib/services/xn.service';
import { Observable, of, fromEvent } from 'rxjs';
import VankeDataChangeList from '../../logic/vanke-dataChange';
import { ShSingleListParamInputModel, ShSingleSearchListModalComponent } from './single-searchList-modal.component';

declare const moment: any;
@Component({
    templateUrl: `./vanke-dataChange-modal.component.html`,
    selector: 'lib-sh-datachange-modal',
    styles: [`
        .modal-title{
            height:25px;
        }
...
export class ShDataChangeModalComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    open(params: any): Observable<any> {
        ...
        this.params = params;
        // console.log(params);
        // 台账-线上数据B-万科数据对接情况A  平台审核C
        this.defaultValue = params.defaultValue || 'A';
        // 台账platmachineAccount  平台审核platVerify
        this.tabConfig = VankeDataChangeList.getConfig(params.type);
    ...
    onPage(e?: { start?: number, length?: number }): void {
        ...
        this.xn.dragon.post(this.currentTab.post_url, params).subscribe(x => {
            if (x.ret === 0) {
                ...
                if (x.data && x.data.lastList && x.data.lastList.length) {

                    // 动态构建表头
                    const headText = JSON.stringify(this.currentSubTab.headText);
                    const newHead = [...JSON.parse(headText)].slice(0, 2);
                    x.data.lastList.forEach((t: any, i: number) => {
                        if (i > 0) {
                            newHead.push(
                                {
                                    label: this.getMaxtime(t.versionsTime.split('_')), value: `field${i}`, type: `field${i}`
                                }
                            );
                        }
                    });
                    ...
                    this.datalist = VankeDataChangeList.reorganizeData(x.data, heads); // 组装数据
    ...
    public invoiceListFilter(invoiceList) {
        ...
        const arr = invoiceList.map(list => list.number);
    ...
    public viewMore(paramItem: any[]) {
        ...
        const params: ShSingleListParamInputModel = {
            ...
            heads: [
                ...
                { label: '发票不含税金额', value: 'invNoTaxAmt', type: 'money' },
