 ...
 * All rights reserved.
 *
 * @file：CountryGradenCapitalpoolComponent
 * @summary：碧桂园-项目管理-基础资产列表
 * @version: 1.0
 * **********************************************************************
...
import ProjectManagerCapitalList from './country-graden-capital-pool';
declare const moment: any;
declare const $: any;

@Component({
    selector: 'country-graden-capital-management',
    templateUrl: `./country-graden-capital-pool.component.html`,
    styleUrls: ['./country-graden-capital-pool.component.css']
})
export class CountryGradenCapitalpoolComponent implements OnInit, AfterViewInit, OnDestroy {
    tabConfig: any;  //表格配置
    ...
    */
    ...
    navBack() {
        if (this.isMachineenter === true) {
            ...
            this.xn.router.navigate(['/country-graden/assets-management/projectPlan-management'], {
                queryParams: {
    ...
    */
        ...
        this.xn.api.dragon.post(urls.generate, params).subscribe(con => {
            ...
            if (result.length) {
                ...
                result.forEach(x => {
                    //碧桂园
                    if (this.proxy === 54) {
                        if (x.label === ' 应收账款债权转让通知书（保理商向项目公司出具）') {
                            x['config']['text'] = '（盖章）';
    ...
    viewProgress(mainFlowId) {
        this.xn.router.navigate([`country-graden/main-list/detail/${mainFlowId}`]);
    }
     ...
     */
    addTransaction() {
        this.xn.router.navigate([`/country-graden/assets-management/enter-pool`], {
            queryParams: {
                ...
                if (x.data.isCapitalPoolSample) {
                    ...
                    XnModalUtils.openInViewContainer(this.xn, this.vcr, EditModalComponent, params).subscribe(v => {
                        if (v === null) {
                            ...
                            //抽样页面
                            this.xn.router.navigate(['/country-graden/assets-management/capital-sample'], {
                                queryParams: {
    ...
    // 数据分析
    dataAnalyse() {
        this.xn.router.navigate(['/country-graden/assets-management/capital-data-analyse'], {
            queryParams: {
                totalCount: this.pageConfig.total,
                sumReceive: this.sumReceive,
                ...this.queryParams
