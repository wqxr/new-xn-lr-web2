 ...
 * 1.0                hucongying       碧桂园数据对接      2020-10-30
 * **********************************************************************
 */
 ...
 */
@Component({
  selector: 'graden-home-comm-list-component',
  templateUrl: './home-comm-list.component.html',
  styleUrls: [
    './home-comm-list.component.css'
  ]
})
export class GradenHomeCommListComponent extends CommonPage implements OnInit {
  @Input() supers: any;
  total = 0;
  rows: any[] = [];
  words = '';
  ...
  public viewProcess(paramMainFlowId: string) {
    this.xn.router.navigate([`country-graden/main-list/detail/${paramMainFlowId}`]);
  }
  ...
  public doProcess(record: any) {
    record.proxyType = record.proxyType === ProxyType.AVENGER ? ProxyType.AVENGER : ProxyType.COUNTRY_GARDEN;
