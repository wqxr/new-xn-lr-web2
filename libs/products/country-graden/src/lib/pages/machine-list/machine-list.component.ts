 ...
 * 1.0                 hucongying      碧桂园数据对接      2020-10-26
 * **********************************************************************
 */
...
import { MfilesViewModalComponent as ApiMfilesViewModalComponent, MfilesViewModalComponent } from 'libs/shared/src/lib/public/modal/mfiles-view-modal.component';
...
import { PushDataToCountryGrdenModalComponent } from '../../share/modal/pushData-toGraden-modal.component';
import { BgyDataChangeModalComponent } from '../../share/modal/bgy-dataChange-modal.component';
...
export class MachineListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // 自定义筛选条件status传值范围定义
  public customSearchNumber: number = CustomSearchNumber['COUNTRY_GRADEN']
  ...
  toViewOnlineData(item) {
    ...
    XnModalUtils.openInViewContainer(this.xn, this.vcr, BgyDataChangeModalComponent, params).subscribe(() => {
    });
  }
  ...
  */
  ...
  public handleHeadClick(paramBtnOperate: ButtonConfigModel) {
    switch (paramBtnOperate.operate) {
      case 'download_file':
        this.downloadFile(paramBtnOperate) // 下载附件
        break;

      case 'export_file':
        this.downloadApprovallist(paramBtnOperate) // 导出清单
        break;

      case 'sub_zhongdeng_register':
        this.subZhongdengRegister() // 中登登记
        break;

      case 'add-data':
        this.batchModify() // 补充信息
        break;

      case 'reject-program':
        this.rejectProgram() // 退单流程
        break;

      case 'edit-helpHandler':
        this.editHelpHandler() // 修改协助处理人(平台)
        break;

      case 'pushList-bgy':
        this.pushBgyData() // 向碧桂园推送清单(保理商)
        break;

      case 'pushBank-bgy':
        this.pushBgyBank() // 向碧桂园推送托管行信息(保理商)
        break;
      ...
      case 'set-pool-advise':
        this.setPoolAdvise(paramBtnOperate) // 建议入池
        break;

      case 'del-pool-advise':
        this.setPoolAdvise(paramBtnOperate) // 取消建议入池
        break;

      case 'set-check-priority':
        this.setCheckPriority(paramBtnOperate) // 设置审核优先级
        break;

      case 'set-loan-priority':
        this.setLoanPriority(paramBtnOperate) // 设置放款优先级
        break;

      case 'set-plan-loan':
        this.setPlanLoan(paramBtnOperate) // 设置预计放款时间
        break;
    ...
    XnModalUtils.openInViewContainer(this.xn, this.vcr, DownloadAttachmentsModalComponent, params).subscribe(data => {
      if (data !== '') {
        ...
        if (data.downloadRange === 'all') {
          newparam.mainFlowIdList = this.listInfo.map(c => c.mainFlowId); // 刷选新碧桂园业务
  ...
  public batchModify() {
    ...
    this.newSelectItems.map((sel) => sel.headquarters = sel.headquarters === HeadquartersTypeEnum[9] ? '碧桂园' : sel.headquarters);
    ...
    this.xn.router.navigate(['/country-graden/batch-modify'], {
  ...
  */
  pushBgyData() {
    ...
    const heads = [
      ...
      { label: '供应商名称', value: 'debtUnit', },
      { label: '项目公司名称', value: 'projectCompany', },
    ...
    const resheads = [...heads,
    { label: '推送结果', value: 'result', type: 'result' },
    { label: '描述', value: 'message' }
    ] as ListHeadsFieldOutputModel[];
    let mainFlowIdList = this.selectedItems.map(v => v.mainFlowId);
    const params = {
      title: '向碧桂园系统推送数据：',
      heads,
      resheads,
      selectedItems: this.selectedItems,
      url: '/sub_system/bgy_system/push_info',
      query: { mainFlowIdList },
    };
    XnModalUtils.openInViewContainer(this.xn, this.vcr, PushDataToCountryGrdenModalComponent, params).subscribe((v) => {
      if (v.action === 'ok') {
        this.onPage({ page: this.paging });
        this.selectedItems = [];
      }
  ...
  */
  pushBgyBank() {
    ...
    const heads = [
      ...
      { label: '托管行户名', value: 'bankAccountName' },
    ...
    const params = {
      ...
      resheads,
      selectedItems: this.selectedItems,
      url: '/sub_system/bgy_system/push_bacth_info',
      query: { mainFlowIdList },
    };
    XnModalUtils.openInViewContainer(this.xn, this.vcr, PushDataToCountryGrdenModalComponent, params).subscribe((v) => {
      if (v.action === 'ok') {
  ...
  public changeInfo(item: any) {
    let allTabFlag = this.rowAuthJudge(item, 'DOING');
   ...
   * @param info 行信息
   */
  bgyeDockOpenView(info: any): boolean {
  ...
  handmore(item: any) {
    ...
    if (item.nowProcedureId === '@end') {
      this.xn.router.navigate([`/country-graden/record/todo/view//${item.recordId}`]);
    }
    ...
    if (!isOk) {
      ...
      if (item.nowProcedureId === 'operate' || item.nowProcedureId === 'review') {
        if (item.flowId === 'bgy_platform_verify' && item.zhongdengStatus === 1) {
          this.xn.msgBox.open(false, '该流程中登登记处于登记中,不可处理');
          return;
        } else {
          this.xn.router.navigate([`/country-graden/record/todo/edit/${item.recordId}`]);
        }
      } else {
        this.xn.router.navigate([`/country-graden/record/todo/view//${item.recordId}`]);
      }
...
export const FlowIdObj = {
  'DOING': ['bgy_financing_pre', 'bgy_financing', 'bgy_platform_verify'],
