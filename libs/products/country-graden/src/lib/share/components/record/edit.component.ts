...
import { BgyDataChangeModalComponent } from '../../modal/bgy-dataChange-modal.component';
import { BgySuspendModalComponent } from '../../modal/bgy-suspend-modal.component';
import { BgyRejectModalComponent } from '../../modal/reject-modal.component';
...
export class CountryGradenEditComponent implements AfterViewInit, AfterViewChecked, OnInit, OnDestroy {
    ...
    public showIconBgyChanges = false;
    ...
    private showSubmit(recordId, fresh?) {  // fresh 为可选，true的时候则刷新，不拉取actions的最后一项，拉checker
        this.xn.dragon.post('/flow/showSubmit', {
            ...
            if (this.showBgyChangesIcon() && this.svrConfig.procedure.procedureId !== 'review') {
                this.startSetInterval(5000, false);
            }
            this.showIconReject = (this.showRejectIcon() && this.showIconReject);
            this.showIconBgyChanges = (this.showBgyChangesIcon() && this.showIconBgyChanges);
    ...
    // 是否显示退回原因图标
    showRejectIcon() {
        return ((this.svrConfig.record.flowId === 'bgy_financing' || this.svrConfig.record.flowId === 'bgy_platform_verify')
            && this.svrConfig.procedure.procedureId === 'operate');
    }

    // 是否显示数据变动图标
    showBgyChangesIcon() {
        return this.svrConfig.record.flowId === 'bgy_platform_verify';
    }
    ...
    public otherJudge(row): boolean {
        ...
        let flag = false;
        if (row.checkerId === 'dealContract' && row.flowId === 'bgy_platform_verify') {
            ...
            return flag;
        } else if (row.checkerId === 'dealContract' && row.flowId === 'bgy_financing') { // 供应商上传资料-交易合同必填
            return !rowVal;
        } else if (row.checkerId === 'performanceFile' && row.flowId === 'bgy_platform_verify') {
            flag = JSON.parse(rowVal).some(x => !(x.percentOutputValue && x.payType));
            return flag;
        } else if (row.checkerId === 'performanceFile' && row.flowId === 'bgy_financing') { // 供应商上传资料-履约证明必填
            return !rowVal;
    ...
    // 显示暂存按钮
    showTempClick() {
        return (this.svrConfig.flow.flowId === 'bgy_platform_verify' && this.svrConfig.record.nowProcedureId === 'operate')
    ...
    private copyDraft(shows: any[], hides: any[], fresh: boolean): void {
        ...
        // memo字段赋值-碧桂园数据对接 平台审核
        if (['bgy_platform_verify'].includes(this.svrConfig.flow.flowId)
     ...
     * 显示碧桂园数据变动
     */
    private showBgyChangesInfo(init: boolean) {
        this.xn.dragon.post2('/sub_system/bgy_system/get_bgy_change_status', {
            mainFlowId: this.svrConfig.record.mainFlowId
        }).subscribe(x => {
            if (x.ret !== 0) {
                clearInterval(this.changesInterval);
            ...
            if (x.ret === 0 && x.data && x.data.isChange) {
                this.showIconBgyChanges = true;
            } else {
                this.showIconBgyChanges = false;
            }
        });
    }
    ...
    */
    ...
    /**
     * 暂停弹窗，确定后需通过接口向碧桂园反馈信息
     */
    suspend() {
        this.xn.msgBox.open(true, [
            `暂停后，系统会将此业务通过接口退回给碧桂园，需等碧桂园更新此业务数据后才能恢复业务状态。`,
            `请确定是否暂停？`
        ], () => {
            const params = {
                title: '发票列表',
                heads: [
                    { label: '发票代码', value: 'bgyInvoiceCode' },
                    { label: '发票号码', value: 'bgyInvoiceNo' },
                    ...
                    { label: '发票类型名称', value: 'bgyInvoiceName' },
                    { label: '更换原因', value: 'reason' },
                ],
                key: 'bgyInvoiceNo',
                mainFlowId: this.svrConfig.record.mainFlowId,
            };
            XnModalUtils.openInViewContainer(this.xn, this.vcr, BgySuspendModalComponent, params).subscribe((v) => {
                if (v.action === 'ok') {
                    // 调用暂停接口
                    const param = {
                        mainFlowId: this.svrConfig.record.mainFlowId,
                        recordId: this.svrConfig.record.recordId,
                        invoiceList: v.invoiceList
                    };
                    XnUtils.checkLoading(this);
   ...
   */
    ...
    openrejectplatWindow() {
        ...
        XnModalUtils.openInViewContainer(this.xn, this.vcr, BgyRejectModalComponent, detail).subscribe(v => {
            if (v.action === 'cancel') {
                return;
            }
