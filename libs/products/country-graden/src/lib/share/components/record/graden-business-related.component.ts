...
@Component({
    selector: 'graden-business-related',
    templateUrl: './graden-business-related.component.html',
    styles: [
        `
            .table {
                font-size: 13px;
...
export class GradenBusinessComponent implements OnInit {
    ...
    viewRecord(record: string, type: string) {
        if (type && type === 'avenger') {
            ...
            this.xn.router.navigate([`/country-graden/record/todo/view/${record}`]);
        }
    }
