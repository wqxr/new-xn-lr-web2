 ...
 * 1.0                hucongying       碧桂园数据对接     2020-10-28
 * **********************************************************************
 */
...
export class NewComponent implements OnInit, AfterViewInit {
    ...
    ngAfterViewInit() {
        ...
        this.route.queryParams.subscribe(params => {
            ...
            this.bgyPaymentUuidList = params.bgyPaymentUuidList || [],
    ...
    private doShow() {
        ...
        if (this.flowId === 'bgy_financing_pre') { // 碧桂园-发起提单
            postObj.headquarters = this.headquarters;
            postObj.bgyPaymentUuidList = this.bgyPaymentUuidList;
        }
        if (this.flowId === 'sub_bgy_check_fail') { // 碧桂园-发起审核失败流程
    ...
    private request(method: string) {
        ...
        for (const checker of this.svrConfig.checkers) {
            ...
            // 碧桂园-发起提单 渠道处理
            if (checker.checkerId === 'productType' && this.flowId === 'bgy_financing_pre') {
        ...
        this.xn.api.dragon.post(`/flow/${method}`, params).subscribe(json => {
            ...
            this.xn.router.navigate([`/country-graden/record/record/${this.flowId}`]);
        });
    }
