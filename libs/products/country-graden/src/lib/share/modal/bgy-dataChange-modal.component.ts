...
import BgyDataChangeList from './bgy-dataChange'

declare const moment: any;
declare const $: any;
@Component({
    templateUrl: `./bgy-dataChange-modal.component.html`,
    styles: [`
        .modal-title{
            height:25px;
        }
...
export class BgyDataChangeModalComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    open(params: any): Observable<any> {
        ...
        this.tabConfig = BgyDataChangeList.getConfig(params.type);
            ...
            if (x.ret === 0) {
                ...
                if (x.data && x.data.list && x.data.list.length) {
                    ...
                    x.data.list.forEach((t: any, i: number) => {
                        if (i > 0) {
                            newHead.push(
                                { label: moment(t.createTime).format('YYYY-MM-DD HH:mm:ss'), value: `field${i}`, type: `field${i}` }
                            );
                        }
                    });
                    ...
                    this.datalist = BgyDataChangeList.reorganizeData(x.data, heads); // 组装数据
