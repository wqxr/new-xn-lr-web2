...
import { TadeListTabIndex } from 'libs/shared/src/lib/config/angency-auth'
...
export class DragonCapitalpoolComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    public initData(paramTabValue: string, init?: boolean) {
        ...
        if (this.defaultValue === TadeListTabIndex.DEAL_FILE) {
            this.getHeadorSearch(this.currentSubTab);
        } else if (this.defaultValue === TadeListTabIndex.PRODUCT_INFO) {
            // 产品信息页面传值
            this.productParams = {
    ...
    tradeChanges() {
        if (this.defaultValue === TadeListTabIndex.BASE_ASSESTS) {
    ...
    */
    ...
    private buildParams() {
        // 分页处理
        let params: any;
        if (this.defaultValue === TadeListTabIndex.BASE_ASSESTS || this.defaultValue === TadeListTabIndex.DUE_DILIGENCE) {
            params = {
                capitalPoolId: this.capitalPoolId,
                type: this.defaultValue === TadeListTabIndex.BASE_ASSESTS ? 1 : 3,
                pageNo: this.paging,
                pageSize: this.pageConfig.pageSize
            };
        } else if (this.defaultValue === TadeListTabIndex.DEAL_FILE) {
            params = {
            ...
            params = {};
        } else if (this.defaultValue === TadeListTabIndex.SPECIAL_ASSETS) {
            params = {
                capitalPoolId: this.capitalPoolId,
                type: 4,
                pageNo: this.paging,
    ...
    navBack() {
        if (this.isMachineenter === true) {
            ...
            this.xn.router.navigate(['/logan/assets-management/projectPlan-management'], {
                queryParams: {
    ...
    */
                ...
                if (x.data.isCapitalPoolSample) {
                    ...
                    XnModalUtils.openInViewContainer(this.xn, this.vcr, EditModalComponent, params).subscribe(v => {
                        if (v === null) {
                            ...
                            // 抽样页面
                            this.xn.router.navigate(['/logan/assets-management/capital-sample'], {
                                queryParams: {
    ...
    // 数据分析
    dataAnalyse() {
        this.xn.router.navigate(['/logan/assets-management/capital-data-analyse'], {
            queryParams: {
                totalCount: this.pageConfig.total,
                sumReceive: this.sumReceive,
                ...this.queryParams
