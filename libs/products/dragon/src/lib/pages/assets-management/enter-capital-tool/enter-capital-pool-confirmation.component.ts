...
export class DragonEnterpoolComponent implements OnInit {
    ...
    */
    ...
    public exportCapital() {
        ...
        const headquarters = Array.from(new Set(this.selectedItems.map(x => x.headquarters)));
        if (headquarters.length > 1) {
            this.xn.msgBox.open(false, '请选择同一批总部公司的交易进行导出清单');
            return;
        } else {
            ...
            XnModalUtils.openInViewContainer(this.xn, this.vcr, CapitalPoolExportListModalComponent, params).subscribe(x => {
                ...
                console.log('总部公司', headquarters);
                let param: any;
                param = {
                    mainFlowIdList: +x.scope === SelectRange.All
                        ? undefined
                        : this.selectedItems.filter(x => x.headquarters === headquarters[0]).map(x => x.mainFlowId),
                    capitalPoolId: '',
                    scope: x.scope,
                ...
                this.xn.api.dragon.download('/project_manage/file_contract/down_excel', param).subscribe((v: any) => {
                    this.xn.api.dragon.save(v._body, `总部公司为${headquarters[0]}的资产池清单.xlsx`);
                }, () => {
                }, () => {
                    this.xn.loading.close();
                });
