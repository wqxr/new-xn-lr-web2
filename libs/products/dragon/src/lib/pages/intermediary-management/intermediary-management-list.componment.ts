...
export class DragonIntermediaryManagementComponent
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  public initData(paramTabValue: string, init?: boolean) {
    ...
    this.pageConfig.first = this.isFirst
      ? 0
      : (this.queryPaging - 1) * this.pageConfig.pageSize; // 设置当前页码
  ...
  getAgencyTypeList(typeList: string[]) {
    ...
    if (typeList && typeList.length > 0) {
      arr = typeList.map((element) =>
        SelectOptions.getConfLabel('agencyType', element)
      );
    }
    return arr;
  }
  ...
  getIntermediaryUserList(item) {
    const title = this.tabConfig.tabList.filter(
      (x: any) => x.value === this.defaultValue
    )[0].label;
    this.xn.router.navigate(['/logan/intermediary/intermediary-user-list'], {
      queryParams: {
        paging: this.paging,
