...
import { applyFactoringTtype, CustomSearchNumber } from 'libs/shared/src/lib/config/select-options';
...
export class DragonMachineListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // 自定义筛选条件status传值范围定义
  public customSearchNumber: number = CustomSearchNumber['XN_LOGAN']
  ...
  public handleHeadClick(paramBtnOperate: ButtonConfigModel) {
    if (paramBtnOperate.operate === 'download_file') {
      ...
      XnModalUtils.openInViewContainer(this.xn, this.vcr, DownloadAttachmentsModalComponent, params).subscribe(data => {
        if (data !== '') {
          this.xn.loading.open();
          const newMainFlowId = this.listInfo.filter(z => z.isProxy === IsProxyDef.DRAGON);
  ...
  */
      ...
      for (const search of this.searches) {
        ...
        if (!XnUtils.isEmpty(this.arrObjs[search.checkerId])) {
          if (search.checkerId === 'transactionStatus') {
            ...
            params.isProxy = IsProxyDef.DRAGON;
  ...
  downloadApprovallist(paramItem: any) {
    ...
    XnModalUtils.openInViewContainer(
      ...
      this.xn.loading.open();
      const newMainFlowId = this.listInfo.filter(z => z.isProxy === IsProxyDef.DRAGON);
      this.newSelectItems = this.selectedItems.filter((x: any) => x.isProxy === IsProxyDef.DRAGON);
      const newParam = {
