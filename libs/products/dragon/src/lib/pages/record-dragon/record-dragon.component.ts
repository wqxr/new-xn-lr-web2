...
export class RecordDragonComponent implements OnInit, AfterViewInit {
  ...
  doProcess(record: any) {
    ...
    if ((record.status !== 1 && record.status !== 0)
      ...
      this.xn.router.navigate([`/logan/record/${this.flowId}/view/${record.recordId}`]);
    } else {
      this.xn.router.navigate([`/logan/record/${this.flowId}/edit/${record.recordId}`]);
    }
  }
  ...
  newProcess() {
    ...
    XnModalUtils.openInViewContainer(this.xn, this.vcr, EditNewVankeModalComponent, params).subscribe(x => {
      if (!!x) {
        ...
        if (x) {
          this.xn.router.navigate([`/logan/record/new/${this.flowId}/${HeadquartersTypeEnum[5]}`],
            {
              queryParams: {
