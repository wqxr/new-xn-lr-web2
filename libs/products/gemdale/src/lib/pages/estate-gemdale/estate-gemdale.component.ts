...
@Component({
    templateUrl: './estate-gemdale.component.html',
    styles: [
        `.table {
            font-size: 13px;
        }`,
...
export class EstateGemdaleComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (this.xn.user.orgType === 99) {
            ...
            this.todoConfig.fields = XnUtils.distinctArray2(this.todoConfig.fields, 'checkerId'); // 去除相同的项

        }
        this.api.post('/user/todo_count', {}).subscribe((json) => {
            this.todoCount01 = json.data.count1;
            this.todoCount02 = json.data.count2;
            this.countJd = json.data.count_jd;
