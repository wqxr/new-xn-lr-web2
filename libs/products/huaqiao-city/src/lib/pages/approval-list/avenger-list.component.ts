...
export class HuaQiaoCityAvengerListComponent implements OnInit {
    ...
    */
    public initData(paramTabValue: string, init?: boolean, ) {
        ...
        if (this.tabConfig.title.includes('审批放款-华侨城') && (this.defaultValue === 'A' || this.defaultValue === 'C')) {

            this.xn.api.post('/tool/is_jindie', {}).subscribe(x => {
                if (x.data) {
                    this.is_jindie = x.data.isOpen;
                    if (this.is_jindie === 1) {
                        this.tabConfig.title = '审批放款-华侨城(已开启金蝶云审批)';
                    } else {
                        this.tabConfig.title = '审批放款-华侨城(线下人工审批)';
                    }
                }
            });
        }
        this.subDefaultValue = 'DOING'; // 重置子标签默认
        if (this.defaultValue === 'C' && this.tabConfig.title.includes('审批放款-华侨城') && !!!init) {
            this.pushApproval(1);
        } else if (this.defaultValue === 'A' && this.tabConfig.title.includes('审批放款-华侨城')) {
    ...
    dragonviewProcess(paramMainFloId: string){
    ...
    public onPage(e?: { page: number, first?: number, pageSize?: number, pageCount?: number }, types?: number) {
        if (this.tabConfig.title.includes('审批放款-华侨城') && this.defaultValue === 'A') {
   ...
   */
        ...
        this.xn.dragon.post('/trade/get_mainflowid_pool_info', params).subscribe(x => {
            if (x.ret === 0 && x.data.capitalPoolId !== '') {
                this.xn.router.navigate(['/oct/assets-management/capital-pool'], {
                    queryParams: {
                        title: '项目管理-华侨城>' + (x.data.type === 1 ? 'ABS储架项目' : '再保理银行项目') + '>' + x.data.projectName + '-' + x.data.headquarters,
    ...
    // 完成签署，推送数据
    finishSignContract(paramBtnOperate) {
        let isOk1 = false;
        let isOk2 = false;
        let alert1 = '';
        let alert2 = '';
