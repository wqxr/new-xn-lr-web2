...
@Component({
    selector: 'huaqiao-city-project-management-component',
    templateUrl: `./huaqiao-city-project-management.component.html`,
    styles: [`
    .item-box {
        position: relative;
        display: flex;
...
export class HuaQiaoCityProjectmanageComponent implements OnInit {
    tabConfig: any;
    ...
    ngOnInit(): void {
        ...
        this.router.data.subscribe(x => {
            // this.tabConfig = x;
            const undetermTabConfig = ProjectManagerList.getTabConfig(this.localStorageService, this.xn, 'projectList');
    ...
    public handleRowClick(item: any, btn: ButtonConfigModel, i: number) {
        ...
        switch (btn.operate) {
            ...
            case 'notice_manage':  // 提醒管理
                this.xn.router.navigate(['/oct/assets-management/notice-manage'], {
                    queryParams: {
                        project_manage_id: item.project_manage_id,
                        projectName: item.projectName
                    }
                ...
                XnModalUtils.openInViewContainer(
                    ...
                    if (!!data) {
                        ...
                        this.xn.dragon.post('/project_manage/agency/add_agency',
                            { project_manage_id: params ? params.value : null, agencyList }).subscribe(x => {
                                if (x.ret === 0) {
                                    this.xn.msgBox.open(false, '添加成功');
                                    this.onPage({ page: this.paging });
                                }
                            });
                    }
    ...
    */
    public alertRatioForm(params?: { project_manage_id: string; value: any }) {
        ...
        const checkers = [
            ...
            //     title: '预付款警戒比例 ',
            //     memo: '',
            //     value: params ? params.value.prePaymentRatio : null
            // },
            // {
            //     checkerId: 'guaranteGoldRatio',
            //     name: 'guaranteGoldRatio',
            //     required: 1,
            //     type: 'rate-input',
            //     options: { size: { min: 0, max: 100 } },
            //     title: '质保金警戒比例 ',
            //     memo: '',
            //     value: params ? params.value.guaranteGoldRatio : null
            // }
        ];
  ...
  */
    ...
    setUpProject(types: number, paramsInfo?: any) {
        const type = this.defaultValue === 'A' ? 1 : 2;
        const checkers = [{
            title: '项目名称',
            checkerId: 'projectName',
            type: 'text',
            ...
            .subscribe((v) => {
                ...
                if (v === null) {
                    ...
                    if (types === 2) {
                        ...
                        v.headquarters = HeadquartersTypeEnum[10];
