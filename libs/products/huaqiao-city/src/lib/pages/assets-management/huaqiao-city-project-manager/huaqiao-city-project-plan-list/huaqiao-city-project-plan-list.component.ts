...
import ProjectManagerplanList from './huaqiao-city-project-plan';

declare const moment: any;
declare const $: any;

@Component({
    selector: 'huaqiao-city-project-plan-list',
    templateUrl: `./huaqiao-city-project-plan-list.component.html`,
    styles: [`
    .item-box {
        position: relative;
        display: flex;
...
export class HuaQiaoCityProjectmanagePlanComponent implements OnInit {
    tabConfig: any;
    ...
    ngOnInit(): void {
        this.router.queryParams.subscribe(x => {
            this.title = '项目管理-华侨城>' + x.title + '>' + x.projectName + '-' + x.headquarters;
    ...
    getPlan(paramInfo: any) {
        ...
        if (paramInfo.poolFlag === 1) {
            ...
            this.xn.dragon.post('/project_manage/old_pool/pool_info', { capitalPoolId: paramInfo.capitalPoolId }).subscribe(x => {
                if (x.ret === 0) {
                    this.xn.router.navigate(['/oct/trading-list'], {
                        queryParams: {
    ...
    changeInfo(paramInfo: string) {
        const isVanke = this.title.indexOf('华侨城') > 0; // 万科
        XnModalUtils.openInViewContainer(
            this.xn,
            this.vcr,
  ...
  */
     ...
     */
    navBack() {
        this.xn.router.navigate(['/oct/assets-management/project-list'], {
            queryParams: {
