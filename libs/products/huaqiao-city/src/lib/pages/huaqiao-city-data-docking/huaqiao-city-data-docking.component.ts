...
@Component({
    selector: 'huaqiao-city-data-docking',
    templateUrl: `./huaqiao-city-data-docking.component.html`,
    styles: [`
    .item-box {
        position: relative;
        display: flex;
...
export class HuaQiaoCityDatalockingComponent implements OnInit {
    tabConfig: any;
    ...
    */
    ...
    public handleHeadBatchClick(paramBtnOperate: ButtonConfigModel) {
        if (this.selectedItems && this.selectedItems.length > 0) {
            ...
            if (paramBtnOperate.operate === 'oct_financing_pre') {
                const msgs = this.vankePreConditionJudge(this.selectedItems);
                if (msgs.length > 0) {
                    this.xn.msgBox.open(false, msgs);
                } else {
                    this.xn.router.navigate([`/oct/record/new/oct_financing_pre/${HeadquartersTypeEnum[10]}`],
                        {
                            queryParams: {
                                productType: '',
                                selectBank: '0',
