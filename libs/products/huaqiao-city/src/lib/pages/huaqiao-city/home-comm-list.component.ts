 ...
 */
@Component({
  selector: 'huaqiaocity-home-comm-list-component',
  templateUrl: './home-comm-list.component.html',
  styleUrls: [
    './home-comm-list.component.css'
  ]
})
export class HuaQiaoCityCommListComponent extends CommonPage implements OnInit {
  @Input() supers: any;
  total = 0;
  rows: any[] = [];
  words = '';
  ...
  ngOnInit() {
    ...
    this.base = new HuaQiaoCommBase(this, this.superConfig);
  ...
  public doProcess(record: any) {
    if ((record.status !== 1 && record.status !== 0)
      || !XnUtils.getRoleExist(record.nowRoleId, this.xn.user.roles)) {
...
enum EditflowSet {
  ...
  '/oct/record/todo/edit/' = 2,
}
enum ViewflowSet {
  '/console/record/avenger/todo/view/' = 1,
  '/oct/record/todo/view/' = 2,
}
