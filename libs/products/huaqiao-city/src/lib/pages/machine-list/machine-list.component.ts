...
import { HeadquartersTypeEnum, applyFactoringTtype, CustomSearchNumber, EnumShBankTradeStatus } from 'libs/shared/src/lib/config/select-options';
...
export class DemoListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // 自定义筛选条件status传值范围定义
  public customSearchNumber: number = CustomSearchNumber['OCT']
...
*/
  ...
  */
  ...
  enterCapitalPool(paramsValue) {
    ...
    this.xn.dragon.post('/trade/get_mainflowid_pool_info', params).subscribe(x => {
      if (x.ret === 0 && x.data.capitalPoolId !== '') {
        this.xn.router.navigate(['/oct/assets-management/capital-pool'], {
          queryParams: {
            title: '项目管理(华侨城)>' + (x.data.type === 1 ? 'ABS储架项目' : '再保理银行项目') + '>'
    ...
    if (paramBtnOperate.operate === 'download_file') {
      ...
      if (this.selectedItems.length > 0) {
        XnModalUtils.openInViewContainer(this.xn, this.vcr, DownloadAttachmentsModalComponent, params).subscribe(data => {
          if (data !== '') {
            this.xn.loading.open();
            const newMainFlowId = this.listInfo;
            this.newSelectItems = this.selectedItems;
            const newparam = {
              mainFlowIdList: this.selectedItems.map(x => x.mainFlowId),
              fileTypeKey: {},
              isClassify: data.isClassify
            };                                                    // 新业务参数
            if (data.downloadRange === 'selected') {
              if (this.selectedItems.length === 0) {
      ...
      this.newSelectItems = this.selectedItems; // 新业务的退单流程
      ...
      if (pauseItems.length > 0) { // 【平台审核-暂停】
  ...
  public batchModify() {
    ...
    this.newSelectItems.map((sel) => sel.headquarters);
    ...
    this.xn.router.navigate(['/oct/oct/machine_list/batch-modify'], {
  ...
  */
  ...
  private buildCondition(searches) {
    ...
    for (let i = 0; i < searches.length; i++) {
      ...
      if (searches[i].options) {
        obj.options = searches[i].options;
      }
      if(searches[i].selectOptions) {
      ...
      for (const search of this.searches) {
        ...
        if (!XnUtils.isEmpty(this.arrObjs[search.checkerId])) {
          if (search.checkerId === 'transactionStatus') {
            ...
            params.isProxy = 55;
            if (this.subDefaultValue === 'ALL' && Number(this.arrObjs[search.checkerId]) === EnumShBankTradeStatus.stop
            || Number(this.arrObjs[search.checkerId]) === EnumShBankTradeStatus.chargeback) {
            params.tradeStatus = EnumShBankTradeStatus.stop;
            params.retreatStatus = this.arrObjs[search.checkerId] === EnumShBankTradeStatus.stop ? 0 : 4;
  ...
  handmore(item: any) {
    ...
    if (item.nowProcedureId === '@end') {
      this.xn.router.navigate([`/oct/record/todo/view/${item.recordId}`]);
    }
    ...
    if (!isOk) {
      ...
      if (item.nowProcedureId === 'operate' || item.nowProcedureId === 'review') {
        this.xn.router.navigate([`/oct/record/todo/edit/${item.recordId}`]);
      } else {
        this.xn.router.navigate([`/oct/record/todo/view/${item.recordId}`]);
      }
    }
...
export const FlowIdObj = {
  DOING: ['oct_financing_pre', 'oct_financing', 'oct_platform_verify', 'oct_factoring_risk'],
  TODO: ['oct_financing_sign', 'oct_factoring_passback', 'wait_verification_500', 'verificating_500', 'factoring_sign_500', 'wait_loan_500'],
  DONE: ['loaded_500'],
  SPECIAL: [],
  stop: ['99', '100']
};
