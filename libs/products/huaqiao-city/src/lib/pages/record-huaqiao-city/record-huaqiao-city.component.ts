...
@Component({
    selector: 'app-record-huaqiao-city',
    templateUrl: './record-huaqiao-city.component.html',
    styleUrls: ['./record-huaqiao-city.component.css']
})
export class RecordHuaQiaoCityComponent implements OnInit, AfterViewInit {

    flowId1 = 'oct_financing_pre';  // 新万科流程
    ...
    doProcess(record: any) {
        ...
        if ((record.status !== 1 && record.status !== 0)
            ...
            this.xn.router.navigate([`/oct/record/${this.flowId1}/view/${record.recordId}`]);
        } else {
            this.xn.router.navigate([`/oct/record/${this.flowId1}/edit/${record.recordId}`]);
        }
    }
    ...
    newProcess() {
        ...
        const params = {
            ...
            checker: [
                {
                    ...
                    options: { ref: 'productType_dw' } // '万科企业股份有限公司'
                },
                {
            ...
            info: { flowId: 'oct_financing_pre', headquarters: HeadquartersTypeEnum[10] }
        };
        XnModalUtils.openInViewContainer(this.xn, this.vcr, EditNewVankeModalComponent, params).subscribe(x => {
            if (!!x) {
                ...
                if (x) {
                    this.xn.router.navigate([`/oct/record/new/${this.flowId1}/${HeadquartersTypeEnum[10]}`],
                        {
                            queryParams: {
