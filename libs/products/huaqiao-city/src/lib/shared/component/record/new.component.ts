...
export class HuaQiaoCityNewComponent implements OnInit, AfterViewInit {
    ...
    private doShow() {
        ...
        if (['oct_financing_pre'].includes(this.flowId)) {
    ...
    private request(method: string) {
        ...
        params.checkers.factoringAppId = applyFactoringTtype.深圳市柏霖汇商业保理有限公司;
        // 加上loading
        ...
        this.xn.api.dragon.post(`/flow/${method}`, params).subscribe(json => {
            ...
            this.xn.router.navigate([`/oct/record/record/${this.flowId}`]);
        });
    }
