...
import { IsProxyDef, MsgStatus, RecordStatusEnum, ShPuhuiFlowIdEnum } from 'libs/shared/src/lib/config/enum/index';
 ...
 */
@Component({
  selector: 'lib-oct-sh-home-comm-list-component',
  templateUrl: './home-comm-list.component.html',
  styleUrls: [
    './home-comm-list.component.css'
  ]
})
export class OctShangHaiHomeCommListComponent extends CommonPage implements OnInit {
  @Input() supers: any;
  total = 0;
  // pageSize = 10;
  // first = 0;
  ...
  public doProcess(record: any) {
    if (!record.flowId) {
      // 查看提醒
      this.xn.router.navigate([`oct-shanghai/view-remind`]);
    } else {
      if (record.flowId === ShPuhuiFlowIdEnum.SUB_SO_PRATTWHITNEY_INPUT && record.status === RecordStatusEnum.UNSIGN) {
        // 上银普惠开户流程-供应商待开户
        this.xn.router.navigate([`/oct-shanghai/record/puhui/new`],
          {
            queryParams: {
              id: ShPuhuiFlowIdEnum.SUB_SO_PRATTWHITNEY_INPUT,
              appId: this.xn.user.appId
            }
          });
      } else if (record.flowId === ShPuhuiFlowIdEnum.SUB_SO_PRATTWHITNEY_ACCOUNT_ACTIVE) {
        // 供应商激活对公账户,跳转到普惠开户管理详情页面
        this.xn.router.navigate([`/oct-shanghai/puhui-account-manage`])
        ...
        if (record.proxyType === undefined) {
          this.xn.router.navigate([`/oct-shanghai/record/todo/view/${record.recordId}`]);
        } else {
        ...
        if (record.proxyType === undefined) {
          ...
          if (record.flowId === 'so_platform_verify' && record.zhongdengStatus === 1) {
            this.xn.msgBox.open(false, '该流程中登登记处于登记中,不可处理');
            return;
          } else if (record.flowId === ShPuhuiFlowIdEnum.SUB_SO_PRATTWHITNEY_INPUT
            || record.flowId === ShPuhuiFlowIdEnum.SUB_SO_PRATTWHITNEY_PLATFORM_VERIFY) {
            // 上银普惠开户流程代办
            this.xn.router.navigate(['/oct-shanghai/record/puhui/todo/edit/' + record.recordId]);
          } else {
...
enum EditflowSet {
  '/oct-shanghai/record/todo/edit/' = 2,
}
enum ViewflowSet {
  '/oct-shanghai/record/todo/view/' = 2,
}
