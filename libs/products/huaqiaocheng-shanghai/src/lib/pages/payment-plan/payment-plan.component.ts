...
@Component({
  selector: 'oct-payment-plan',
  templateUrl: './payment-plan.component.html',
  styleUrls: ['./payment-plan.component.css']
})
export class OctPaymentPlanComponent implements OnInit {
  tabConfig: any;
  ...
  doProcess(record: any) {
    ...
    if ((record.status !== 1 && record.status !== 0)
      ...
      this.xn.router.navigate([`/oct-shanghai/record/${record.flowId}/view/${record.recordId}`]);
    } else {
      this.xn.router.navigate([`/oct-shanghai/record/${record.flowId}/edit/${record.recordId}`]);
    }
  }
   ...
   */  
  newProcess() {
    this.xn.router.navigate([`/oct-shanghai/record/new/so_financing_pre/${HeadquartersTypeEnum[10]}`]);
  }
}
