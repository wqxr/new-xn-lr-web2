...
import { SelectOptions, EnumShBankExtStatus, EnumShBankTradeStatus, extStatusToStatus, ShInterFaceErrStatus, ShInterFaceStepStatus, VankeExtStatus, Option, HeadquartersTypeEnum } from 'libs/shared/src/lib/config/select-options';
import * as _ from 'lodash';
import { LoadingService } from 'libs/shared/src/lib/services/loading.service';
import { SelectItemsModel } from 'libs/shared/src/lib/config/checkers';
import { EditParamInputModel, ShEditModalComponent } from 'libs/products/bank-shanghai/src/lib/share/modal/edit-modal.component';
...
import { ShanghaiFinancingContractModalComponent } from 'libs/products/bank-shanghai/src/lib/share/modal/shanghai-asign-contract.modal.component';
...
import { QRSInterfaceType } from '../../logic/public-enum';
import { MfilesPdfViewModalComponent } from '../../share/modal/mfiles-pdf-modal.component';
import { ModalSize } from 'libs/shared/src/lib/common/modal/components/modal';

@Component({
  selector: 'oct-shanghai-account-list',
  templateUrl: './shanghai-account-list.component.html',
  styleUrls: ['./shanghai-account-list.component.css']
})
export class OctShanghaiAccountListComponent implements OnInit, AfterViewInit, OnDestroy {
  tabConfig: any;
        ...
        if (!XnUtils.isEmpty(this.arrObjs[search.checkerId])) {
          switch (search.checkerId) {
            ...
            case 'factoringStartDate':          // 起息日
              ...
              break;
            case 'planLoanTime':          // 起息日
              const planLoanTime = JSON.parse(this.arrObjs[search.checkerId]);
              params['planLoanTime'] = JSON.stringify({beginTime: Number(planLoanTime['beginTime']), endTime: Number(planLoanTime['endTime'])});
            break;
            case 'financingType':
  ...
  public handleHeadBatchClick(paramBtnOperate: ButtonConfigModel) {
    if (paramBtnOperate.operate === 'download_file') {
      ...
      const newSelectItems = this.selectedItems.filter((x: any) => x.isProxy === IsProxyDef.OCT_SH); // 新业务的退单流程
    ...
    } else if (paramBtnOperate.operate === 'add_info') {
      // 补充信息
      if (!this.selectedItems.length) {
        this.xn.msgBox.open(false, '请选择交易');
        return false;
        ...
        // 部分交易状态下不能操作
        const notWaitAddStepArr = this.selectedItems.filter((x: any) => !([EnumShBankTradeStatus.bank_finish].includes(Number(x.tradeStatus))
          && [EnumShBankExtStatus.input_contract_info].includes(Number(x.statusEx)))).map((y: any) => y.mainFlowId);
        if(notWaitAddStepArr.length > 0) {
          this.xn.msgBox.open(false, [
            `有${notWaitAddStepArr.length}笔交易：`,
            ...notWaitAddStepArr,
            `不是"待补充信息"状态，不能进行此操作`
          ]);
          return false;
      ...
      this.addInfo(this.selectedItems);
    ...
    } else if (paramBtnOperate.operate === 'output_qrs') { // 生成付确
      if (!this.selectedItems.length) {
        this.xn.msgBox.open(false, '请选择交易');
        return false;
      } else {
        // 部分交易状态下不能操作
        const notOutputStepArr = this.selectedItems.filter((x: any) =>
          !([EnumShBankTradeStatus.bank_finish].includes(Number(x.tradeStatus)))).map((y: any) => y.mainFlowId);
        if(notOutputStepArr.length > 0) {
          this.xn.msgBox.open(false, [
            `"上海银行复核"状态之后的业务才能进行批量生成付确，`,
            `其中有${notOutputStepArr.length}笔交易：`,
            ...notOutputStepArr,
            `不能进行此操作`
          ]);
          return false;
      ...
      console.log('output_qrs-----------', this.selectedItems);
      this.generateQrs({});
    } else if(paramBtnOperate.operate === 'financingApply') {
      // 融资申请
      if (!this.selectedItems.length) {
        this.xn.msgBox.open(false, '请选择交易');
        return false;
        ...
        ].includes(Number(x.statusEx)))).map((y: any) => y.mainFlowId);
        if(notWaitAddStepArr.length > 0) {
          this.xn.msgBox.open(false, [
            `有${notWaitAddStepArr.length}笔交易：`,
            ...notWaitAddStepArr,
      ...
      this.financingApply();
    } else if (paramBtnOperate.operate === 'setCheckPriority') { // 设置审核优先级
      if (!this.selectedItems.length) {
        this.xn.msgBox.open(false, '请选择交易');
        return false;
      }
      this.setCheckPriority(paramBtnOperate);
    } else if (paramBtnOperate.operate === 'setLoanPriority') { // 设置放款优先级
      if (!this.selectedItems.length) {
        this.xn.msgBox.open(false, '请选择交易');
        return false;
      }
      this.setLoanPriority(paramBtnOperate);
    } else if (paramBtnOperate.operate === 'setPlanLoan') { // 设置预计放款时间
      if (!this.selectedItems.length) {
        this.xn.msgBox.open(false, '请选择交易');
        return false;
      }
  ...
  downloadFile(btn: ButtonConfigModel) {
    ...
    const checkers = [{
      ...
      selectOptions: [
        { value: 15, label: '《项目公司付款确认书》' },
        { value: 16, label: '《总部公司付款确认书》' },
    ...
    XnModalUtils.openInViewContainer(this.xn, this.vcr, ShEditModalComponent, params).subscribe(data => {
      if (!!data) {
        this.xn.loading.open();
        const newMainFlowId = this.listInfo.filter(z => z.isProxy === IsProxyDef.OCT_SH);
        const newSelectItems = this.selectedItems.filter((x: any) => x.isProxy === IsProxyDef.OCT_SH);
  ...
  financingApply() {
    ...
    this.xn.dragon.post('/shanghai_bank/so_bank_gateway/repeatFinancingApply', param).subscribe((res: any) => {
      if (res && res.ret === 0){
        this.xn.msgBox.open(true, '融资申请处理中，请稍后刷新列表', () => {
          this.onPage({ page: this.paging });
        });
      }
    });
  ...
  /**
   * @description: 补充信息--目前只能单笔补充
   * @param {any} items
   * @return {*}
   */  
  public addInfo(items: any[] = []) {
    let checkers = [
      {
        title: '授信协议编号', checkerId: 'agreement_no', type: 'two-input', required: true,
        options: { readonly: false }, value: '', memo: '只能手动输入，不可复制粘贴'
      },
      {
        title: '项目编号', checkerId: 'contract_id', type: 'two-input', required: true,
        options: { readonly: false }, value: '', memo: '只能手动输入，不可复制粘贴'
      },
      {
        title: '合同编号', checkerId: 'contract_no', type: 'two-input', required: true,
        options: { readonly: false }, value: '', memo: '只能手动输入，不可复制粘贴'
      },
      {
        title: '融资利率', checkerId: 'discount_rate', type: 'two-input', required: true,
        options: { readonly: false }, value: '', memo: '只能手动输入，不可复制粘贴', validators: { number3: true }
      },
      {
        title: '付款确认书编号', checkerId: 'payConfirmId', type: 'two-input', required: true,
        options: { readonly: false }, value: '', memo: '只能手动输入，不可复制粘贴',
      },
      {
        title: '保理融资到期日', checkerId: 'factoringEndDate', type: 'sh-date', required: true, value: '', options: {}
      },
      {
        title: '项目公司付款确认书', checkerId: 'proJectPayConfirmFile', type: 'file', required: true, value: '', options: { readonly: false, fileext: "pdf", picSize: 500}
      },
      {
        title: '总部付款确认书', checkerId: 'groupPayConfirmFile', type: 'file', required: true, value: '', options: { readonly: false, fileext: "pdf", picSize: 500}
      },
    ];
    this.xn.loading.open();
    this.xn.dragon.post('/shanghai_bank/so_general/getContractInfo', { mainFlowId: items[0].mainFlowId }).subscribe((resInfo: any) => {
      if (resInfo && resInfo.ret === 0) {
        this.xn.loading.close();
        checkers.map((item: any) => {
          resInfo.data = {
            ...resInfo.data,
            factoringEndDate: XnUtils.isEmptys(resInfo.data?.factoringEndDate, [0]) ? '' : XnUtils.formatDate(resInfo.data?.factoringEndDate)
          }
        ...
        XnModalUtils.openInViewContainer(this.xn, this.vcr, ShEditModalComponent, params).subscribe((res) => {
          if (!!res) {
            const paramObj = {
              mainFlowId: items[0].mainFlowId,
              ...res,
              factoringEndDate: XnUtils.isEmptys(res?.factoringEndDate, [0]) ? '' : new Date(res?.factoringEndDate).getTime()
            };
            // 保存合同编号，调用【授信协议及合同查询接口】/调用【应收账款导入接口】
            this.xn.dragon.post('/shanghai_bank/so_general/hand_contract_info', paramObj).subscribe(json => {
              this.xn.loading.close();
              if (json && json.ret === 0) {
                this.onPage({ page: this.paging });
              }
   ...
   */
  editHelpHandler() {
    const newSelectItems = this.selectedItems.filter((x: any) => x.isProxy === IsProxyDef.OCT_SH); // 新业务
  ...
  disabledJudge(item: any, type: string): boolean {
    ...
    switch (type) {
      ...
      case 'enter_verify':
        // 平台审核-初审、复核
        status = [99].includes(this.xn.user.orgType) && item.wkType.toString() === '0';
        break;
      default:
        status = false;
        break;
  ...
  enterVerifyPage(item: any) {
    if (['@end'].includes(item.nowProcedureId)) {
      this.xn.router.navigate([`/oct-shanghai/record/todo/view/${item.recordId}`]);
      return false;
    }
    ...
    if (!!isOk) {
      if (['operate', 'review'].includes(item.nowProcedureId)) {
        if (item.flowId === 'so_platform_verify' && item.zhongdengStatus === 1) {
          this.xn.msgBox.open(false, '该流程中登登记处于登记中,不可处理');
          return false;
        } else {
          this.xn.router.navigate([`/oct-shanghai/record/todo/edit/${item.recordId}`]);
        }
      } else {
        this.xn.router.navigate([`/oct-shanghai/record/todo/view/${item.recordId}`]);
      }
  ...
  toCallInterfaceAgain(item: any) {
    ...
    XnModalUtils.openInViewContainer(this.xn, this.vcr, ShSingleSearchListModalComponent, params).subscribe(v => {
      if (v && v.action === 'ok') {
        ...
        // 下推下一个状态
        this.xn.dragon.post('/shanghai_bank/so_bank_gateway/goToSignFinancingContractStatus', { mainFlowId: item.mainFlowId }).subscribe(json => {
          if (json && json.ret === 0) {
   ...
   * @return {*}
   */  
  public generateQrs(model: {downloadRange?: string, factoringEndDate?: string, payConfirmIds?: any[]}) {
    let _checkers = this.selectedItems.map((y: any, index: number) => {
      return {
        title: y.mainFlowId, checkerId: y.mainFlowId, type: 'special-text-array', required: true, value: '', options: {},
        childChecker: [
          {
            title: '付确编号', checkerId: 'payConfirmId', type: 'text', required: true,
            value: model?.payConfirmIds ? model?.payConfirmIds[index] : '', options: {others: {
              itemSpan: 'col-sm-4',
              labelSpan: 'col-sm-4',
              controlSpan: 'col-sm-8',
            }},
          },
          // {
          //   title: '交易ID', checkerId: 'mainFlowId', type: 'special-text', required: false,
          //   value: y.mainFlowId || '', options: {others: {
          //     itemSpan: 'col-sm-4',
          //     labelSpan: 'col-sm-3',
          //     controlSpan: 'col-sm-9',
          //   }},
          // },
          {
            title: '供应商名称', checkerId: 'debtUnit', type: 'text', required: false,
            value: y.debtUnit || '', options: {readonly: true, others: {
              itemSpan: 'col-sm-4',
              labelSpan: 'col-sm-4',
              controlSpan: 'col-sm-8',
            }},
          ...
          {
            title: '应收账款金额', checkerId: 'receive', type: 'input-number', required: false,
            value: !XnUtils.isEmptys(y.receive, [0]) ? y.receive : '', options: { readonly: true, currency:true, others: {
              itemSpan: 'col-sm-4',
              labelSpan: 'col-sm-5',
              controlSpan: 'col-sm-7',
            }},
          },
        ]
      }
    ...
    const checkers = [
      { checkerId: 'downloadRange', name: 'downloadRange', required: true, type: 'radio', title: '付确类型',
        value: !XnUtils.isEmptys(model?.downloadRange, [0]) ? model?.downloadRange : null,
        selectOptions: [
          { value: 'HeadquartersQrs', label: '总部付确' },
          { value: 'ProjectcompanyQrs', label: '项目公司付确' },
        ]
      },
      { title: '保理融资到期日', checkerId: 'factoringEndDate', type: 'sh-date', required: true,
        value: !XnUtils.isEmptys(model?.factoringEndDate, [0]) ? model?.factoringEndDate : null},
      ..._checkers
    ];
    const params: EditParamInputModel = {
    ...
    XnModalUtils.openInViewContainer(this.xn, this.vcr, ShEditModalComponent, params).subscribe(z => {
      // console.log('生成付确--------', z);
      if (!!z) {
        this.xn.loading.open();
        this.priviewQrsFiels(z);
      }
    });
   ...
   * @param params
   */
  private priviewQrsFiels(params: any) {
    let {downloadRange, factoringEndDate, ...payConfirmIdObj} = params || {};
    this.xn.dragon.post(`${QRSInterfaceType[params.downloadRange]}`, {
      factoringEndDate: new Date(params.factoringEndDate || '').getTime(),
      mFlowIds: Object.keys(payConfirmIdObj || {}).map((y: string) => {
        return {
          mainFlowId: y,
          payConfirmId: payConfirmIdObj[y]?.payConfirmId
        }
      }) || []
    }).pipe(map((qrsRes: any) => {
      this.xn.loading.close();
      if (qrsRes && qrsRes.ret === 0) {
        let filesList = qrsRes.data || [];
        if (filesList.length) {
          XnModalUtils.openInViewContainer(this.xn, this.vcr, MfilesPdfViewModalComponent, {filesList, pageTitle: '生成付确'}).subscribe(x => {
            this.xn.loading.open();
            if (x.action === 'ok') {
              this.xn.loading.close();
            } else {
              ...
              this.generateQrs({
                downloadRange,
                factoringEndDate,
                payConfirmIds: Object.keys(payConfirmIdObj || {}).map((y: string) => payConfirmIdObj[y]?.payConfirmId)
              });
            }
          });
        }
