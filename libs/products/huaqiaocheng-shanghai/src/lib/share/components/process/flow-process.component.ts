...
import {
  FlowProcessNodeOutputModel, FlowProcessOutputModel, LocalFlowProcessModel
} from './flow-process.model';
import Process from './mock-process';
@Component({
  selector: 'xn-oct-sh-flow-process-component',
  template: `
...
export class OctFlowProcessComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    if (this.mainfromlist && this.mainfromlist.length > 0 && (!!param.mainFlowId || param.mainFlowId === '1')) {
      this.xn.api.dragon.post('/flow/navigation', {
        modelId: this.mainfromlist[0].modelId, // 协议流程 对应cfg目录的model.json vanke万科流程
        ntype: 1,
      }).subscribe(res => {
        const nodeList = Process.getCustom({ orgType: this.xn.user.orgType, nodeList: res.data.nodeList });
        this.flowProessmain = {
          ...res.data,
          nodeList
        };
        if (this.flowProessmain.modelId === 'dragon') {
          ...
          this.localFlowProcess = this.buildRows(nodeList);
        }
      });
    } else {
      this.xn.api.dragon.post('/flow/navigation', {
