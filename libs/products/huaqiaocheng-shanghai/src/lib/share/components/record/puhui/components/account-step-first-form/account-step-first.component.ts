...
@Component({
  selector: 'oct-account-step-first',
...
export class OctAccountStepFirstComponent implements OnInit, AfterViewChecked {
   ...
   */
  validshrhldPercent() {
    const checkers: CheckersOutputModel[] = this.svrConfig.checkers;
    // 拿到股东信息数组
    const shrhldListvalue = JSON.parse(checkers.filter(c => c.checkerId === 'shrhldList')[0].value);
    // 计算持股比例之和
    let percentTotal = 0;
    shrhldListvalue.forEach(shrld => {
      if (shrld['shrhldPercent']) {
        const shrhldPercent = Number(shrld['shrhldPercent']);
        percentTotal = _.add(percentTotal, shrhldPercent)
      }
    });
    percentTotal = Number(percentTotal.toFixed(2));
    if (percentTotal > 100) {
      this.xn.msgBox.open(false, `股东持股比例之和 ${percentTotal} 超过100,请修改后重新提交`);
      return false;
    } else {
      return true;
    }
