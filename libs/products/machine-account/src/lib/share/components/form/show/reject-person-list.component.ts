  ...
  template: `
    ...
    <tbody>
      <tr *ngFor="let item of items;let i=index">
        ...
        <td>
          <ng-container *ngIf="item['mainFlowId'].endsWith('lg') || item['mainFlowId'].endsWith('wk')||item['mainFlowId'].endsWith('oct')">
                {{ item?.flowId | xnMainFlowStatus : 5253 }}
            </ng-container>
            <ng-container *ngIf="item['mainFlowId'].endsWith('bgy')">
                {{ item?.flowId | xnSelectTransform:'countryGradenTradeStatus' }}
            </ng-container>
            <ng-container *ngIf="item['mainFlowId'].endsWith('jd')">
                {{ item?.flowId | xnSelectTransform:'newGemdaleTradeStatus' }}
            </ng-container>
            <ng-container *ngIf="item['mainFlowId'].endsWith('hz')">
                {{ item?.flowId | xnSelectTransform:'agileXingshunTradeStatus' }}
            </ng-container>
            <ng-container *ngIf="item['mainFlowId'].endsWith('yjl')">
                {{ item?.flowId | xnSelectTransform:'agileXsTradeStatus' }}
            </ng-container>
        </td>
        <td *ngIf="false" style='word-break:break-all'>{{ item?.stopReason }}</td>
