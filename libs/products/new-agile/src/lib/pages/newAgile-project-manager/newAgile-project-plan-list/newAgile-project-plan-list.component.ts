...
import ProjectManagerplanList from './newAgile-project-plan';

declare const moment: any;

@Component({
    selector: 'newAgile-project-plan-list',
    templateUrl: `./newAgile-project-plan-list.component.html`,
    styles: [`
    .item-box {
        position: relative;
        display: flex;
...
export class NewAgileProjectmanagePlanComponent implements OnInit {
    tabConfig: any;
    ...
    ngOnInit(): void {
        this.router.queryParams.subscribe(x => {
            this.title = '项目管理-星顺-雅居乐>' + x.title + '>' + x.projectName + '-' + x.headquarters;
    ...
    getPlan(paramInfo: any) {
        ...
        if (paramInfo.poolFlag === 1) {
            this.xn.router.navigate(['/new-agile/assets-management/capital-pool'], {
                queryParams: {
                    title: this.title,
    ...
    public capitalItemForm(params?: { id: string; name: string }) {
        ...
        XnModalUtils.openInViewContainer(
            ...
            data.headquarters = HeadquartersTypeEnum[4]
            if (!!data) {
    ...
    */
    ...
    addTransaction(item: any, btn: ButtonConfigModel) {
        /** poolFlag  1=新项目 0=老项目 */
        if (item.poolFlag === 1) {
            this.xn.router.navigate([`/new-agile/enter-pool`], {
                queryParams: {
  ...
  */
     ...
     */
    navBack() {
        this.xn.router.navigate(['/new-agile/assets-management/project-list'], {
            queryParams: {
