...
/**
 * base: src\app\pages\console\manage\supplier-unsigned-contract.component.ts
 */
@Component({
        ...
        this.xn.api.post('/custom/vanke_v5/project/supplier_sign_contracts',
            { mainFlowId: item.mainFlowId }).subscribe(json => {
                const contracts = json.data.contractList.map(c => {
                    ...
                    if (c.label === '应收账款转让合同(债权人<-->保理商)（通用版）雅居乐') {
                        c.config.text = '卖方（盖章）';
                    } else if (c.label === '应收账款转让登记协议(债权人<-->保理商)（通用版）雅居乐') {
                        c.config.text = '甲方公章/合同专用章';
                    } else if (c.label === '应收账款转让通知书(债权人-->付款人)（通用版）雅居乐') {
                        c.config.text = '（盖章）';
                    } else if (c.label === '应收账款转让通知书(债权人-->债务人)（通用版）雅居乐') {
                        c.config.text = '（盖章）';
