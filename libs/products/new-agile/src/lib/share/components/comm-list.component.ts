...
/**
 *  公用列表
 *  base: src\app\public\component\comm-list.component.ts
 */
@Component({
    selector: 'xn-new-agile-comm-list-component',
...
export class NewAgileCommListComponent extends CommonPage implements OnInit {
    ...
    private supplierAppIdSet() {
        this.xn.api
            .post('/custom/vanke_v5/app/get_app', {
                orgName: this.xn.user.orgName,
            })
            .subscribe((x) => {
    ...
    handleClick(row) {
        this.xn.router.navigate([`/new-agile/invoice-display/invoice-list`]);
    ...
    doProcess(record: any) {
        this.xn.router.navigate([
            `/new-agile/record/todo/view/${record.recordId}`,
        ]);
    ...
    private buildParams() {
        ...
        if (this.searches.length > 0 && !$.isEmptyObject(this.arrObjs)) {
            const conditions = this.searches.filter(x => !XnUtils.isEmpty(this.arrObjs[x.checkerId]));
            if (conditions.length) {
                params.where = {
                    _complex: {},
                };

                conditions.forEach(search => {
                    params.where._complex[search.checkerId] = ['select', 'listing'].includes(search.type)
    ...
    private selectFiles(paramAction, paramFileArr, params): Array<any> {
        ...
        params.logs.map((v) => {
            if (
                v &&
                v.flowId &&
                (v.flowId !== 'factoring_loan' ||
                    v.flowId !== 'factoring_repayment')
            ) {
        ...
        if (this.xn.user.orgType === 1) {
            ...
            if (params.logs.length) {
                logsData = params.logs.filter(
                    (x) =>
                        !!x.appId && x.appId === this.supplierOperateAppId.appId
                );
            }
