...
@Component({
    selector: 'xn-new-agile-flow-detail-chart',
...
export class FlowDetailChartComponent implements OnInit {
    ...
    private sliceShow(start, end) {
        const obj = $.extend(true, {}, this.nowAssetOriginal);
        ...
        setTimeout(() => {
            ...
            // 调整当前资产饼图
            const asset = this.convertPageAsset(this.nowAsset);
            const option = this.optionTerm(asset.data, asset.title, asset.subTitle);
            const num = this.assetIndex - this.pageIndex * this.pageSize;  // 当前是这页的第几个？
            const elements = this.pageCharts.toArray();
            const element: any = elements[num];
            element.chart.setOption(option);  // 这里认为chart一定存在
        }, 0);
    }

    private showPageAssets() {
        const assets = this.pageAssets.map(asset => this.convertPageAsset(asset));
    ...
    writeDown(chart) {
        ...
        const option = this.optionTerm2(chart.data, chart.title);
