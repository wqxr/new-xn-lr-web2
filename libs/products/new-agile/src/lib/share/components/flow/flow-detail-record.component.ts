...
@Component({
    selector: 'xn-new-agile-flow-detail-record',
...
export class FlowDetailRecordComponent implements OnInit {
    ...
    ngOnInit() {
        // 当前步骤为 当前状态下一
        this.steped = this.currentStep(parseInt(this.params.status, 10));
        ...
        const resRoles = this.xn.user.roles.filter(function(item) {
    ...
    public currentStep(paramsStatus: number) {
        const step = FlowCustom.getCurrentFlowProcessStep(paramsStatus);
...
*/
    ...
    public isFactoringViewDetail(paramsRow) {
        return [EnterpriseMenuEnum.FACTORING].includes(this.xn.user.orgType)
         && ['financing_supplier18'].includes(paramsRow.flowId);
     }
    ...
    public checkSubprocess(paramRow: any) {
        this.xn.router.navigate([`new-agile/record/view/${paramRow.recordId}`],
