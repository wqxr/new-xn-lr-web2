 ...
 */
@Component({
  templateUrl: './invoice-transfer-input.component.html',
  styles: [
    `
      .plat-table-footer {
        font-size: 14px;
...
@DynamicForm({ type: 'invoice-transfer', formModule: 'new-agile-input' })
export class InvoiceTransferInputComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    this.ctrl1 = this.form.get('payables');
  ...
  public onEdit(item: any, i?: number, bool?: boolean) {
    ...
    item.mainFlowId = this.svrConfig.mainFlowId;
      ...
      if (bcontinue) {
        ...
        if (this.calculateData(this.ctrl1.value) > this.calculateData(item[i].invoiceAmount) &&
          ...
          if (totalinvoiceAmount > this.calculateData(this.ctrl1.value)) {
            item[i].transferMoney = this.calculateData(this.calculateData(this.ctrl1.value) - previnvoiceAmount);
            bcontinue = true;
          } else {
          ...
          if (this.calculateData(this.ctrl1.value) < this.calculateData(item[i].invoiceAmount) && i === 0) {
            ...
            item[i].transferMoney = this.calculateData(this.calculateData(this.ctrl1.value) - totalinvoiceAmount);
            bcontinue = true;
  ...
  changeTransferMoney(item) {
    ...
    XnModalUtils.openInViewContainer(this.xn, this.vcr, EditInfoModalComponent, params).subscribe(x => {
      const rep = /^(0|([1-9]\d*))(\.\d+)?$/g;
      if (x === null || !x || !(x.transferMoney)) {
        return;
      } else if (!rep.test(x.transferMoney.replace(/[,]/g, ''))) {
        this.xn.msgBox.open(false, '转让金额格式错误，请重新输入');
        return;
      } else if (x.transferMoney.replace(/[,]/g, '') < 0) {
        this.xn.msgBox.open(false, '转让金额不可以为负数');
        return;
      } else if (x.transferMoney.replace(/[,]/g, '') > parseFloat(item.invoiceAmount)) {
        this.xn.msgBox.open(false, '转让金额不可以大于税收金额');
        return;

      } else {
        item.transferMoney = x.transferMoney.replace(/[,]/g, '');
        this.toValue();
      }
