 ...
 */
@Component({
    selector: 'xn-new-agile-home-comm-list-component',
    templateUrl: './home-comm-list.component.html',
    styleUrls: [
        './home-comm-list.component.css'
    ]
})
export class NewAgileHomeCommListComponent extends CommonPage implements OnInit {
    @Input() supers: any;
    total = 0;
    // pageSize = 10;
    // first = 0;
    ...
    public viewProcess(paramMainFlowId: string) {
        this.xn.router.navigate([`new-agile/main-list/detail/${paramMainFlowId}`]);
    }
    ...
    public doProcess(record: any) {
        ...
        if ((record.status !== 1 && record.status !== 0)
            ...
            if (record.proxyType === undefined) {
                this.xn.router.navigate([`/new-agile/record/todo/view/${record.recordId}`]);
            } else {
                this.xn.router.navigate([`/new-agile/record/todo/edit/${record.recordId}`]);
            }
        } else {

            if (record.flowId === 'yajule_add_contract') {
                // todo 特殊处理，雅居乐供应商补充协议 直接跳转签署合同页面
                this.xn.router.navigate([`/new-agile/record/supplier_sign/${record.mainFlowId}`]);
            } else {
                if (record.proxyType === undefined) {
                    this.xn.router.navigate([`/new-agile/record/todo/view/${record.recordId}`]);
                } else {
