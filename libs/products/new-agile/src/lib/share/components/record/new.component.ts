...
export class NewComponent implements OnInit, AfterViewInit {
    ...
    ngAfterViewInit() {
        ...
        this.route.queryParams.subscribe((params) => {
            ...
            this.Relate = {
                ...
                relateValue:
                    params.relateValue ||
                    JSON.parse(sessionStorage.getItem('relate_value')),
            };
    ...
    public authorizationFile() {
        ...
        // this.xn.api
        //     .download('/custom/vanke_v5/project/download_qrh', {})
        //     .subscribe((v: any) => {
    ...
    private doShow() {
        ...
        this.xn.api
            .post('/record/record?method=show_new', postObj)
            .subscribe((json) => {
    ...
    private postShow() {
        ...
        if (!XnUtils.isEmpty(this.svrConfig.flow.relatedFlowId)) {
            new DataTablePicker(this.xn).open(
                '请选择要处理的相关流程记录',
                'list-related',
                ...
                for (const checkerId in json.data) {
                    if (json.data.hasOwnProperty(checkerId)) {
                        ...
                        if (!isNullOrUndefined(ctrl)) {
                            ...
                            this.svrConfig.constParams.checkers[checkerId] =
                                json.data[checkerId];
                        }
                    }
                }
    ...
    private request(method: string) {
        ...
        const params: any = {
            modelId: BusinessMode.Yjl, // 默认为 雅居乐-星顺模式
            flowId: this.flowId,
            ...
            recordId:
                (this.svrConfig.record && this.svrConfig.record.recordId) || '',
        ...
        if (this.svrConfig.constParams) {
            params.relatedRecordId =
                this.svrConfig.constParams.relatedRecordId || '';
            if (this.svrConfig.constParams.checkers) {
                for (const key in this.svrConfig.constParams.checkers) {
                    if (
                        this.svrConfig.constParams.checkers.hasOwnProperty(key)
                    ) {
                        params.checkers[
                            key
                        ] = this.svrConfig.constParams.checkers[key];
                    }
                }
            }
        }
        ...
        this.xn.api
            .post(`/record/record?method=${method}`, params)
            .subscribe((json) => {
                this.presubmitSpecial(params, json);
                this.xn.router.navigate([
                    `/console/record/record/${this.flowId}`,
                ]);
            });
    }
