 ...
 * All rights reserved.
 *
 * @file：NewGemdaleAvengerListComponent
 * @summary：金地审批放款列表
 * @version: 1.0
 * **********************************************************************
...
export class NewGemdaleAvengerListComponent implements OnInit {
  ...
  */
  public initData(paramTabValue: string, init?: boolean,) {
    ...
    if (this.defaultValue === paramTabValue && !init) {
      ...
      if (this.tabConfig.title.includes('审批放款-金地') && (this.defaultValue === 'A' || this.defaultValue === 'C')) {
        this.xn.api.post('/tool/is_jindie', {}).subscribe(x => {
          if (x.data) {
            this.is_jindie = x.data.isOpen;
            if (this.is_jindie === 1) {
              this.tabConfig.title = '审批放款-金地(已开启金蝶云审批)';
            } else {
              this.tabConfig.title = '审批放款-金地(线下人工审批)';
            }
          }
        });
      }
      this.subDefaultValue = 'DOING'; // 重置子标签默认
      if (this.defaultValue === 'C' && this.tabConfig.title.includes('审批放款-金地') && !!!init) {
        this.pushApproval(1);
      } else if (this.defaultValue === 'A' && this.tabConfig.title.includes('审批放款-金地')) {
  ...
  public onPage(e?: { page: number, first?: number, pageSize?: number, pageCount?: number }, types?: number) {
    if (this.tabConfig.title.includes('审批放款-金地') && this.defaultValue === 'A') {
 ...
 */
  ...
  // 打开当前信贷信息窗口
  openApproval(Paramitem: any, value: string) {
