 ...
 * All rights reserved.
 *
 * @file：NewGemdaleCapitalpoolComponent
 * @summary：金地-项目管理-基础资产列表
 * @version: 1.0
 * **********************************************************************
...
import { NewGemdaleSingleSearchListModalComponent, SingleListParamInputModel } from '../../../../share/modal/single-searchList-modal.component';
import ProjectManagerCapitalList from './new-gemdale-capital-pool';
declare const moment: any;
declare const $: any;

@Component({
  // selector: 'new-gemdale-capital-management',
  templateUrl: `./new-gemdale-capital-pool.component.html`,
  styleUrls: ['./new-gemdale-capital-pool.component.css']
})
export class NewGemdaleCapitalpoolComponent implements OnInit, AfterViewInit, OnDestroy {
  tabConfig: any;  // 表格配置
  ...
  private defaultHeadquarter = HeadquartersTypeEnum[9];   // 默认总部公司
  ...
  public downloadSelectedAttach() {
    ...
    XnModalUtils.openInViewContainer(this.xn, this.vcr, CapitalPoolDownloadAttachmentsModalComponent, params).subscribe(x => {
      if (!!x) {
        ...
        const rObj = {};
        x.contentType.split(',').forEach(i => rObj[i] = true);
  ...
  private buildCondition(searches) {
    ...
    for (const search of searches) {
      ...
      if (search.checkerId === 'type' || search.checkerId === 'contractType') {
        search.options.ref = 'vankeContracttype';
      }
      obj.options = search.options;
      obj.show = search.show;
  ...
  public checkAll() {
    ...
    this.selectSumReceive = this.selectedItems.reduce((accumulator, currentValue) =>
      accumulator + Number(currentValue.receive ? currentValue.receive : 0)
      , 0).toFixed(2);
    this.selectSumChangePrice = this.selectedItems.reduce((accumulator, currentValue) =>
      accumulator + Number(currentValue.changePrice ? currentValue.changePrice : 0)
      , 0).toFixed(2);
  }
  ...
  signContacts() {
    this.xn.dragon.post(
      '/contract/second_contract_info/create_sign_second_contract',
      { project_manage_id: this.projectId }
    ).subscribe(x => {
      if (x.ret === 0) {
  ...
  public generateContacts() {
    this.xn.dragon.post(
      '/contract/second_contract_info/create_second_contract',
      { project_manage_id: this.projectId }
    ).subscribe(x => {
      if (x.ret === 0) {
        const fileList = x.data;
  ...
  private doGenerateOrSign(params: any, urls: any) {
    this.xn.api.dragon.post(urls.generate, params).subscribe(con => {
      ...
      if (result.length) {
        ...
        result.forEach(x => {
          // 金地
          if (x.label === '应收账款转让通知书（保理商出给项目公司）') {
            x.config.text = '（盖章）';
          } else if (x.label === '应收账款债权转让通知书（保理商向碧桂园地产出具）') {
            x.config.text = '（盖章）';
          } else {
            x.config.text = '（盖章）';
          }
  ...
  /**
   * 查看交易详情
   * @param mainFlowId mainFlowId
   */
  viewProgress(mainFlowId) {
    this.xn.router.navigate([`new-gemdale/main-list/detail/${mainFlowId}`]);
  }
      ...
      if (!!x) {
        ...
        const param = {
          mainFlowIdList: (+x.scope === SelectRange.All || +x.scope === SelectRange.Sample)
            ? undefined
            : this.selectedItems.map(i => i.mainFlowId),
          capitalPoolId: this.capitalPoolId,
          isSample: null
        };
        if (String(x.scope) === '3') {
        ...
        if (Number(x.contentType) === SelectContent.Default) {
          this.xn.api.dragon.download('/sub_system/jd_system/jd_pool_excel_download', param).subscribe((v: any) => {
            this.xn.loading.close();
  ...
  public handleRowClick(item: any, btn: ButtonConfigModel) {
    if (btn.operate === 'sub_start_survey') {
      ...
      if (item.flowId === 'jd_financing') {
        this.xn.msgBox.open(false, `供应商尚未上传资料，暂时无法尽调，请稍候再试`);
        return '';
      }
      // 经办人（管理人或律所角色）
      const rolesArr = this.xn.user.roles.filter(x => x === 'operator');
   ...
   */
  addTransaction() {
    this.xn.router.navigate([`/new-gemdale/assets-management/enter-pool`], {
      queryParams: {
        ...
        if (x.data.isCapitalPoolSample) {
          ...
          XnModalUtils.openInViewContainer(this.xn, this.vcr, EditModalComponent, params).subscribe(v => {
            if (v === null) {
              ...
              // 抽样页面
              this.xn.router.navigate(['/new-gemdale/assets-management/capital-sample'], {
                queryParams: {
  ...
  // 数据分析
  dataAnalyse() {
    this.xn.router.navigate(['/new-gemdale/assets-management/capital-data-analyse'], {
      queryParams: {
        totalCount: this.pageConfig.total,
        sumReceive: this.sumReceive,
        ...this.queryParams
