...
import ProjectManagerplanList from './new-gemdale-project-plan';

declare const moment: any;

@Component({
    selector: 'new-gemdale-project-plan-list',
    templateUrl: `./new-gemdale-project-plan-list.component.html`,
    styles: [`
    .item-box {
        position: relative;
        display: flex;
...
export class NewGemdaleProjectmanagePlanComponent implements OnInit {
    tabConfig: any;
    ...
    ngOnInit(): void {
        this.router.queryParams.subscribe(x => {
            this.title = '项目管理-金地>' + x.title + '>' + x.projectName + '-' + x.headquarters;
    ...
    getPlan(paramInfo: any) {
        ...
        if (paramInfo.poolFlag === 1) {
            ...
            this.xn.router.navigate(['/new-gemdale/asset-pool/trading-list'], {
                queryParams: {
        ...
        XnModalUtils.openInViewContainer(this.xn, this.vcr, EditModalComponent, params)
            .subscribe((v) => {
                if (v === null) {
                    ...
                    v.secondDiscountRate = Number(v.secondDiscountRate);
  ...
  */
     ...
     */
    navBack() {
        this.xn.router.navigate(['/new-gemdale/assets-management/project-list'], {
            queryParams: {
