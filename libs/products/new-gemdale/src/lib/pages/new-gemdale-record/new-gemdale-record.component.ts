...
@Component({
    templateUrl: './new-gemdale-record.component.html',
    styleUrls: ['./new-gemdale-record.component.css']
})
export class NewGemdaleRecordComponent implements OnInit, AfterViewInit {
    ...
    doProcess(record: any) {
        ...
        if ((record.status !== 1 && record.status !== 0)
            ...
            this.xn.router.navigate([`/new-gemdale/record/${this.flowId1}/view/${record.recordId}`]);
        } else {
            this.xn.router.navigate([`/new-gemdale/record/${this.flowId1}/edit/${record.recordId}`]);
        }
    }
    ...
    viewZdList(item) {
        ...
        XnModalUtils.openInViewContainer(this.xn, this.vcr, NewGemdaleSingleSearchListModalComponent, params).subscribe(() => {
