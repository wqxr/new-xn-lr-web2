 ...
 * 1.0                hucongying        金地数据对接      2020-12-02
 * **********************************************************************
 */
...
import { ProxyType,OrgTypeEnum } from 'libs/shared/src/lib/config/enum/common-enum';
declare const $: any;
...
@Component({
  ...
  selector: 'new-gemdale-home-comm-list-component',
  templateUrl: './home-comm-list.component.html',
  styleUrls: [
    './home-comm-list.component.css'
  ]
})
export class NewGemdaleHomeCommListComponent extends CommonPage implements OnInit {
  @Input() supers: any;
  total = 0;
  rows: any[] = [];
  words = '';
  ...
  //对项目公司展示
  showProject(){
    return this.xn.user.orgType===OrgTypeEnum.PROJECT_COMPANY
  }
  ...
  public viewProcess(paramMainFlowId: string) {
    this.xn.router.navigate([`new-gemdale/main-list/detail/${paramMainFlowId}`]);
  }
  ...
  public doProcess(record: any) {
    record.proxyType = record.proxyType === ProxyType.AVENGER ? ProxyType.AVENGER : ProxyType.NEW_GEMDALE;
