...
export class GemdaleCapitalPoolCommListComponent extends CommonPage implements OnInit {
    total = 0;
    pageSize = 10;
    first = 0;
    rows: any[] = [];
    ...
    public handleAdd(val: any) {
        this.xn.router.navigate(['/new-gemdale/unhandled-list'], {
            queryParams: {
                capitalId: val.id,
    ...
    public generateAndSign() {
        ...
        const urls = {
            ...
            capital13: '/llz/capital_list/capital13', // 金地国寿应收账款转让通知书
            update_capital13: '/llz/capital_list/update_capital13',
            exp_capital13: { noStamp: false },
        };
        XnModalUtils.openInViewContainer(
            ...
            if (x !== '') {
                ...
                if (param === HeadquartersTypeEnum[2]) {
                    if (!!x.gemdaleAndSing) {
                        const url = {
                            generate: urls[x.gemdaleAndSing],
                            update: urls[`update_${x.gemdaleAndSing}`],
                            exp: urls[`exp_${x.gemdaleAndSing}`]
                        };
                        this.doGenerateOrSign(url, params, param);
                    } else {
                        const url = {
    ...
    public generate() {
        ...
        const urls = {
            ...
            // 金地新增的线下签署合同
            second_jd_01: '/llz/capital_list/capital05',
            exp_second_jd_01: { noStamp: true },
            second_jd_02: '/llz/capital_list/capital06',
            exp_second_jd_02: { noStamp: true },
            second_jd_03: '/llz/capital_list/capital07',
            exp_second_jd_03: { noStamp: true },
            second_jd_04: '/llz/capital_list/capital08',
            exp_second_jd_04: { noStamp: true },
            second_jd_05: '/llz/capital_list/capital09',
            exp_second_jd_05: { noStamp: true },
            second_jd_06: '/llz/capital_list/capital10',
            exp_second_jd_06: { noStamp: true },
            second_jd_07: '/llz/capital_list/capital11',
            exp_second_jd_07: { noStamp: true },
            second_jd_08: '/llz/capital_list/capital12',
            exp_second_jd_08: { noStamp: true },
        };
        XnModalUtils.openInViewContainer(
            this.xn,
            this.vcr,
            ...
            if (x !== '') {
                ...
                if (param === HeadquartersTypeEnum[2]) {
                    ...
                    params.templateFlag = x.generatingContract;
                    url.update = '';
                } else {
                    params = {
                        list: selectedRows.map(r => {
    ...
    /**
       ...
       * @param params
       */
    private doGenerateOrSign(url, params, param) {
        this.xn.api.post(url.generate, params)
            .pipe(
                map(con => {
                    ...
                    let result = JSON.parse(JSON.stringify(contractList));
                    result.isProxy = 14;
                    const contracts = result;
                    if (result.length) {
                        result.forEach(tracts => {
                            if (tracts.label.includes('应收账款债权转让通知书') && param !== HeadquartersTypeEnum[2]) {
