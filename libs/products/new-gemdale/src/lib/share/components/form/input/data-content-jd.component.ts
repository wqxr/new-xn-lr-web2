...
export class DataContentJdComponent implements OnInit, OnChanges {
    ...
    ngOnInit() {
        ...
        // 监听保理商值 获取市场部对接人信息
        this.ctrlHeadquarters = this.form.get('factorName');
        this.ctrlHeadquarters.valueChanges.subscribe(v => {
            if (v && this.items.length > 0) {
                this.xn.dragon.post('/sub_system/jd_system/jd_get_marke_info',
                    {
                        factoringOrgName: v, // 保理商名字
                        list: this.items     // 提单数据列表
                    }).subscribe(z => {
                        if (z.ret === 0) {
                            this.items = [...z.data];
                            this.toValue();
            ...
            if (json.type === 'complete') {
                if (json.data.ret === 0) {
                    if (json.data.data.length > 0) {
                        ...
                        // 如果已经选了保理商 就更新市场部对接人信息
                        if (this.ctrlHeadquarters.value) {
                            this.xn.dragon.post('/sub_system/jd_system/jd_get_marke_info',
                                {
                                    factoringOrgName: this.ctrlHeadquarters.value, // 保理商名字
                                    list: this.items     // 提单数据列表
                                }).subscribe(z => {
                                    if (z.ret === 0) {
                                        this.items = [...z.data];
