...
export class NewGemdaleEditComponent
  implements AfterViewInit, AfterViewChecked, OnInit, OnDestroy
{
  ...
  private showSubmit(recordId, fresh?) {
    ...
    this.xn.dragon
      ...
      .subscribe((json) => {
        ...
        this.showIconJdChanges =
          this.showJdChangesIcon() && this.showIconJdChanges;
  ...
  showRejectIcon() {
    return (
      (this.svrConfig.record.flowId === 'jd_financing' ||
        this.svrConfig.record.flowId === 'jd_platform_verify') &&
  ...
  public onSubmit() {
    this.innerSubmit(() => {
      for (const checker of this.svrConfig.checkers) {
        ...
        // 金地-发起提单 渠道处理
        if (
          checker.checkerId === 'productType' &&
          this.flowId === 'jd_financing_pre'
        ) {
          const { proxy, status } =
            JSON.parse(this.mainForm.value[checker.checkerId]) || {};
          const { type, selectBank } =
            JSON.parse(this.mainForm.value[checker.checkerId]) || {};
          const productType = {
            type: type ? type : proxy,
            selectBank: selectBank ? selectBank : status,
          };
  ...
  showTempClick() {
    return (
      (this.svrConfig.flow.flowId === 'jd_platform_verify' &&
  ...
  private copyDraft(shows: any[], hides: any[], fresh: boolean): void {
    ...
    if (
      ['jd_platform_verify'].includes(this.svrConfig.flow.flowId) &&
  ...
  private showJdChangesInfo(init: boolean) {
    this.xn.dragon
      .post2('/sub_system/jd_system/get_jd_change_status', {
        mainFlowId: this.svrConfig.record.mainFlowId,
      })
      .subscribe((x) => {
        if (x.ret !== 0) {
