...
@Component({
    selector: 'new-gemdale-business-related',
    templateUrl: './new-gemdale-business-related.component.html',
    styles: [
        `
            .table {
                font-size: 13px;
...
export class NewGemdaleBusinessComponent implements OnInit {
    ...
    viewRecord(record: string, type: string) {
        if (type && type === 'avenger') {
            ...
            this.xn.router.navigate([`/new-gemdale/record/todo/view/${record}`]);
        }
    }
