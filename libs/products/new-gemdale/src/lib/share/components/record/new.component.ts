...
export class NewComponent implements OnInit, AfterViewInit {
  ...
  private doShow() {
    ...
    if (this.flowId === 'jd_financing_pre') { // 发起提单
      this.billNumberList.length > 0 ? postObj.billNumberList = this.billNumberList : delete postObj.billNumberList;
      postObj.factorName = '深圳市柏霖汇商业保理有限公司';
    }
    if (this.flowId === 'sub_jd_check_fail') { // 发起审核失败流程
      postObj.idList = this.Relate.relateValue;
    }
    if (this.flowId === 'sub_project_confirmation') { // 项目公司确认应收账款金额流程
      postObj.mainIds = this.Relate.relateValue;
    }
    ...
    const typeName = '深圳市柏霖汇商业保理有限公司';
    postObj.factoringAppId = `${applyFactoringTtype[typeName]}`;
  ...
  private request() {
    ...
    const typeName = '深圳市柏霖汇商业保理有限公司';
    params.factoringAppId = `${applyFactoringTtype[typeName]}`;
    ...
    this.xn.api.dragon.post(`/flow/new`, params).subscribe(json => {
      this.flowCustom.afterSubmitandGettip(this.svrConfig, this.mainForm.value);
      this.xn.router.navigate([`/new-gemdale/record/record/${this.flowId}`]);
    });
  }
