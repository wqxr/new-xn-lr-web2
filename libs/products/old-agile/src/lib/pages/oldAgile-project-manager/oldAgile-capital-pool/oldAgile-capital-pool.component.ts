...
import { CapitalType, DragonContractCreateType, SelectRange } from 'libs/shared/src/lib/common/dragon-vanke/emus';
...
import ProjectManagerCapitalList from './oldAgile-capital-pool';
...
@Component({
    selector: 'oldAgile-capital-management',
    templateUrl: `./oldAgile-capital-pool.component.html`,
    styleUrls: ['./oldAgile-capital-pool.component.css']
})
export class OldAgileCapitalpoolComponent implements OnInit, AfterViewInit, OnDestroy {
    tabConfig: any;  //表格配置
    ...
    */
    ...
    navBack() {
        if (this.isMachineenter === true) {
            ...
            this.xn.router.navigate(['/old-agile/assets-management/projectPlan-management'], {
                queryParams: {
    ...
    */
    ...
    private doGenerateOrSign(params: any, urls: any) {
        this.xn.api.dragon.post(urls.generate, params).subscribe(con => {
            ...
            if (result.length) {
                ...
                XnModalUtils.openInViewContainer(this.xn, this.vcr, FinancingFactoringVankeModalComponent, result).subscribe(x => {
                    if (x === 'ok') {
                        const p = con.data;
                        this.xn.loading.open();
     ...
     */
    addTransaction() {
        this.xn.router.navigate([`/old-agile/assets-management/enter-pool`], {
            queryParams: {
                ...
                if (x.data.isCapitalPoolSample) {
                    ...
                    XnModalUtils.openInViewContainer(this.xn, this.vcr, EditModalComponent, params).subscribe(v => {
                        if (v === null) {
                            ...
                            //抽样页面
                            this.xn.router.navigate(['/old-agile/assets-management/capital-sample'], {
                                queryParams: {
    ...
    // 数据分析
    dataAnalyse() {
        this.xn.router.navigate(['/old-agile/assets-management/capital-data-analyse'], {
            queryParams: {
                totalCount: this.pageConfig.total,
                sumReceive: this.sumReceive,
                ...this.queryParams
