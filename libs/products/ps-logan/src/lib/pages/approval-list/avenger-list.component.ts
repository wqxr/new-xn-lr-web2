...
export class DragonAvengerListComponent implements OnInit {
    ...
    */
    public initData(paramTabValue: string, init?: boolean,) {
        ...
        if (this.tabConfig.title.includes('审批放款-龙光-博时资本') && (this.defaultValue === 'A' || this.defaultValue === 'C')) {
            // isLgBoShi    0=非龙光博时资本 1= 龙光博时资本
            this.xn.api.post('/tool/is_jindie', { isLgBoShi: 1 }).subscribe(x => {
                if (x.data) {
                    this.is_jindie = x.data.isOpen;
                    if (this.is_jindie === 1) {
                        this.tabConfig.title = '审批放款-龙光-博时资本(已开启金蝶云审批)';
                    } else {
                        this.tabConfig.title = '审批放款-龙光-博时资本(线下人工审批)';
                    }
                }
            });
        }
        this.subDefaultValue = 'DOING'; // 重置子标签默认
        if (this.defaultValue === 'C' && this.tabConfig.title.includes('审批放款-龙光-博时资本') && !!!init) {
            this.pushApproval(1);
        } else if (this.defaultValue === 'A' && this.tabConfig.title.includes('审批放款-龙光-博时资本')) {
    ...
    public onPage(e?: { page: number, first?: number, pageSize?: number, pageCount?: number }, types?: number) {
        if (this.tabConfig.title.includes('审批放款-龙光-博时资本') && this.defaultValue === 'A') {
            ...
            XnModalUtils.openInViewContainer(this.xn, this.vcr, EditModalComponent, params).subscribe(v => {
                if (v === null) {
                    ...
                    this.xn[this.apiProxyEnum[this.defaultValue]].post(paramBtnOperate.post_url,
                        {
                            list,
                            isLgBoShi: 1 // isLgBoShi    0=非龙光博时资本 1= 龙光博时资本
                        }).subscribe(x => {
                            if (x.ret === 0) {
                                if (this.subDefaultValue === 'SPECIAL') {
     ...
     * @param i 下标
     */
    public handleRowClick(paramItem: any, paramBtnOperate: ButtonConfigModel, i: number) {
   ...
   */
    ...
    downloadApprovallist(paramItem: any) {
        ...
        XnModalUtils.openInViewContainer(
            ...
            // isLgBoShi    0=非龙光博时资本 1= 龙光博时资本
            this.xn.api.download(paramItem.post_url, { ...param, isLgBoShi: 1 })
                .subscribe((con: any) => {
