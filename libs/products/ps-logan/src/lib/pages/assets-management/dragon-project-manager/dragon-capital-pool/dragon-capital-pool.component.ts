...
import ProjectManagerCapitalList from './dragon-capital-pool';
declare const moment: any;
declare const $: any;

@Component({
    selector: 'dragon-capital-management',
    templateUrl: `./dragon-capital-pool.component.html`,
    styleUrls: ['./dragon-capital-pool.component.css']
})
export class DragonCapitalpoolComponent implements OnInit, AfterViewInit, OnDestroy {
    tabConfig: any;  // 表格配置
    ...
    private defaultHeadquarter = HeadquartersTypeEnum[5];   // 默认总部公司
    ...
    public uploadContract(row, head) {
        ...
        XnModalUtils.openInViewContainer(this.xn, this.vcr, NewFileModalComponent, params).subscribe(v => {
            ...
            param = {
                ...
                templateFlag: head.templateFlag,
    ...
    */
    ...
    navBack() {
        if (this.isMachineenter === true) {
            ...
            this.xn.router.navigate(['/pslogan/assets-management/projectPlan-management'], {
                queryParams: {
    ...
    public handleHeadClick(paramBtnOperate: ButtonConfigModel) {
        switch (paramBtnOperate.operate) {
            ...
            case 'remove_transaction':
                ...
                XnModalUtils.openInViewContainer(this.xn, this.vcr, VankeDeleteTransactionEditModalComponent, param).subscribe((x) => {
                    this.selectedItems = [];
                    setTimeout(() => {
     ...
     */
    signContacts() {
        this.xn.dragon.post('/contract/second_contract_info/create_sign_second_contract',
            { project_manage_id: this.projectId }).subscribe(x => {
                if (x.ret === 0) {
                    const fileList = x.data;
                    if (!fileList.length) {
                        return this.xn.msgBox.open(false, '暂无合同生成并签署');
                    }
                    XnModalUtils.openInViewContainer(this.xn, this.vcr, VankeCapitalPoolGeneratingContractModalComponent, { fileList }).subscribe(x => {
                        if (!!x) {
                            this.xn.loading.open();
                            // 准备参数
                            let params = {};
                            params = {
                                ...
                                style: x[0].style,
                                id: x[0].id,
                                templateFlag: x[0].templateFlag,
                                templateName: x[0].templateName,
                                isLgBoShi: 1 // 0=非龙光博时资本 1= 龙光博时资本
                            };
    ...
    */
                ...
                XnModalUtils.openInViewContainer(this.xn, this.vcr, VankeCapitalPoolGeneratingContractModalComponent, { fileList }).subscribe(z => {
                    if (!!z) {
                        ...
                        const params = {
                            ...
                            style: z[0].style,
                            id: z[0].id,
                            templateFlag: z[0].templateFlag,
                            templateName: z[0].templateName,
                            isLgBoShi: 1 // 0=非龙光博时资本 1= 龙光博时资本
                        };
     ...
     */
    viewProgress(mainFlowId: string) {
        this.xn.router.navigate([`pslogan/main-list/detail/${mainFlowId}`]);
    }
     ...
     */
    addTransaction() {
        this.xn.router.navigate([`/pslogan/assets-management/enter-pool`], {
            queryParams: {
                ...
                if (x.data.isCapitalPoolSample) {
                    ...
                    XnModalUtils.openInViewContainer(this.xn, this.vcr, EditModalComponent, params).subscribe(v => {
                        if (v === null) {
                            ...
                            // 抽样页面
                            this.xn.router.navigate(['/pslogan/assets-management/capital-sample'], {
                                queryParams: {
    ...
    // 数据分析
    dataAnalyse() {
        this.xn.router.navigate(['/pslogan/assets-management/capital-data-analyse'], {
            queryParams: {
                totalCount: this.pageConfig.total,
                sumReceive: this.sumReceive,
                ...this.queryParams
    ...
    startChange() {
        ...
        if (isOk) {
            const msg = this.selectedItems.filter((x) => x.tradeStatus !== 104).map((y) => y.mainFlowId).join('、');
