 ...
 * 1.0                 congying          项目计划管理         2020-01-17
 * **********************************************************************
 */
...
export class DragonProjectmanagePlanComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.router.queryParams.subscribe(x => {
      this.title = '项目管理-龙光-博时资本>' + x.title + '>' + x.projectName + '-' + x.headquarters;
    ...
    XnModalUtils.openInViewContainer(this.xn, this.vcr, EditModalComponent, params)
      .subscribe((v) => {
        if (v === null) {
          ...
          const params = { ...v, capitalPoolId: item.capitalPoolId };
  ...
  changeInfo(paramInfo: string) {
    ...
    XnModalUtils.openInViewContainer(
      ...
      { project: paramInfo, type: 1, isVanke, isLoganBoshi: true }
    ).subscribe(v => {
      if (v === 'ok') {
        this.onPage({ page: this.paging });
      }
...
*/
   ...
   */
  navBack() {
    this.xn.router.navigate(['/pslogan/assets-management/project-list'], {
      queryParams: {
  ...
  setPlan() {
    ...
    XnModalUtils.openInViewContainer(
      ...
      this.vcr,
      CapitalPoolsetProjectPlanModalComponent,
      { project: this.projectId, type: 2, isVanke, isLoganBoshi: true }
    ).subscribe(v => {
      if (v === 'ok') {
        this.onPage({ page: this.paging });
      }
