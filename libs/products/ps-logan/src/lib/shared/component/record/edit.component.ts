...
export class PsLoganEditComponent implements AfterViewInit, AfterViewChecked, OnInit, OnDestroy {
    ...
    public otherJudge(row): boolean {
        ...
        let flag = false;
        if (row.checkerId === 'dealContract' && row.flowId === 'vanke_platform_verify') {
            ...
            return flag;
        } else if (row.checkerId === 'dealContract' && row.flowId === 'vanke_financing') { // 供应商上传资料-交易合同必填
            return !rowVal;
        } else if (row.checkerId === 'performanceFile' && row.flowId === 'vanke_platform_verify') {
            flag = JSON.parse(rowVal).some(x => !(x.percentOutputValue && x.payType));
            return flag;
        } else if (row.checkerId === 'performanceFile' && row.flowId === 'vanke_financing') { // 供应商上传资料-履约证明必填
            return !rowVal;
    ...
    private copyDraft(shows: any[], hides: any[], fresh: boolean): void {
        ...
        if (['vanke_platform_verify', 'dragon_platform_verify'].includes(this.svrConfig.flow.flowId)
