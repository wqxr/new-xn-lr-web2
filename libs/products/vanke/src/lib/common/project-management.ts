...
import { ListHeadsFieldOutputModel, TabConfigModel, TabListOutputModel } from 'libs/shared/src/lib/config/list-config-model';
...
export default class ProjectManagerList {
    ...
    static getTabConfig(localStorageService: any, xn: any, listName: string): any {
        ...
        if(listName==='projectList'){
            tabConfig.tabList=tabConfig.tabList.concat(specialTabConfig.specialTabList);
        }
        tabConfig['level'] = 0;
...
class specialTabConfig{
    static specialTabList:TabListOutputModel[]=[   {
        label: 'ABN',
        value: 'G',
