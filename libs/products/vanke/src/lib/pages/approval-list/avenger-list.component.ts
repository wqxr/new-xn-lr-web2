...
import { ApiProxyEnum, SubTabEnum } from './approval-list-table';
...
import { PersonUploadloanReceiptComponent } from 'libs/shared/src/lib/public/dragon-vanke/modal/person-upload-loanreceipt-modal.component';
import {
  IsCashierType,
  IsProxyDef,
  HasPerformanceEnum,
...
@Component({
  ...
  styleUrls: ['./approval-list.less'],
})
export class VankeAvengerListComponent implements OnInit {
  ...
  onScroll($event: any) {
    ...
    if ($event.srcElement.scrollLeft !== this.scrollX) {
      ...
      if (this.currentTab.value === 'E') {
        const columLast2 = $('.height').find(
          '.head-height tr th:nth-last-child(2),.table-height tr td:nth-last-child(2)'
        ); // 操作列
        columLast2.each((index, col: any) => {
          // 固定最后一列
          col.style.transform = 'translateX(' + lastHeadX + 'px)';
          col.style.backgroundColor = '#fff';
        });
  ...
  public initData(paramTabValue: string, init?: boolean) {
    ...
    if (
      this.tabConfig.title.includes('审批放款-万科') &&
      ['A', 'C'].includes(this.defaultValue)
    ) {
      this.xn.api.post('/tool/is_jindie', {}).subscribe((x) => {
        if (x.data) {
          this.is_jindie = x.data.isOpen;
          if (this.is_jindie === 1) {
            this.tabConfig.title = '审批放款-万科(已开启金蝶云审批)';
          } else {
            this.tabConfig.title = '审批放款-万科(线下人工审批)';
          }
        }
      });
    }
      ...
      .subscribe(
        (x) => {
          if (x.data && x.data.data && x.data.data.length) {
            ...
            this.allPayableAmounts = x.data.changePrice
              ? x.data.changePrice
              : 0;
  ...
  watchApproval(type?: number) {
    this.xn.avenger.post2('/jd_blh/approval_progress', {}).subscribe((data) => {
      if (data.ret !== 0) {
        ...
        } else if (data.data.isCompleted === true && type === undefined) {
          if (this.subDefaultValue === 'SPECIAL') {
            this.cancelSpecial(this.subDefaultValue);
          } else {
            this.getList(this.currentParams);
  ...
  public cancelSpecial(paramSubTabValue?: string) {
    ...
    this.xn.loading.open();
    this.xn[this.apiProxyEnum[this.defaultValue]]
      .post('/aprloan/approval/list', params)
      .subscribe((x) => {
        this.selectedItems = [];
        this.selectedReceivables = 0;
        this.selectedPayableAmounts = 0;
    ...
    if (!this.isAllChecked()) {
      ...
      this.selectedItems.forEach((item) => {
        ...
        this.selectedPayableAmounts = Number(
          (
            this.selectedPayableAmounts +
            (item.changePrice ? item.changePrice : 0)
          ).toFixed(2)
        );
      });
    } else {
  ...
  public singleChecked(paramItem, index) {
    if (paramItem.checked && paramItem.checked === true) {
      ...
      this.selectedPayableAmounts = Number(
        (
          this.selectedPayableAmounts -
          (paramItem.changePrice ? paramItem.changePrice : 0)
        ).toFixed(2)
      );
    } else {
  ...
  public handleHeadClick(paramBtnOperate: ButtonConfigModel) {
    if (
      !['download_approval_list', 'system_loan_receipt'].includes(
        paramBtnOperate.operate
      ) &&
    ...
    if (
     this.shieldArray.includes(paramBtnOperate.operate) &&
      this.xn.user.roles.indexOf('admin') < 0
    ) {
    ...
    if (paramBtnOperate.operate === 'initiate_examination') {
      const handleApproval = () => {
        ...
        this.xn[this.apiProxyEnum[this.defaultValue]]
          .post(paramBtnOperate.post_url, { list })
          .subscribe((x) => {
            if (x.ret === 0) {
              if (this.subDefaultValue === 'SPECIAL') {
                ...
                // this.getList(this.currentParams);
                this.localStorageService.setCacheValue('isstartStatus', {
                  startStatus: true,
                });
      ...
      // FIXME: 去除中登撤销证明判断
      // if (
      //   this.subDefaultValue === 'PRETRADE' &&
      //   this.selectedItems.some(
      //     (c) =>
      //       !c.zdCancelCertificate || !JSON.parse(c.zdCancelCertificate).length
      //   )
      // ) {
      //   this.xn.msgBox.open(
      //     false,
      //     '必须上传《中登撤销证明》后，才能对“进度款前置融资产品”发起业务审批'
      //   );
      //   return;
      // } else {
      handleApproval();
      // }
    } else if (paramBtnOperate.operate === 'approval_ok') {
      // 审批成功
    ...
    } else if (paramBtnOperate.operate === 'approval_reject') {
      // 审批拒绝
      const mainflowIdlist = [];
      this.selectedItems.map((item) => {
        item.creditStatus = 2;
    ...
    } else if (paramBtnOperate.operate === 'sign_contract') {
      // 批量签署合同
      this.batchSign(paramBtnOperate);
    } else if (paramBtnOperate.operate === 'loan_ok') {
      // 放款成功
    ...
    } else if (paramBtnOperate.operate === 'loan_fail') {
      // 放款失败
      this.xn[this.apiProxyEnum[this.defaultValue]]
        .post(paramBtnOperate.post_url, {
          mainFlowIds: this.selectedItems.map((x: any) => x.mainFlowId),
    ...
    } else if (paramBtnOperate.operate === 'approval_again') {
      // 重新审批
      this.xn.router.navigate(
        [`/console/record/avenger/new/sub_approval_again_530`],
        {
          queryParams: {
      ...
      this.xn.avenger
        .download('/aprloan/approval/accountDownload', {
          mainFlowIds: this.selectedItems.map((x: any) => x.mainFlowId),
      ...
      XnModalUtils.openInViewContainer(
        ...
        if (v.cashierDown === IsCashierType.Wei_fang) {
          this.xn.avenger
            .download('/aprloan/approval/loadFinancingWf', {
              mainFlowIds: this.selectedItems.map((x: any) => x.mainFlowId),
          ...
          this.xn.avenger
            .download('/aprloan/approval/loadFinancingPf', {
              mainFlowIds: this.selectedItems.map((x: any) => x.mainFlowId),
              isProxy: this.selectedItems[0].isProxy,
            })
            .subscribe((x: any) => {
          ...
          this.xn.avenger
            .download('/aprloan/approval/loadFinancing', {
              mainFlowIds: this.selectedItems.map((x: any) => x.mainFlowId),
              isProxy: this.selectedItems[0].isProxy,
            })
            .subscribe((x: any) => {
      ...
      paramBtnOperate.operate === 'person_return_upload' ||
      paramBtnOperate.operate === 'system_loan_receipt'
    ) {
      this.getPersonreturnList(paramBtnOperate.operate);
    } else if (
      paramBtnOperate.operate === 'finance_approval_ok' ||
      paramBtnOperate.operate === 'finance_approval_fail'
    ) {
      const type = paramBtnOperate.operate === 'finance_approval_ok' ? 1 : 0;
      const mainFlowId = this.selectedItems.map((item) => item.mainFlowId);
      this.xn[this.apiProxyEnum[this.defaultValue]]
        .post(paramBtnOperate.post_url, { type: type, mainFlowIds: mainFlowId })
        .subscribe((x) => {
          if (x.data) {
      ...
      paramBtnOperate.operate === 'upload_zhongdeng_revocation_certificate'
    ) {
      const params = {
        title: '上传中登撤销证明',
        checker: [
          {
            title: '中登撤销证明',
            checkerId: 'zdCancelCertificate',
            type: 'dragonMfile',
            options: {
              fileext: 'jpg, jpeg, png, pdf',
              others: { helpMsg: 'viewMfile' },
              readonly: false,
            },
            required: 1,
          },
      ...
      XnModalUtils.openInViewContainer(
        ...
        const mainFlowId: any[] = this.selectedItems.map((c) => c.mainFlowId);
        this.xn.avenger
          .post('/aprloan/approval/uploadZdCancelCertificate', {
            mainFlowId,
            fileInfo: v.zdCancelCertificate,
          })
          .subscribe((res) => {
            this.initData('A', true);
          });
      });
    }
  }
  ...
  private batchSign(btn) {
    ...
    if (mainFlowIds[0].endsWith('lg')) {
      let isOk = false;
      let alert = '';
      const certificateStatus = this.selectedItems.map((x) => ({
        isPerformance: x.isPerformance,
        mainFlowId: x.mainFlowId,
      }));
      certificateStatus.forEach((x) => {
        if (
          x.isPerformance === HasPerformanceEnum.NONE ||
          x.isPerformance === HasPerformanceEnum.YES
        ) {
          alert += x.mainFlowId + '、';
          isOk = true;
        }
    ...
    this.xn[proxyType].post(btn.post_url, { mainFlowIds }).subscribe((con) => {
      const cons = JSON.parse(con.data.contracts);
      cons.isProxy = IsProxyDef.VANKE_ABS;
      ...
      cons.forEach((x) => {
        if (x.label.includes('国内无追索权商业保理合同')) {
          ...
          x.label === '国内商业保理合同线上线下通用版-华泰'
        ) {
          x.config.text = '甲方(公章/电子签章)';
          ...
          x.label.includes('国内商业保理合同(三方协议)') ||
        ...
        } else if (x.label.includes('国内商业保理合同（万科线上版）')) {
          x.config.text = '甲方(电子签章、数字签名)：';
        } else if (x.label.includes('国内商业保理合同（万科线下版）')) {
          x.config.text = '甲方(公章)：';
        } else if (
          x.label.includes('应收账款转让通知书（适用于供应商向债务人出具）') ||
          x.label.includes('应收账款转让通知书（适用于供应商向万科股份出具）')
        ) {
        ...
        this.xn[this.apiProxyEnum[this.defaultValue]]
          ...
          .subscribe((x) => {
            if (x.ret === 0) {
              ...
              this.localStorageService.setCacheValue('ispushStatus', {
                startStatus: true,
              });
  ...
  private buildCondition(searches) {
    ...
    // );
    // 深拷贝，并排序;
    this.shows = _.cloneDeep(objList.sort((a, b) => a.sortOrder - b.sortOrder));
    ...
    this.searchForm.valueChanges.subscribe((v) => {
      ...
      // 深拷贝;要进行搜索的变量
      this.arrObjs = _.cloneDeep(arrObj);
      this.onUrlData();
    });
  }
  ...
  private buildParams(addparams: number) {
    ...
    if (this.searches.length > 0) {
      for (const search of this.searches) {
        if (!XnUtils.isEmpty(this.arrObjs[search.checkerId])) {
          if (
            search.checkerId === 'cnDownTimes' ||
            search.checkerId === 'kjDownTimes'
          ) {
            params[search.checkerId] = Number(this.arrObjs[search.checkerId]);
          } else if (
            search.checkerId === 'startTime' ||
            search.checkerId === 'createTime'
          ) {
          ...
          } else if (search.checkerId === 'factoringDate') {
            const date = JSON.parse(this.arrObjs[search.checkerId]);
            params.factoringEndDateStart = date.beginTime;
            params.factoringEndDateEnd = date.endTime;
    ...
    // 列表子标签页为“进度款前置融资产品”，需要传isPreTrade，进行过滤
    if (this.currentSubTab.value === 'PRETRADE') {
      params.isPreTrade = IsPreTrade.YES;
    }
    params.headquarters = HeadquartersTypeEnum[1];
    return params;
  }
  ...
  public handleClick(type: number) {
    if (type === 1) {
      this.xn.router.navigate([
        `/console/record/avenger/new/sub_approval_honor_530`,
      ]);
    } else if (type === 2) {
      this.xn.router.navigate([
        `/console/record/avenger/new/sub_approval_return_530`,
      ]);
    } else if (type === 3) {
    }
  }
  ...
  openApproval(Paramitem: any) {
    this.xn.avenger
      .post('/jd_blh/approval_info', { mainFlowId: Paramitem.mainFlowId })
      .subscribe((x) => {
        if (x.ret === 0) {
          XnModalUtils.openInViewContainer(
            this.xn,
  ...
  openFrees(Paramitem: any) {
    this.xn.avenger
      .post('/jd_blh/fee_info', { mainFlowId: Paramitem.mainFlowId })
      .subscribe((x) => {
        if (x.ret === 0) {
          XnModalUtils.openInViewContainer(
            this.xn,
  ...
  openChangedetail(paramItem: any) {
    ...
    this.xn.avenger
      .post('/aprloan/approval/changeList', {
        mainFlowId: paramItem.mainFlowId,
      })
      .subscribe((x) => {
        if (x.ret === 0) {
          XnModalUtils.openInViewContainer(
  ...
  openResultcompare(paramItem: any) {
    this.xn.avenger
      .post('/aprloan/approval/fkqrsList', { mainFlowId: paramItem.mainFlowId })
      .subscribe((x) => {
        if (x.ret === 0) {
          XnModalUtils.openInViewContainer(
            this.xn,
  ...
  downloadApprovallist(paramItem: any) {
    ...
    XnModalUtils.openInViewContainer(
      ...
      this.xn.api.download(paramItem.post_url, param).subscribe((con: any) => {
