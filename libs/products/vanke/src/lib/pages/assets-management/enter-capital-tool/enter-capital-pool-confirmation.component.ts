 ...
 * 1.0                 congying       万科-拟入池交易列表   2020-08-19
 * **********************************************************************
 */
...
import { SelectRange, CapitalType } from './emus';
...
import { RiskFreezeStatusEnum } from 'libs/shared/src/lib/config/enum';
...
export class VankeEnterpoolComponent implements OnInit {
    ...
    public isFirst = true;
    public headquarters = HeadquartersTypeEnum[1];
    fitProject = '';
    ...
    public handleHeadClick(paramBtnOperate: ButtonConfigModel) {
        if (paramBtnOperate.operate === 'enter-capitalpool') { // 移入资产池
            ...
            this.exportCapital();
        } else if (paramBtnOperate.operate === 'frozen_risk') {
            // 冻结/解冻风险审核
            this.frozenRisk(paramBtnOperate);


        }
    }
    private frozenRisk(paramBtnOperate: ButtonConfigModel) {
        if (this.selectedItems.length === 0) {
            this.xn.msgBox.open(false, '请选择交易');
            return;
        }
        const mainlds = this.selectedItems.map(x => x.mainFlowId);
        this.xn.loading.open();
        this.xn.api.dragon.post(paramBtnOperate.post_url, { mainFlowIds: mainlds }).subscribe(x => {
            if (x.ret === 0) {
                let msg='';
                if(this.selectedItems.map(x=>x.riskFreezeStatus)[0]===RiskFreezeStatusEnum.UNFREEZE){
                    msg='冻结成功';
                }else{
                    msg='解冻成功';
                }
