...
import {
  CapitalType,
  DragonContractCreateType,
  SelectRange,
  SelectContent,
} from 'libs/shared/src/lib/common/dragon-vanke/emus';
...
import ProjectManagerCapitalList from './vanke-capital-pool';
import { AgencyType } from 'libs/shared/src/lib/config/angency-auth';
...
@Component({
  ...
  styleUrls: ['./vanke-capital-pool.component.css'],
})
export class VankeCapitalpoolComponent
  ...
  private defaultHeadquarter = HeadquartersTypeEnum[1]; // 默认总部公司
  ...
  ngOnInit(): void {
    ...
    this.router.data.subscribe((x) => {
      const undetermTabConfig = ProjectManagerCapitalList.getTabConfig(
        this.localStorageService,
        this.xn,
        'cpaitalTadeList'
      );
      this.tabConfig = !!undetermTabConfig
        ? undetermTabConfig
        : new TabConfigModel();
  ...
  tradeChanges() {
    if (this.defaultValue === 'A') {
      this.xn.dragon
        .post('/project_manage/pool/trade_change_tip', {
          capitalPoolId: this.capitalPoolId,
        })
        .subscribe((x) => {
          if (x.ret === 0 && x.data) {
  ...
  getHeadorSearch(Tabconfig: any) {
    ...
    console.log('newCurrentsubTab===>', newCurrentsubTab);
    this.xn.dragon
      .post('/project_manage/file_contract/list_search', {
        ...
        if (y.ret === 0) {
          y.data.forEach((z) => {
            newCurrentsubTab.headText.push({
              label: z.label,
              value: z.bodyContract,
              show: true,
              type: 'contract',
              bodyContractYyj: z.bodyContractYyj,
              templateFlag: z.templateFlag,
            });
          });
          y.data.forEach((z) => {
            newCurrentsubTab.searches.push({
              title: z.label,
              checkerId: z.searchContract,
              type: 'select',
              required: false,
              sortOrder: 10,
              show: true,
          ...
          if (this.proxy === 52) {
            newCurrentsubTab.searches = newCurrentsubTab.searches.filter(
              (x) => x.checkerId !== 'statusFactoringPayConfirm'
            );
          }
  ...
  private isDifferentCompany() {
    const company =
      XnUtils.distinctArray(
        this.listInfo.filter((x) => x.isProxy).map((c) => c.isProxy)
      ) || [];
  ...
  private buildCondition(searches) {
    ...
    for (let i = 0; i < searches.length; i++) {
      ...
      if (
        searches[i].checkerId === 'type' ||
        searches[i].checkerId === 'contractType'
      ) {
        searches[i].options.ref =
          this.proxy === 52 ? 'dragonContracttype' : 'vankeContracttype';
      }
  ...
  public checkAll() {
    ...
    this.selectSumReceive = this.selectedItems
      .reduce(
        (accumulator, currentValue) =>
          accumulator + Number(currentValue.receive ? currentValue.receive : 0),
        0
      )
      .toFixed(2);
    this.selectSumChangePrice = this.selectedItems
      .reduce(
        (accumulator, currentValue) =>
          accumulator +
          Number(currentValue.changePrice ? currentValue.changePrice : 0),
        0
      )
      .toFixed(2);
  }
  ...
  public singleChecked(paramItem, index) {
    if (paramItem.checked && paramItem.checked === true) {
      ...
      this.selectedItems = this.selectedItems.filter(
        (x) => x.mainFlowId !== paramItem.mainFlowId
      );
    } else {
      ...
      for (const search of this.searches) {
        if (!XnUtils.isEmpty(this.arrObjs[search.checkerId])) {
          if (search.checkerId === 'transactionStatus') {
            ...
            if (arr && arr.length) {
              const lawSurveyList = arr
                .filter((x) => Number(x) <= 5)
                .map((y) => Number(y));
              const managerSurveyList = arr
                .filter((x) => Number(x) > 5)
                .map((y) => Number(y - 5));
  ...
  navBack() {
    if (this.isMachineenter === true) {
      ...
      this.xn.router.navigate(
        ['/vanke/assets-management/projectPlan-management'],
        {
          queryParams: {
  ...
  public handleHeadClick(paramBtnOperate: ButtonConfigModel) {
    if (paramBtnOperate.operate === 'generate_Contract') {
      ...
      this.xn.dragon
        .post('/project_manage/pool/update_change_pool', {
          capitalPoolId: this.capitalPoolId,
        })
        .subscribe((x) => {
  ...
  syncConfirmation() {
    ...
    const mainFlowIdList = this.selectedItems.map(x => x.mainFlowId);
    this.xn.loading.open();
    this.xn.api.dragon.post('/sub_system/download_pay_confirm/download_buyer_file', { mainFlowIdList: mainFlowIdList }).subscribe(x => {
      this.xn.loading.close();
      if (x.ret === 0 && x.data.data.length > 0) {
        // 打开弹框
        const params = {
          get_url: '',
          get_type: '',
          ...
          heads: [
            ...
            { label: '信息', value: 'msg', type: 'text' },
            { label: '结果', value: 'result', type: 'selects', options: 'payConfimDownload' }
          ],
          searches: [],
          key: '',
          data: x.data.data,
          total: x.data.data.length,
          inputParam: {},
          other: {},
          rightButtons: [{ label: '确定', value: 'submit' }]
        };
  ...
  // 限制同步买方确认函按钮
  isShowConfirm(){
    return this.defaultValue==='B' && this.xn.user.agencyType===AgencyType['factor']
  }
  ...
  signContacts() {
    this.xn.dragon
      .post('/contract/second_contract_info/create_sign_second_contract', {
        project_manage_id: this.projectId,
      })
      .subscribe((x) => {
        if (x.ret === 0) {
          ...
          XnModalUtils.openInViewContainer(
            ...
            if (!!x) {
              ...
              const url = this.returnContractUrl(2);
  ...
  public generateContacts() {
    this.xn.dragon
      .post('/contract/second_contract_info/create_second_contract', {
        project_manage_id: this.projectId,
      })
      .subscribe((x) => {
        if (x.ret === 0) {
          ...
          XnModalUtils.openInViewContainer(
            ...
            if (!!z) {
              ...
              const url = this.returnContractUrl(1);
              this.doGenerateOrSign(params, url);
            }
          });
        }
  ...
  public doPush() {
    ...
    this.xn.msgBox.open(
      true,
      [`是否要推送以下交易？（共${selectArr.length}笔）`, ...selectArr],
      () => {
        this.xn.loading.open();
        // 构建阐述
        const param = {
  ...
  public doPushAdd() {
    ...
    if (this.proxy !== 52) {
      this.xn.msgBox.open(
        false,
        '仅总部公司为“深圳市龙光控股有限公司”的业务可推送企业(补充协议)'
      );
      return;
    }
    const selectArr = this.selectedItems.map((x) => x.mainFlowId);
        ...
        result.forEach((x) => {
          ...
          if (this.proxy === 53) {
            if (x.label === '国内无追索权商业保理合同（ABS版本）') {
              ...
              x.label === '应收账款债权转让通知书（适用于保理商通知）-国寿' || x.label==='应收账款转让通知书（适用于保理商向债务人及万科股份出具）-华泰'
            ) {
  ...
  viewProgress(mainFlowId) {
    this.xn.router.navigate([`vanke/main-list/detail/${mainFlowId}`]);
  }
  ...
  viewDetailFunc(mainFlowId: string) {
    ...
    this.xn.dragon
      .post('/project_manage/file_contract/business_detail', { mainFlowId })
      .subscribe((x) => {
        if (x.ret === 0) {
          this.params.checker.forEach((obj) => {
            if (obj.value === 'type') {
              ...
              obj.value === 'headPreDate' ||
              obj.value === 'factoringEndDate' ||
              obj.value === 'realLoanDate' ||
              obj.value === 'priorityLoanDate'
            ) {
              obj.data = x.data[0][obj.value]
                ? moment(x.data[0][obj.value]).format('YYYY-MM-DD')
                : '';
            } else {
              obj.data = x.data[0][obj.value] || '';
            }
  ...
  viewSurveyInfo(
    ...
    firstAdvise: string,
    surveyMan: string,
    firstSurveyMan: string
  ) {
    const _advise = item[advise] || item[firstAdvise];
    const _type = item[advise] ? '终审尽调意见' : '初审尽调意见';
    const _surveyMan = item[surveyMan] || item[firstSurveyMan];
    const params: EditParamInputModel = {
      title: '查看尽调意见',
      checker: [
        {
    ...
    XnModalUtils.openInViewContainer(
      ...
      if (!!x) {
        ...
        const param = {
          headquarters: this.headquarters,
          mainFlowIdList:
            +x.scope === SelectRange.All || +x.scope === SelectRange.Sample
              ? undefined
        ...
        if (Number(x.contentType) === SelectContent.Default) {
          this.xn.api.dragon
            .download('/project_manage/file_contract/down_excel', param)
            .subscribe(
              (v: any) => {
  ...
  public handleRowClick(item: any, btn: ButtonConfigModel) {
    if (btn.operate === 'sub_start_survey') {
      ...
      if (
        [101, 102].includes(DragonflowIdEnum[flowId]) ||
        [201, 202].includes(VankeflowIdEnum[flowId])
      ) {
        this.xn.msgBox.open(
          false,
          `供应商尚未上传资料，暂时无法尽调，请稍候再试`
        );
        return '';
      }
      // 经办人（管理人或律所角色）
  ...
  public batchModify() {
    ...
    this.selectedItems.map(
      (sel) =>
        (sel.headquarters =
          this.headquarters === HeadquartersTypeEnum[1]
            ? '万科'
            : this.headquarters)
    );
  ...
  private selectRowAndDifferentCompany() {
    ...
    // 未选择列表中数据时，检查公司名称是否一致
    if (!params.hasSelect) {
      params.selectedCompany = XnUtils.distinctArray(
        this.listInfo.map((c) => tradeType[c.isProxy])
      );
      ...
      params.selectedCompany = XnUtils.distinctArray(
        selectedRows.map((c) => tradeType[c.isProxy])
      ).toString();
    }
    return params;
  }
   ...
   */
  addTransaction() {
    this.xn.router.navigate([`/vanke/assets-management/enter-pool`], {
      queryParams: {
  ...
  capitalSample() {
    this.xn.dragon
      .post('/sample/get_sample_sign', { capitalPoolId: this.capitalPoolId })
      .subscribe((x) => {
        if (x.ret === 0 && x.data) {
          if (x.data.lastMainFlowIdList && x.data.lastMainFlowIdList.length) {
            ...
            this.localStorageService.setCacheValue(
              'assetSampleList',
              JSON.stringify(x.data.lastMainFlowIdList)
            );
          }
          if (x.data.isCapitalPoolSample) {
            const params: EditParamInputModel = {
              ...
              checker: [
                {
                  title: '抽样类型',
                  checkerId: 'selectSample',
                  type: 'radio',
                  required: 1,
                  options: { ref: 'selectSample' },
                },
              ] as CheckersOutputModel[],
              buttons: ['取消', '继续抽样'],
            };
            XnModalUtils.openInViewContainer(
              ...
              if (v === null) {
                ...
                // 抽样页面
                this.xn.router.navigate(
                  ['/vanke/assets-management/capital-sample'],
                  {
                    queryParams: {
  ...
  // 数据分析
  dataAnalyse() {
    this.xn.router.navigate(['/vanke/assets-management/capital-data-analyse'], {
      queryParams: {
        totalCount: this.pageConfig.total,
        sumReceive: this.sumReceive,
        ...this.queryParams,
  ...
  startChange() {
    ...
    if (this.proxy !== 52) {
      this.xn.msgBox.open(
        false,
        '仅总部公司为“深圳市龙光控股有限公司”的业务可发起变更'
      );
      return;
    }

    const isOk = this.selectedItems.some((x) => x.tradeStatus !== 104);
    if (isOk) {
      const msg = this.selectedItems
        .filter((x) => x.tradeStatus !== 104)
        .map((y) => y.mainFlowId)
        .join('、');
      this.xn.msgBox.open(
        false,
        `交易${msg}还没有完成供应商签署合同，不能发起变更发行流程`
      );
      return;
    }

    const params = {
      hasSelect: this.hasSelectRow(),
      selectedCompany: tradeType[52],
    };
    XnModalUtils.openInViewContainer(
      this.xn,
      this.vcr,
