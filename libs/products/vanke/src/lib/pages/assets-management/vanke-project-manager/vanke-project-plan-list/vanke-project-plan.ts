...
export default class ProjectManagerplanList {
  ...
  static uploadFileConfig = [
    {
      title: '计划管理人文件',
      checkerId: 'palnManagerFile',
      type: 'dragonMfile',
      required: false,
      value: '',
      leftButtons: [
        { label: '下载所有', operate: 'downloadAll', readonly: true },
        { label: '下载当前文件', operate: 'downloadNow', readonly: true },
      ],
      options: {
   ...
   *  @param listName 列表名称
   */
  static getTabConfig(localStorageService: any, listName: string): any {
    const projectManageAuthMap =
      localStorageService.caCheMap.get('projectManageAuth');
    ...
    tabConfig.level = 0;
    tabConfig[LevelEnum[0]] = ProjectManagerplanList.filterAuth(
      tabConfig[LevelEnum[0]],
      agencyInfo,
      LevelEnum[0],
      0
    );
  ...
  static filterAuth(
    authInfo: any,
    agencyInfo: { orgType: number | string; agencyType: number | string },
    nodeName: string,
    level: number
  ) {
    if (Object.prototype.toString.call(authInfo) === '[object Object]') {
      for (const key of Object.keys(authInfo)) {
        authInfo[key] = ProjectManagerplanList.filterAuth(
          authInfo[key],
          agencyInfo,
          key,
          level
        ...
        .map((tabObj: any) => {
          ...
          if (LevelEnum[level] === 'tabList') {
            tabObj[LevelEnum[tabObj.level]].forEach((subObj: any) => {
              subObj.headText = [
                ...ProjectManagerplanList.headSearches[tabObj.value][
                  subObj.value
                ].heads,
              ];
              subObj.searches = [
          ...
          if (
            tabObj.hasOwnProperty(LevelEnum[tabObj.level]) &&
            tabObj[LevelEnum[tabObj.level]]
          ) {
            tabObj[LevelEnum[tabObj.level]] = ProjectManagerplanList.filterAuth(
              tabObj[LevelEnum[tabObj.level]],
              agencyInfo,
              LevelEnum[tabObj.level],
              tabObj.level
            );
          }
          return tabObj;
  ...
  static getOrgUploadFileConfig(localStorageService: any, xn: any) {
    const agencyFilesAuthMap =
      localStorageService.caCheMap.get('agencyFilesAuth');
    const agencyInfoMap = localStorageService.caCheMap.get('agencyInfo');
    const agencySession = {
      orgType: Number(window.sessionStorage.getItem('orgType')),
      agencyType: Number(window.sessionStorage.getItem('agencyType')),
    ...
    const agencyInfo = !!agencyInfoMap
      ? JSON.parse(agencyInfoMap)
      : XnUtils.deepCopy(agencySession, {});
    const fileConfig = XnUtils.deepCopy(
      ProjectManagerplanList.uploadFileConfig,
      []
    );
    const returnFileConfig = fileConfig.filter((obj: any) =>
      agencyFilesAuth[obj.checkerId].agencyTypeList.includes(
        Number(agencyInfo.agencyType)
      )
    );
    returnFileConfig.forEach((item: any) => {
      const fileAuth = agencyFilesAuth[item.checkerId];
      if (
        fileAuth.download.includes(Number(agencyInfo.agencyType)) &&
        !fileAuth.delete.includes(Number(agencyInfo.agencyType)) &&
      ...
      // 查看文件 下载按钮权限
      if (fileAuth.download.includes(Number(agencyInfo.agencyType))) {
        item.leftButtons.forEach((t) => {
          t.readonly = false;
        });
      }
    });
