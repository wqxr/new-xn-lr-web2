...
export class RecordVankeComponent implements OnInit, AfterViewInit {
    ...
    constructor(private xn: XnService, private vcr: ViewContainerRef, private router: ActivatedRoute,) {
        ...
        this.type = SelectOptions.get('productType_dw');
    ...
    doProcess(record: any) {
        ...
        if ((record.status !== 1 && record.status !== 0)
            ...
            this.xn.router.navigate([`/vanke/record/${this.flowId1}/view/${record.recordId}`]);
        } else {
            this.xn.router.navigate([`/vanke/record/${this.flowId1}/edit/${record.recordId}`]);
        }
    }
    ...
    newProcess() {
        ...
        XnModalUtils.openInViewContainer(this.xn, this.vcr, EditNewVankeModalComponent, params).subscribe(x => {
            if (!!x) {
                ...
                this.flowId1 = [VanketypeEnum.ABN,VanketypeEnum.ABS,VanketypeEnum.ZBD].includes(Number(channel.proxy))? 'vanke_abs_step_financing_pre' : 'vanke_financing_pre';
                if (x) {
                    this.xn.router.navigate([`/vanke/record/new/${this.flowId1}/${HeadquartersTypeEnum[1]}`],
                        {
                            queryParams: {
