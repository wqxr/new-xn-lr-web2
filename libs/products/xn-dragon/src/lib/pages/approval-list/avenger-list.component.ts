...
export class DragonAvengerListComponent implements OnInit {
  ...
  public initData(paramTabValue: string, init?: boolean,) {
    ...
    if (this.tabConfig.title.includes('审批放款-龙光') && (this.defaultValue === 'A' || this.defaultValue === 'C')) {
      ...
      this.xn.api.post('/tool/is_jindie', {}).subscribe(x => {
        if (x.data) {
          ...
          if (this.is_jindie === 1) {
            this.tabConfig.title = '审批放款-龙光(已开启金蝶云审批)';
          } else {
            this.tabConfig.title = '审批放款-龙光(线下人工审批)';
          }
        }
      });
    }
    this.subDefaultValue = 'DOING'; // 重置子标签默认
    if (this.defaultValue === 'C' && this.tabConfig.title.includes('审批放款-龙光') && !!!init) {
      this.pushApproval(1);
    } else if (this.defaultValue === 'A' && this.tabConfig.title.includes('审批放款-龙光')) {
  ...
  public onPage(e?: { page: number, first?: number, pageSize?: number, pageCount?: number }, types?: number) {
    if (this.tabConfig.title.includes('审批放款-龙光') && this.defaultValue === 'A') {
  ...
  /**
   *  查看合同，只读
   * @param paramContractInfo paramContractInfo
   */
  ...
  /**
   *  判断数据是否长度大于显示最大值
   * @param paramFileInfos paramFileInfos
   */
 ...
 adminControl(){
  ...
  /**
   *  表头按钮组事件
   * @param paramBtnOperate paramBtnOperate
   */
  public handleHeadClick(paramBtnOperate: ButtonConfigModel) {
    ...
    if (paramBtnOperate.operate === 'initiate_examination') {    // 发起业务审批
    ...
    } else if (paramBtnOperate.operate === 'loadFinancing') {// 下载出纳表格
      this.loadFinancing();
      ...
      const types = paramBtnOperate.operate === 'finance_approval_ok' ? ApprovalType.SUCCESS : ApprovalType.FAIL;
      const mainFlowId = this.selectedItems.map(item => item.mainFlowId);
      this.xn[this.apiProxyEnum[this.defaultValue]].post(paramBtnOperate.post_url, { type: types, mainFlowIds: mainFlowId })
        .subscribe(x => {
          if (x.data) {
  ...
  private batchSign(btn) {
    ...
    this.xn[proxyType].post(btn.post_url, { mainFlowIds })
      .subscribe(con => {
        ...
        // TODO: change the way to solve data
        cons.forEach(x => {
   ...
   */
  enterCapitalPool(paramsValue: any) {
    const params = {
    ...
    this.xn.dragon.post('/trade/get_mainflowid_pool_info', params).subscribe(x => {
      if (x.ret === 0 && x.data.capitalPoolId !== '') {
        ...
        this.xn.router.navigate(['/xnlogan/capital-pool/trading-list'], {
          queryParams: {
  ...
  private buildCondition(searches) {
    ...
    const objList = [];
    const timeId = this.searches.filter(v => v.type === 'quantum').map(v => v.checkerId);
    this.timeId = XnUtils.deepClone(timeId);
    for (const searh of searches) {
      const obj = {} as any;
      obj.title = searh.title;
      obj.checkerId = searh.checkerId;
      obj.required = false;
      obj.type = searh.type;
      obj.sortOrder = searh.sortOrder;
      obj.options = searh.options;
      if (searh.checkerId === this.timeId[0]) {
        obj.value = JSON.stringify(tmpTime);
      } else {
        obj.value = this.arrObjs[searh.checkerId];
      }
      objList.push(obj);
    }
    // 深拷贝，并排序;
    const sortArr = objList.sort((a, b) => a.sortOrder - b.sortOrder);
    this.shows = XnUtils.deepClone(sortArr);
    ...
    this.searchForm.valueChanges.subscribe((v) => {
      ...
      if (changeId !== '' && this.nowTimeCheckId) {
        ...
        // FIXME: 这里的parseInt 方法如果没有传递 radix 参数，对于数值转换不是很安全
        // we may read from https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/parseInt
      ...
      this.arrObjs = XnUtils.deepClone(arrObj); // $.extend(true, {}, arrObj); // 深拷贝;要进行搜索的变量
      this.onUrlData();
    });
  }
...
enum docTextEnum {
  ELSE = '（盖章）',
  '国内无追索权商业保理合同' = '乙方（保理商、受让人）数字签名',
  '应收账款转让协议书' = '乙方（受让方）',
  '应收账款转让登记协议' = '乙方（受让方）',
  '国内商业保理合同(三方协议)' = '甲方(电子签章、数字签名)',
  '应收账款转让清单' = '甲方(电子签章、数字签名)',
  '国内商业保理合同-线上版（新增邮储版）' = '甲方(电子签章、数字签名)',
  '保理合同-国寿' = '保理商（盖章）',
  '邮储' = '保理商： （公章）',
  '国内商业保理合同（万科线上版）' = '甲方(电子签章、数字签名)：',
  '国内商业保理合同（万科线下版）' = '甲方(公章)：',
  '应收账款转让通知书（适用于供应商向债务人出具）' = '供应商/债权人：',
  '应收账款转让通知书（适用于供应商向万科股份出具）' = '供应商/债权人：',
  '国内商业保理合同(万科版-通力电梯修订版)' = '甲方授权代表：'
}
