...
export class CapitalDataAnalyseComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    public pageConfig = {
        contractTypeList: {
            total: 0,  //总数据量
            size: 5,  //每页显示数量
            pageArr: [5, 10, 20, 30, 50, 100], //总页数数组
            currentPage: 1, //当前页码,
    ...
    private buildRows(): void {
        const config = CapitalcapitalDataAnalyseConfig.setRowsValue(this.rowsData);
   ...
   */
    ...
    onPage(checkerId: string){
        ...
        const tableList = this.deepCopy(this.tableListData[checkerId], []);
