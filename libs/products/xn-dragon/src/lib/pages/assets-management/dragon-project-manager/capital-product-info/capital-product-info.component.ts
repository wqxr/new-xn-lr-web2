...
@Component({
    selector: 'dragon-capital-product-info',
    templateUrl: './capital-product-info.component.html',
    styleUrls: ['./capital-product-info.component.css']
})
export class DragonCapitalProductInfoComponent implements OnInit, AfterViewInit, OnDestroy {
    @Input() params: any;
    ...
    private buildRows(): void {
        ...
        this.xn.dragon.post('/project_manage/pool/pool_invest_tier_list', param).subscribe(res => {
            ...
            // 基础资产池概况
            const config = CapitalcapitalProductInfoConfig.setRowsValue(params);
    ...
    onPage(checkerId: string, e?: { page: number, first?: number, pageSize?: number, total?: number }) {
        ...
        const post = CapitalcapitalProductInfoConfig.tableListAll.find(tableObj => tableObj.checkerId === checkerId);
    ...
    public handleRowClick(item: any, btn: any, index: number) {
        if (btn.operate === 'project_file_download') {
            ...
            const files = JSON.parse(item.files)[0];
            // let dirName = FileType[item.fileType] + '-' + fileName + '-' + new Date().getTime() + '.zip';
            // this.xn.api.dragon.download('/project_manage/file_agency/download_agency_file', param).subscribe((v: any) => {
            //     this.xn.loading.close();
            //     this.xn.api.dragon.save(v._body, dirName);
            // });

            // this.xn.loading.open();
            // this.xn.api.dragon.download('/file/downFile', {
            //     files: new Array(JSON.parse(item.files)),
            // }).subscribe((v: any) => {
            //     this.xn.loading.close();
     ...
     */
    public onCancel() {
        this.xn.router.navigate(['/xnlogan/assets-management/capital-pool'], {
            queryParams: {
                ...this.queryParams
            }
        });
