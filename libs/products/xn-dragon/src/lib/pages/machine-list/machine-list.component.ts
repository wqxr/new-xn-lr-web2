...
@Component({
  selector: 'xn-dragon-list-component',
  ...
  */
  ...
  handmore(item: any) {
    ...
    if (item.nowProcedureId === '@end') {
      this.xn.router.navigate([`/xnlogan/record/todo/view/${item.recordId}`]);
    }
    ...
    if (!isOk) {
      ...
      if (item.nowProcedureId === 'operate' || item.nowProcedureId === 'review') {
        if (item.flowId === 'dragon_platform_verify' && item.zhongdengStatus === 1) {
          ...
          this.xn.router.navigate([`/xnlogan/record/todo/edit/${item.recordId}`]);
        }
      } else {
        this.xn.router.navigate([`/xnlogan/record/todo/view/${item.recordId}`]);
      }
