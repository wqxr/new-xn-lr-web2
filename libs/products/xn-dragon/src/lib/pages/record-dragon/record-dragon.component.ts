...
export class RecordDragonComponent implements OnInit, AfterViewInit {
    ...
    doProcess(record: any) {
        ...
        if ((record.status !== 1 && record.status !== 0)
            ...
            this.xn.router.navigate([`/xnlogan/record/${this.flowId}/view/${record.recordId}`]);
        } else {
            this.xn.router.navigate([`/xnlogan/record/${this.flowId}/edit/${record.recordId}`]);
        }
    }
    ...
    newProcess() {
        ...
        const params = {
            ...
            checker: [
                {
                    ...
                    options: { ref: 'newenterprise_dragon2' } // '万科企业股份有限公司'
                },
                {
                    title: '渠道', checkerId: 'productType', type: 'linkage-select', required: 1,
                    options: { ref: 'productType_logan' } // '万科企业股份有限公司'
                },
                {
            ...
            info: { flowId: 'vanke_financing_pre', headquarters: HeadquartersTypeEnum[7] }
        };
        XnModalUtils.openInViewContainer(this.xn, this.vcr, EditNewVankeModalComponent, params).subscribe(x => {
            if (!!x) {
                ...
                if (x) {
                    this.xn.router.navigate([`/xnlogan/record/new/${this.flowId}/${HeadquartersTypeEnum[5]}`],
                        {
                            queryParams: {
