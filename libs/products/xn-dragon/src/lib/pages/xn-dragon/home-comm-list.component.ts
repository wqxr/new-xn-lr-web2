...
import HuaQiaoCommBase from './comm-base';
...
import XnDragonCommBase from './comm-base';
 ...
 */
@Component({
  selector: 'xndragoncity-home-comm-list-component',
  templateUrl: './home-comm-list.component.html',
  styleUrls: [
    './home-comm-list.component.css'
  ]
})
export class XnDragonCommListComponent extends CommonPage implements OnInit {
  @Input() supers: any;
  total = 0;
  rows: any[] = [];
  words = '';
  ...
  ngOnInit() {
    ...
    this.base = new XnDragonCommBase(this, this.superConfig);
  ...
  viewProcess(paramMainFloId: string) {
    this.xn.router.navigate([
      `xnlogan/main-list/detail/${paramMainFloId}`
    ]);
  }
  ...
  public doProcess(record: any) {
    record.proxyType = record.proxyType === ProxyType.AVENGER ? ProxyType.AVENGER : ProxyType.XN_LOGAN;
