 ...
 * All rights reserved.
 *
 * @file：XnGemdaleAvengerListComponent
 * @summary：金地审批放款列表
 * @version: 1.0
 * **********************************************************************
...
export class XnGemdaleAvengerListComponent implements OnInit {
  ...
  viewProcess(mainFlowId: string) {
    this.xn.router.navigate([`/xn-gemdale/main-list/detail/${mainFlowId}`])
  }
  ...
  */
  public initData(paramTabValue: string, init?: boolean,) {
    ...
    if (this.tabConfig.title.includes('审批放款-金地-香纳') && (this.defaultValue === 'A' || this.defaultValue === 'C')) {
      this.xn.api.post('/tool/is_jindie', {}).subscribe(x => {
        if (x.data) {
          this.is_jindie = x.data.isOpen;
          if (this.is_jindie === 1) {
            this.tabConfig.title = '审批放款-金地-香纳(已开启金蝶云审批)';
          } else {
            this.tabConfig.title = '审批放款-金地-香纳(线下人工审批)';
          }
        }
      });
    }
    this.subDefaultValue = 'DOING'; // 重置子标签默认
    if (this.defaultValue === 'C' && this.tabConfig.title.includes('审批放款-金地-香纳') && !!!init) {
      this.pushApproval(1);
    } else if (this.defaultValue === 'A' && this.tabConfig.title.includes('审批放款-金地-香纳')) {
  ...
  public onPage(e?: { page: number, first?: number, pageSize?: number, pageCount?: number }, types?: number) {
    if (this.tabConfig.title.includes('审批放款-金地-香纳') && this.defaultValue === 'A') {
  ...
  public handleHeadClick(paramBtnOperate: ButtonConfigModel) {
    ...
    switch (paramBtnOperate.operate) {
      ...
      case 'system_loan_receipt':
        this.getPersonreturnList(paramBtnOperate.operate);
        break;
      case 'finance_approval_ok': // 财务审批通过
        this.financeApprovalOk(paramBtnOperate);
        break;
      case 'finance_approval_fail': // 财务审批拒绝
        this.financeApprovalOk(paramBtnOperate);
        break;
      default:
        break;
