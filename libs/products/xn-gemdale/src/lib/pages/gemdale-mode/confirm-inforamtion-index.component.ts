 ...
 * All rights reserved.
 *
 * @file：ComfirmInformationIndexComponent
 * @summary：项目公司确认应收账款金额、总部公司签署付款确认书、项目公司回执签署页面
 * @version: 1.0
 * **********************************************************************
...
export class ComfirmInformationIndexComponent implements OnInit {
    ...
    private signPagmentConfirm(btn) {
        this.localStorageService.setCacheValue('headquarters_sign', btn.headquarters_sign); // 获取签署付款确认书
        this.localStorageService.setCacheValue('update_qrs_contract', btn.update_qrs_contract); // 回传付款确认书
        this.localStorageService.setCacheValue('selected', this.selectedItems);
        this.xn.router.navigate(['/xn-gemdale/gemdale-supports/confirmation']);
    }
     ...
     * @param 
     */
    private confirmReceivable() {
        this.xn.router.navigate(['/xn-gemdale/record/new'], {
            queryParams: {
                id: 'sub_project_confirmation',
                relateValue: this.selectedItems.map(main => main.mainFlowId)
            }
        });
    }
