...
import { ChangeDetectorRef, Component, OnInit, ViewChild, ViewContainerRef, AfterViewInit, OnDestroy } from '@angular/core';
...
export class RecepitListComponent implements OnInit, AfterViewInit, OnDestroy {
  tabConfig: any;
  ...
  public handleRowClick(item, btn, i: number) {
    switch (btn.operate) {
      // 查看
      case 'view_detail':
        this.xn.router.navigate([`xn-gemdale/main-list/detail/${item.mainFlowId}`]);
        break;
    }
  }
  ...
  private signReceipt(btn) {
    ...
    this.xn.dragon.post('/sub_system/jd_system/project_sign', { mainFlowIdList: this.selectedItems.map((x: any) => x.mainFlowId) })
      .subscribe(x => {
        ...
        const templateFlag2 = x.data?.templateFlag2 || '';
        ...
        XnModalUtils.openInViewContainer(this.xn, this.vcr, DragonFinancingContractModalComponent, contracts)
          .subscribe(v => {
            if (v === 'ok') {
              ...
              let templateFlagList = templateFlag2 !== '' ? [templateFlag, templateFlag2] : templateFlag;
              console.log('进来了', templateFlagList.constructor);
              if (templateFlagList.constructor === Array) {
                const uploadContract = templateFlagList.map(x => {
                  // 回传合同
                  data.templateFlag = x;
                  return this.xn.dragon.post('/contract/second_contract_info/update_second_contract', data)
                })
                forkJoin(uploadContract).subscribe(() => {
                  this.onPage({ page: this.paging });
                })
              } else {
                // 回传合同
                this.xn.dragon.post('/contract/second_contract_info/update_second_contract', data).subscribe(() => {
                  this.onPage({ page: this.paging });
                });
              }
  ...
  onScroll($event: any) {
    ...
    if ($event.srcElement.scrollLeft !== this.scrollX) {
      ...
      const lastHeadX = -($event.srcElement.scrollWidth - $event.srcElement.offsetWidth) + $event.srcElement.scrollLeft - 10;
