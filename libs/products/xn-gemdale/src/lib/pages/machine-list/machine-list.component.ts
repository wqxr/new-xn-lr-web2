...
export class MachineListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // 自定义筛选条件status传值范围定义
  public customSearchNumber: number = CustomSearchNumber['XN_GEMDALE']
  ...
  */
  ...
  /**
   * 进入资产池
   * @param paramsValue 交易id
   */
  enterCapitalPool(paramsValue: string) {
    const params = {
    ...
    this.xn.dragon.post('/trade/get_mainflowid_pool_info', params).subscribe(x => {
      if (x.ret === 0 && x.data.capitalPoolId !== '') {
        this.xn.router.navigate(['/xn-gemdale/assets-management/capital-pool'], {
          queryParams: {
            title: '项目管理-金地-香纳>' + (x.data.type === 1 ? 'ABS储架项目' : '再保理银行项目') + '>'
  ...
  */
  ...
  handmore(item: any) {
    ...
    if (item.nowProcedureId === '@end') {
      this.xn.router.navigate([`/xn-gemdale/record/todo/view//${item.recordId}`]);
    }
    ...
    if (!isOk) {
      ...
      if (item.nowProcedureId === 'operate' || item.nowProcedureId === 'review') {
        if (item.flowId === 'jd_platform_verify' && item.zhongdengStatus === 1) {
          ...
          this.xn.router.navigate([`/xn-gemdale/record/todo/edit/${item.recordId}`]);
        }
      } else {
        this.xn.router.navigate([`/xn-gemdale/record/todo/view//${item.recordId}`]);
      }
