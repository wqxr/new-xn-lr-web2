...
import RepaymentConfigList from './xn-gemdale-repayment';
...
@Component({
  templateUrl: `./xn-gemdale-repayment.component.html`,
  styles: [
    `
      [nz-button] {
        margin-right: 8px;
...
export class XnGemdaleRepaymentComponent implements OnInit {
  ...
  public onPage(
    ...
    console.log('parmas.pageNo', params.pageNo);
    this.xn.dragon.post2(this.currentTab.post_url, params).subscribe(
      (x) => {
        this.loading = false;
        if (x.ret === 0) {
  ...
  /**
   * 构建列表请求参数
   * @param searchModel 搜索集合对象
   */
  private buildParams(searchModel: { [key: string]: any }) {
    const params: any = {
      pageNo: (this.paging - 1) * this.pageConfig.pageSize,
