...
import { XnGemdaleSingleSearchListModalComponent } from '../../share/modal/single-searchList-modal.component';
import { XnGemdalegetCustomListComponent } from '../../share/modal/custom-list-modal.component';
import { XnGemdalegetCustomFiledComponent } from '../../share/modal/custom-field-modal.component';
...
declare const $: any;
@Component({
  // selector: 'xn-gemdale-data-docking',
  templateUrl: `./xn-gemdale-data-docking.component.html`,
  styles: [`
    .item-box {
        position: relative;
        display: flex;
...
export class XnGemdaleDatalockingComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public initData(paramTabValue: string) {
    this.defaultValue = paramTabValue || 'A';
  ...
  /**
   *  子标签tab切换，加载列表
   * @param  paramSubTabValue paramSubTabValue
   */
  ...
  public handleHeadClick(paramBtnOperate: ButtonConfigModel) {
    if (paramBtnOperate.operate === 'custom_search') {
      ...
      XnModalUtils.openInViewContainer(this.xn, this.vcr, XnGemdalegetCustomFiledComponent, params).subscribe(v => {
        if (v && v.action === 'ok') {
      ...
      XnModalUtils.openInViewContainer(this.xn, this.vcr, XnGemdalegetCustomListComponent, params).subscribe(v => {
        if (v && v.action === 'ok') {
  ...
  newGemdaleFinancingPre() {
    ...
    this.xn.router.navigate([`/xn-gemdale/record/new/jd_financing_pre/${HeadquartersTypeEnum[2]}`],
      {
        queryParams: {
          billNumberList,
        }
    ...
    XnModalUtils.openInViewContainer(this.xn, this.vcr, EditModalComponent, params)
      .subscribe((v) => {
        if (v === null) {
          ...
          const data = { ...v, billNumberList };
          this.xn.dragon.post('/sub_system/jd_system/jd_batch_modify', data).subscribe(x => {
            if (x.ret === 0) {
