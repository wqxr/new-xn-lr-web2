...
@Component({
    templateUrl: './xn-gemdale-record.component.html',
    styleUrls: ['./xn-gemdale-record.component.css']
})
export class XnGemdaleRecordComponent implements OnInit, AfterViewInit {
    ...
    doProcess(record: any) {
        ...
        if ((record.status !== 1 && record.status !== 0)
            ...
            this.xn.router.navigate([`/xn-gemdale/record/${this.flowId1}/view/${record.recordId}`]);
        } else {
            this.xn.router.navigate([`/xn-gemdale/record/${this.flowId1}/edit/${record.recordId}`]);
        }
    }
    ...
    viewZdList(item) {
        ...
        XnModalUtils.openInViewContainer(this.xn, this.vcr, XnGemdaleSingleSearchListModalComponent, params).subscribe(() => {
