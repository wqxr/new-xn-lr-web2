 ...
 */
@Component({
  selector: 'xn-gemdale-home-comm-list-component',
  templateUrl: './home-comm-list.component.html',
  styleUrls: [
    './home-comm-list.component.css'
  ]
})
export class XnGemdaleHomeCommListComponent extends CommonPage implements OnInit {
  @Input() supers: any;
  total = 0;
  rows: any[] = [];
  words = '';
  ...
  public viewProcess(paramMainFlowId: string) {
    this.xn.router.navigate([`xn-gemdale/main-list/detail/${paramMainFlowId}`]);
  }
  ...
  public doProcess(record: any) {
    record.proxyType = record.proxyType === ProxyType.AVENGER ? ProxyType.AVENGER : ProxyType.XN_GEMDALE;
