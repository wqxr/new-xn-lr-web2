...
import { JdDataChangeModalComponent } from '../../modal/jd-dataChange-modal.component';
import { JdRejectModalComponent } from '../../modal/reject-modal.component';
...
export class XnGemdaleEditComponent implements AfterViewInit, AfterViewChecked, OnInit, OnDestroy {
  ...
  public showIconJdChanges = false;
  ...
  ngOnDestroy(): void {
   ...
   // 上一步的展示与隐藏
   showFlow(paramAction){
    return (paramAction['collapse'] && paramAction['collapse'] === true) || paramAction.flowId==='sub_project_confirmation';
  }
  ...
  private showSubmit(recordId, fresh?) {  // fresh 为可选，true的时候则刷新，不拉取actions的最后一项，拉checker
    this.xn.dragon.post('/flow/showSubmit', {
      ...
      if (this.showJdChangesIcon() && this.svrConfig.procedure.procedureId !== 'review') {
        this.startSetInterval(5000, false);
      }
      this.showIconReject = (this.showRejectIcon() && this.showIconReject);
      this.showIconJdChanges = (this.showJdChangesIcon() && this.showIconJdChanges);
  ...
  // 是否显示退回原因图标
  showRejectIcon() {
    return ((this.svrConfig.record.flowId === 'jd_financing' || this.svrConfig.record.flowId === 'jd_platform_verify')
      && this.svrConfig.procedure.procedureId === 'operate');
  }

  // 是否显示数据变动图标
  showJdChangesIcon() {
    return this.svrConfig.record.flowId === 'jd_platform_verify' && this.svrConfig.wkType && this.svrConfig.wkType === 1;
  }
  ...
  public onSubmit() {
    this.innerSubmit(() => {
      for (const checker of this.svrConfig.checkers) {
        ...
        // 金地-发起提单 渠道处理
        if (checker.checkerId === 'productType' && this.flowId === 'jd_financing_pre') {
          const { proxy, status } = JSON.parse(this.mainForm.value[checker.checkerId]) || {};
          const { type, selectBank } = JSON.parse(this.mainForm.value[checker.checkerId]) || {};
          const productType = { type: type ? type : proxy, selectBank: selectBank ? selectBank : status };
  ...
  public otherJudge(row): boolean {
    ...
    let flag = false;
    if (row.checkerId === 'dealContract' && row.flowId === 'jd_platform_verify') {
      flag = JSON.parse(rowVal)
        .some(x => !(x.contractName && x.contractId && x.contractMoney && x.payRate && x.contractType && x.contractJia && x.contractYi));
      return flag;
    } else if (row.checkerId === 'dealContract' && row.flowId === 'jd_financing') { // 供应商上传资料-交易合同必填
      return !rowVal;
    } else if (row.checkerId === 'performanceFile' && row.flowId === 'jd_platform_verify') {
      flag = JSON.parse(rowVal).some(x => !(x.percentOutputValue && x.payType));
      return flag;
    } else if (row.checkerId === 'performanceFile' && row.flowId === 'jd_financing') { // 供应商上传资料-履约证明必填
      return !rowVal;
  ...
  flowClick() {
    ...
    this.xn.router.navigate([`/xn-gemdale/record/new/`],
      {
        queryParams: {
          id: 'sub_jd_change',
          relate: 'mainFlowId',
  ...
  // 显示暂存按钮
  showTempClick() {
    return (this.svrConfig.flow.flowId === 'jd_platform_verify' && this.svrConfig.record.nowProcedureId === 'operate')
  ...
  private copyDraft(shows: any[], hides: any[], fresh: boolean): void {
    ...
    // memo字段赋值-金地数据对接 平台审核
    if (['jd_platform_verify'].includes(this.svrConfig.flow.flowId)
   ...
   * 显示金地数据变动
   */
  private showJdChangesInfo(init: boolean) {
    this.xn.dragon.post2('/sub_system/jd_system/get_jd_change_status', {
      mainFlowId: this.svrConfig.record.mainFlowId
    }).subscribe(x => {
      if (x.ret !== 0) {
        clearInterval(this.changesInterval);
      ...
      if (x.ret === 0 && x.data && x.data.isChange) {
        this.showIconJdChanges = true;
      } else {
        this.showIconJdChanges = false;
      }
    });
  }
  ...
  rejectBtnShow(flowId: string): boolean {
    const notShowFlow = ['jd_financing', 'jd_platform_verify', 'jd_factoring_risk']; // 金地主流程 隐藏中止按钮
  ...
  openrejectplatWindow() {
    ...
    XnModalUtils.openInViewContainer(this.xn, this.vcr, JdRejectModalComponent, detail).subscribe(v => {
      if (v.action === 'cancel') {
        return;
      }
