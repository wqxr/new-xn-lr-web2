...
export class NewComponent implements OnInit, AfterViewInit {
    ...
    private doShow() {
        ...
        if (this.flowId === 'jd_financing_pre') { // 发起提单
            ...
            postObj.factorName = '深圳市香纳商业保理有限公司';
        }
        ...
        postObj.factoringAppId = `${applyFactoringTtype['深圳市香纳商业保理有限公司']}`;
    ...
    private request(method: string) {
        ...
        this.xn.api.dragon.post(`/flow/${method}`, params).subscribe(json => {
            ...
            this.xn.router.navigate([`/xn-gemdale/record/record/${this.flowId}`]);
        });
    }
