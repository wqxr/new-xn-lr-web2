...
@Component({
    selector: 'xn-gemdale-business-related',
    templateUrl: './xn-gemdale-business-related.component.html',
    styles: [
        `
            .table {
                font-size: 13px;
...
export class XnGemdaleBusinessComponent implements OnInit {
    ...
    viewRecord(record: string, type: string) {
        if (type && type === 'avenger') {
            ...
            this.xn.router.navigate([`/xn-gemdale/record/todo/view/${record}`]);
        }
    }
