...
export class VankeAvengerListComponent implements OnInit {
    ...
    */
    public initData(paramTabValue: string, init?: boolean,) {
        ...
        if (this.tabConfig.title.includes('审批放款-万科') && (this.defaultValue === 'A' || this.defaultValue === 'C')) {
        ...
        if (this.defaultValue === 'C' && this.tabConfig.title.includes('审批放款-万科') && !!!init) {
            this.pushApproval(1);
        } else if (this.defaultValue === 'A' && this.tabConfig.title.includes('审批放款-万科')) {
    ...
    public onPage(e?: { page: number, first?: number, pageSize?: number, pageCount?: number }, types?: number) {
        if (this.tabConfig.title.includes('审批放款-万科') && this.defaultValue === 'A') {
    ...
    public singleChecked(paramItem, index) {
        ...
        if (paramItem.checked && paramItem.checked === true) {
            ...
            this.selectedPayableAmounts = Number(
                (this.selectedPayableAmounts - (paramItem.changePrice ? paramItem.changePrice : 0)).toFixed(2));
        } else {
...
const signText = [
    ...
    { label: '应收账款转让协议书-万科香纳工行再保理', text: '乙方（电子签章、数字签名）' },
    { label: '应收账款转让登记协议-万科香纳工行再保理', text: '乙方（电子签章、数字签名）' },
    { label: '国内无追索权商业保理合同-万科香纳工行再保理', text: '乙方（保理商、受让人）数字签名' }
];
