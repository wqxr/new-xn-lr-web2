...
export class VankeCapitalpoolComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    */
    public downloadSelectedAttach() {
        ...
        const params = { hasSelect, selectedCompany: this.defaultHeadquarter, fileList: this.downLoadList, capitalType: CapitalType.New };
    ...
    */
    ...
    navBack() {
        if (this.isMachineenter === true) {
            ...
            this.xn.router.navigate(['/vanke/assets-management/projectPlan-management'], {
                queryParams: {
  ...
  syncConfirmation() {
    ...
    */
        ...
        this.xn.api.dragon.post(urls.generate, params).subscribe(con => {
            ...
            if (result.length) {
                ...
                result.forEach(x => {
                    ...
                    if (x.label === '国内再保理业务目标应收账款债权转让通知书-万科香纳工行再保理') {
                        x.config.text = '（盖章）';
                    } else {
                        x.config.text = '（盖章）';
                    }
    ...
    public batchModify() {
        ...
        this.selectedItems.map((sel) => sel.headquarters = this.headquarters === HeadquartersTypeEnum[1] ? '万科' : this.headquarters);
                ...
                if (x.data.isCapitalPoolSample) {
                    ...
                    XnModalUtils.openInViewContainer(this.xn, this.vcr, EditModalComponent, params).subscribe(v => {
                        if (v === null) {
                            ...
                            // 抽样页面
                            this.xn.router.navigate(['/vanke/assets-management/capital-sample'], {
                                queryParams: {
