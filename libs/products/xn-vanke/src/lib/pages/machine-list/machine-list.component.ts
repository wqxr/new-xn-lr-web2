...
import MachineIndexTabConfig, { ApiProxyEnum, MachineTypeEnum, SubTabEnum } from './machine-list.config';
...
export class DemoListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // 自定义筛选条件status传值范围定义
  public customSearchNumber: number = CustomSearchNumber['XN_VANKE']
  ...
  */
    ...
    this.xn.dragon.post('/trade/get_mainflowid_pool_info', params).subscribe(x => {
      if (x.ret === 0 && x.data.capitalPoolId !== '') {
        this.xn.router.navigate(['/vanke/assets-management/capital-pool'], {
          queryParams: {
            title: '项目管理(万科)>' + (x.data.type === 1 ? 'ABS储架项目' : '再保理银行项目') + '>'
  ...
  public viewMore(paramItem, mainFlowId: string, isProxy: any) {
    if (isProxy === 53) {
      ...
      this.xn.api.post('/llz/risk_warning/invoice_management/invoice_all', { mainFlowId }).subscribe(x => {
        // 打开弹框
        const params = {
          get_url: '',
          get_type: '',
  ...
  downLoadFile(paramBtnOperate: ButtonConfigModel): void {
    ...
    XnModalUtils.openInViewContainer(this.xn, this.vcr, DownloadAttachmentsModalComponent, params).subscribe(data => {
      if (data !== '') {
        this.xn.loading.open();
        this.newSelectItems = this.selectedItems.filter((x: any) => x.isProxy === IsProxyDef.VANKE_ABS);
  ...
  subZhongdengRegister(paramBtnOperate: ButtonConfigModel) {
    ...
    this.newSelectItems = this.selectedItems.filter((x: any) => x.isProxy === 50 || x.isProxy === 52 || x.isProxy === 53); // 新业务的退单流程
  ...
  public batchModify(paramBtnOperate: ButtonConfigModel) {
    ...
    if (this.oldSelectItems.length > 0) {
      this.xn.msgBox.open(false, [
        ...
        ...this.oldSelectItems.map(o => o.mainFlowId),
        '不能进行"补充信息"操作', () => {
          if (this.oldSelectItems.length === this.selectedItems.length) { return }
        }
      ]);
    }
    this.newSelectItems = this.selectedItems.filter((x: any) => x.isProxy === 50 || x.isProxy === 52 || x.isProxy === 53); // 新业务的退单流程
    const param = { mainList: this.newSelectItems };
    this.newSelectItems.map((sel) => sel.headquarters = sel.headquarters === HeadquartersTypeEnum[1] ? '万科' : sel.headquarters);
  ...
  */
  editHelpHandler(paramBtnOperate: ButtonConfigModel) {
    ...
    this.newSelectItems = this.selectedItems.filter((x: any) => x.isProxy === 50 || x.isProxy === 52 || x.isProxy === 53); // 新业务
  ...
  */
  ...
  rejectProgram(paramBtnOperate: ButtonConfigModel) {
    ...
    } else if (pauseItems.length > 0) { // 【平台审核-暂停】
      this.xn.msgBox.open(false, [
    ...
    this.xn.router.navigate([`/xnvanke/record/new/`],
      {
        queryParams: {
  ...
  private buildCondition(searches) {
    ...
    this.shows = XnUtils.deepClone(objList)  // 深拷贝;
    ...
    this.searchForm.valueChanges.subscribe((v) => {
      ...
      // FIXME: 替换$.extned 方法改用工具类提供的 deepClone
      // $.extend(true, {}, arrObj);
      ...
      for (const search of this.searches) {
        ...
        if (!XnUtils.isEmpty(this.arrObjs[search.checkerId])) {
          if (search.checkerId === 'transactionStatus') {
            ...
            params.flowId = obj.tradeStatus === '0' || obj.tradeStatus === '1' || obj.tradeStatus === '2' ? 'vanke_platform_verify' : obj.tradeStatus;
          ...
          } else if (search.checkerId === 'ccsAduitDatetime') { // 一线审核时间
          ...
          } else if (search.checkerId === 'ccsApproveTime') {  // 一线审批时间
          ...
          } else if (search.checkerId === 'ccsZauditDate') {// 资金中心审核时间
  ...
  exportFile(paramItem: any) {
    ...
    XnModalUtils.openInViewContainer(
      ...
      this.xn.loading.open();
      this.newSelectItems = this.selectedItems.filter((x: any) => x.isProxy === 50 || x.isProxy === 52 || x.isProxy === 53);
      const newParam = {
  ...
  handmore(item: any) {
    ...
    if (item.nowProcedureId === '@end') {
      if (item.mainFlowId.endsWith('wk')) {
        this.xn.router.navigate([`/logan/record/todo/view/${item.recordId}`]);
      } else {
      ...
      if (item.nowProcedureId === 'operate' || item.nowProcedureId === 'review') {
        if (item.mainFlowId.endsWith('wk')) {
          if (item.flowId === 'dragon_platform_verify' && item.zhongdengStatus === 1) {
            ...
            this.xn.router.navigate([`/logan/record/todo/edit/${item.recordId}`]);
          }
        } else {
...
export const FlowIdObj = {
  DOING: ['vanke_financing_pre', 'vanke_financing', 'vanke_platform_verify', 'vanke_factoring_risk'],
  TODO: ['vanke_financing_sign', 'vanke_factoring_passback', 'wait_verification_500', 'verificating_500', 'factoring_sign_500', 'wait_loan_500'],
  DONE: ['loaded_500'],
  SPECIAL: [],
  stop: ['99', '100']
};
