        ...
        if (svrConfig.procedure.procedureId === 'operate') {
            ...
            try {
                if (contractFile === '') {
                    ...
                    // let others = JSON.parse(svrConfig.checkers.filter((x: any) => x.checkerId === 'dealContract')[0].other);
                    // others.show = true;
                    // svrConfig.checkers.filter((x: any) => x.checkerId === 'dealContract')[0].other = JSON.stringify(others);
                } else {

                }
                if (certificateFile === '') {
                    alert.push(`${alert.length + 1}、资质证明文件没上传`);
                    // let others = JSON.parse(svrConfig.checkers.filter((x: any) => x.checkerId === 'certificateFile')[0].other);
                    // others.show = true;
                    // svrConfig.checkers.filter((x: any) => x.checkerId === 'certificateFile')[0].other = JSON.stringify(others);
                } else {
                ...
                if (invoiceFile === '') {
                    ...
                    // let others = JSON.parse(svrConfig.checkers.filter((x: any) => x.checkerId === 'invoice')[0].other);
                    // others.show = true;
                    // svrConfig.checkers.filter((x: any) => x.checkerId === 'invoice')[0].other = JSON.stringify(others);
                }
