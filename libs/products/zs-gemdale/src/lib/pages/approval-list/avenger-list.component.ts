 ...
 * All rights reserved.
 *
 * @file：ZsGemdaleAvengerListComponent
 * @summary：金地审批放款列表
 * @version: 1.0
 * **********************************************************************
...
export class ZsGemdaleAvengerListComponent implements OnInit {
   ...
   */
  viewProcess(mainFlowId: string) {
    this.xn.router.navigate([`/zs-gemdale/main-list/detail/${mainFlowId}`])
  }
  ...
  */
  public initData(paramTabValue: string, init?: boolean,) {
    ...
    if (this.tabConfig.title.includes('审批放款-金地-前海中晟') && (this.defaultValue === 'A' || this.defaultValue === 'C')) {
      this.xn.api.post('/tool/is_jindie', {}).subscribe(x => {
        if (x.data) {
          this.is_jindie = x.data.isOpen;
          if (this.is_jindie === 1) {
            this.tabConfig.title = '审批放款-金地-前海中晟(已开启金蝶云审批)';
          } else {
            this.tabConfig.title = '审批放款-金地-前海中晟(线下人工审批)';
          }
        }
      });
    }
    this.subDefaultValue = 'DOING'; // 重置子标签默认
    if (this.defaultValue === 'C' && this.tabConfig.title.includes('审批放款-金地-前海中晟') && !!!init) {
      this.pushApproval(1);
    } else if (this.defaultValue === 'A' && this.tabConfig.title.includes('审批放款-金地-前海中晟')) {
  ...
  public onPage(e?: { page: number, first?: number, pageSize?: number, pageCount?: number }, types?: number) {
    if (this.tabConfig.title.includes('审批放款-金地-前海中晟') && this.defaultValue === 'A') {
  ...
  watchApproval(type?: number) {
    ...
    this.xn.avenger.post2('/jd_blh/approval_progress', {
    }).subscribe(data => {
      if (data.ret !== 0) {
        if (this.subDefaultValue === 'SPECIAL') {
          this.cancelSpecial(this.subDefaultValue);
  ...
  pushApproval(type?: number) {
    const that = this;
    this.xn.avenger.post2('/jd_blh/push_progress', {
    }).subscribe(data => {
      if (data.ret !== 0) {
  ...
  public showContract(paramContractInfo, type: string, proxy: number) {
    if (type !== 'receivetable') {
      this.viewModal = DragonPdfSignModalComponent;
    } else if (type === 'receivetable') {
  ...
  public handleHeadClick(paramBtnOperate: ButtonConfigModel) {
    if (paramBtnOperate.operate !== 'download_approval_list' && !this.selectedItems.length) {
      return this.xn.msgBox.open(false, '请选择交易');
    }
    if(['approval_ok','approval_reject','finance_approval_ok','finance_approval_fail','loan_ok','loan_fail'].includes(paramBtnOperate.operate) && this.xn.user.roles.indexOf('admin')<0 ){
      this.xn.msgBox.open(false,'非管理员不能操作,请联系管理员');
      return;
  }
    switch (paramBtnOperate.operate) {
      // 发起审批
      case 'initiate_examination':
        this.initiateExamination(paramBtnOperate)
        break;
      // 审批成功
      case 'approval_ok':
        this.approvalOk(paramBtnOperate)
        break;
      // 审批拒绝
      case 'approval_reject':
        this.approvalReject(paramBtnOperate)
        break;
      // 批量签署合同
      case 'sign_contract':
      ...
      // 放款成功
      case 'loan_ok':
        this.loanOk(paramBtnOperate)
        break;
      // 放款失败
      case 'loan_fail':
        this.loanFail(paramBtnOperate)
        break;
      // 重新审批
      case 'approval_again':
        this.approvalAgain(paramBtnOperate)
        break;
      // 导出清单
      case 'download_approval_list':
        this.downloadApprovalList(paramBtnOperate);
        break;
      // 会计下载表格
      case 'accountDownload':
        this.accountDownload(paramBtnOperate);
        break;
      // 出纳下载表格
      case 'loadFinancing':
        this.loadFinancing(paramBtnOperate);
        break;
      // 推送数据
      case 'finish_sign_contract':
        this.finishSignContract();
        break;
      // 完成放款
      case 'loan_done':
        this.loanDone(paramBtnOperate);
        break;
      // 金地放款反馈
      case 'loan_back':
        this.loanBack(paramBtnOperate);
        break;
      case 'person_return_upload':
        this.getPersonreturnList(paramBtnOperate.operate);
        break;
      case 'system_loan_receipt':
        this.getPersonreturnList(paramBtnOperate.operate);
        break;
      case 'finance_approval_ok':
        this.financeApprovalOk(paramBtnOperate);
        break;
      case 'finance_approval_fail':
        this.financeApprovalOk(paramBtnOperate);
        break;
      default:
        break;
  ...
  /**
   * 发起审批
   * @param paramBtnOperate 表头按钮参数配置
   */
  initiateExamination(paramBtnOperate: ButtonConfigModel) {
  ...
  financeApprovalOk(paramBtnOperate: ButtonConfigModel) {
    const type = paramBtnOperate.operate === 'finance_approval_ok' ? ApprovalType.SUCCESS : ApprovalType.FAIL;
   ...
   * @param paramBtnOperate 表头按钮参数配置
   */
  approvalOk(paramBtnOperate: ButtonConfigModel) {
   ...
   * @param paramBtnOperate 表头按钮参数配置
   */
  approvalReject(paramBtnOperate: ButtonConfigModel) {
    const mainflowIdlist = [];
    this.selectedItems.map(item => {
      item.creditStatus = 2;
   ...
   * @param paramBtnOperate 表头按钮参数配置
   */
  loanOk(paramBtnOperate: ButtonConfigModel) {
   ...
   * @param paramBtnOperate 表头按钮参数配置
   */
  loanFail(paramBtnOperate: ButtonConfigModel) {
    this.xn[this.apiProxyEnum[this.defaultValue]].post(paramBtnOperate.post_url,
   ...
   * @param paramBtnOperate 表头按钮参数配置
   */
  approvalAgain(paramBtnOperate: ButtonConfigModel) {
   ...
   * @param paramBtnOperate 表头按钮参数配置
   */
  downloadApprovalList(paramBtnOperate: ButtonConfigModel) {
    ...
    XnModalUtils.openInViewContainer(
      ...
      this.xn.api.download(paramBtnOperate.post_url,
        param)
        .subscribe((con: any) => {
   ...
   * @param paramBtnOperate 表头按钮参数配置
   */
  accountDownload(paramBtnOperate: ButtonConfigModel) {
   ...
   * @param paramBtnOperate 表头按钮参数配置
   */
  loadFinancing(paramBtnOperate: ButtonConfigModel) {
  ...
  approvalLimit() {
    ...
    XnModalUtils.openInViewContainer(this.xn, this.vcr, EditModalComponent, params).subscribe(v => {
      if (v === null) {
        ...
        const list = this.selectedItems.map((x: any) => x.mainFlowId);

        this.xn.avenger.post('/jd_blh/jd_finish_sign', {
          approvalMemo: this.approvalMemo,
          valueDate: this.valueDate, mainFlowIds: list
        }).subscribe(x => {
          if (x.ret === 0) {
   ...
   * @param paramBtnOperate 表头按钮参数配置
   */
  loanDone(paramBtnOperate: ButtonConfigModel) {
    this.xn.loading.open()
...
* @param paramBtnOperate 表头按钮参数配置
*/
  loanBack(paramBtnOperate: ButtonConfigModel) {
    this.xn.loading.open()
    this.xn.dragon.post(paramBtnOperate.post_url,
      { mainFlowIdList: this.selectedItems.map(x => x.mainFlowId) })
      .subscribe(v => {
        this.xn.loading.close()
        if (v.ret === 0) {
          this.xn.msgBox.open(false, '反馈成功');
          this.initData('E', true);
        }
      }, () => {
        this.xn.loading.close();
 ...
 */
    ...
    this.xn[proxyType].post(btn.post_url, { mainFlowIds })
      .subscribe(con => {
        ...
        cons.forEach(x => {
          ...
          } else if (x.label === '国内商业保理合同（线上线下通用版）') {
  ...
  openFrees(Paramitem: any) {
    this.xn.avenger.post('/jd_blh/fee_info', { mainFlowId: Paramitem.mainFlowId }).subscribe(x => {
      if (x.ret === 0) {
