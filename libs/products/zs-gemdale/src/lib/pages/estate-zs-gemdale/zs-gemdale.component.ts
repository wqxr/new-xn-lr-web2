...
@Component({
  templateUrl: './zs-gemdale.component.html',
  styles: [
    `.table {
            font-size: 13px;
        }`,
...
export class EstateZsGemdaleComponent implements OnInit {
  public ZsGemdaleTodo: any; // 新金地任务列表
  public count_jd_zs: number; // 金地前海中晟待办数量
  ...
  ngOnInit() {
    ...
    this.route.data.subscribe((config: any) => {
      this.ZsGemdaleTodo = config.ZsGemdaleTodo;
      this.ZsGemdaleTodo.productIdent = this.productIdent;
      // isPerson是否个人待办标识
      this.ZsGemdaleTodo.isPerson = config.isPerson;
      ...
      this.xn.dragon.post('/list/todo_record/todo_count',
        ...
        if (this.productIdent) {
          this.count_jd_zs = config.isPerson ? json.data[this.productIdent] : this.count_jd_zs
        }
