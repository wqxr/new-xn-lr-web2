...
export class RecepitListComponent implements OnInit {
    ...
    public handleRowClick(item, btn, i: number) {
        switch (btn.operate) {
            // 查看
            case 'view_detail':
                this.xn.router.navigate([`zs-gemdale/main-list/detail/${item.mainFlowId}`]);
                break;
        }
    }
    ...
    */
    onScroll($event: any) {
        ...
        if ($event.srcElement.scrollLeft !== this.scroll_x) {
            ...
            const lastHead_X = -($event.srcElement.scrollWidth - $event.srcElement.offsetWidth) + $event.srcElement.scrollLeft - 10;
