...
export class ZsGemdaleEditComponent implements AfterViewInit, AfterViewChecked, OnInit, OnDestroy {
  ...
  public otherJudge(row): boolean {
    ...
    if (row.checkerId === 'dealContract' && row.flowId === 'jd_platform_verify') {
      flag = JSON.parse(rowVal).some(x =>
        !(x.contractName && x.contractId && x.contractMoney && x.payRate && x.contractType && x.contractJia && x.contractYi)
      );
      return flag;
  ...
  flowClick() {
    ...
    this.xn.router.navigate([`/zs-gemdale/record/new/`],
      {
        queryParams: {
          id: 'sub_jd_change',
          relate: 'mainFlowId',
