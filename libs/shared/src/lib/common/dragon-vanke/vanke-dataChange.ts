...
export default class VankeDataChangeList {
    ...
    platmachineAccount: {
      ...
      tabList: [
        ...
        },
        {
          ...
          post_url: '/sub_system/vanke_system/get_vanke_history',
          params: 2,
        },
      ],
    } as TabConfigModel,
    ...
    allFiledArr.forEach((val, index, arr) => {
      if (resData.lastList) {
        const item = {
          ...
          fieldName: val, // 字段标识
          isChanged: this.filedChanged(resData, val, heads), // 是否有变动，标红
   ...
   */
  static filedChanged(res: any, val: string, heads: any): object {
    const changedObj: any = {};
    const lastList = res.lastList;
    if (lastList.length > 0) {
      lastList.forEach((item, index) => {
        const curVal = item?.[val];
        const nextVal = lastList[index + 1]?.[val];
        // 当前业务数据未返回 则不进行比对
        if (index < lastList.length - 1 && curVal !== undefined) {
          // console.log('inside', curVal, nextVal, heads[index].value)
          // lodash.isEqual() 方法支持比较多类型相等
          if (_.isObject(curVal)) {
            if (index === 0) {
              changedObj.currentField = [];
              curVal
                .filter((i) => i.isRed === true)
                .map((subItem) => {
                  changedObj.currentField.push(subItem.number);
                });
            } else {
              changedObj[heads[index].value] = [];
              curVal
                .filter((i) => i.isRed === true)
                .map((subItem) => {
                  changedObj[heads[index].value].push(subItem.number);
                });
            }
          } else if (!_.isEqual(curVal, nextVal)) {
            // 有变动
            if (index === 0) {
              changedObj.currentField = [];
              changedObj.currentField.push(heads[0].value);
            } else {
              changedObj[heads[index].value] = [];
              changedObj[heads[index].value].push(
                heads[index - 1].value,
                heads[index].value
              );
            }
...
export const optionalFiledArr = [
  ...
  'isSignFlag', // 付款确认书电子章
  'isSignBuyer', // 买方确认函电子章
  'giveFileTime', // 付款确认书出具时间
  'signBuyer', // 确认函获取标识
  'hasDownBuyer', // 买方确认函是否下载
  'hasDownConfirm', // 付款确认书是否下载
  'isUpdated', // 修改标识
  'contractName', // 合同名称
  'contractNumberReal', // 合同编号
  'isAuth', // 授权标识
];
...
export const filedLabelArr = [
  ...
  { label: '合同归档号', value: 'contractNumber' }, // 合同归档号
  ...
  { label: '资产转让折扣率', value: 'discountRate' }, // 资产转让折扣率
  ...
  { label: '付款确认书电子章', value: 'isSignFlag' }, // 0=未签 1=已签
  { label: '买方确认函电子章', value: 'isSignBuyer' }, // 0=未签 1=已签
  { label: '付款确认书出具时间', value: 'giveFileTime' },
  { label: '确认函获取标识', value: 'signBuyer' }, // 0=纸质 1=电子
  { label: '买方确认函是否下载', value: 'hasDownBuyer' }, //  0=未下载 1=已下载
  { label: '付款确认书是否下载', value: 'hasDownConfirm' }, //  0=未下载 1=已下载
  { label: '修改标识', value: 'isUpdated' }, //  0=未修改 1=已修改
  { label: '合同名称', value: 'contractName' },
  { label: '合同编号', value: 'contractNumberReal' },
  { label: '授权标识', value: 'isAuth' }, // 0=未授权 1=已授权
];
