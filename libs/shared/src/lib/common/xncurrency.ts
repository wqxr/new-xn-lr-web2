...
export class XNCurrency {
    /** 货币整数值，单位分 */
    private m_IntValue = 0;
    /** 是否有错误 */
    private m_ErrFlag = false;
    /** 错误信息 */
    private m_ErrMsg   = '';
    /**
     * 构造函数
    ...
    public constructor(paramValue: number | string | XNCurrency = 0) {
        if (XNCurrency.isCurrency(paramValue)) {
            ...
            const v = Parse(paramValue as number | string);
            if (v.errFlag) {
                this.m_IntValue = 0;
                this.m_ErrFlag  = true;
    ...
    public add(paramNumber: number | string | XNCurrency): XNCurrency {
        const r = new XNCurrency(paramNumber);
    ...
    public selfMul(paramNumber: number | string): boolean {
        ...
        const r = new XNCurrency(v.value * this.value);
        if (r.isErr) {
            return false;
        } else {
            this.assign(r);
    ...
    public selfDiv(paramNumber: number | string): boolean {
        ...
        const r = new XNCurrency(this.value / v.value);
        if (r.isErr) {
            return false;
        } else {
            this.assign(r);
