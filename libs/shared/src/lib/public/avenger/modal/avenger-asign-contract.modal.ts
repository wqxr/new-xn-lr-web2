...
export class AvengerFinancingContractModalComponent implements OnInit {
  ...
  open(paramContracts: any): Observable<string> {
    ...
    let paramCFCAList = paramContracts.filter(
      (x) => x.caSignType === SignTypeEnum['cfcaSignType']
    );
    let paramCAList = paramContracts.filter(
  ...
  private cfcaSign(paramContract: any) {
    ...
    this.xn.avenger.post2('/cfca/try_sign_contract', params).subscribe((x) => {
      if (x.data.checkSms) {
