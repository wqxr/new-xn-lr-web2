...
export class CommListComponent extends CommonPage implements OnInit {
  ...
  ngOnInit() {
    if (!!this.superConfig) {
      ...
      this.pageSize =
        (this.superConfig.list && this.superConfig.list.pageSize) ||
        this.pageSize;
      setTimeout(() => {
  ...
  onBtnClick(btn, row): void {
    // showRegisterCompany 注册公司列表配置标识
    const showRegisterCompany = this.base?.superConfig?.showRegisterCompany;
    if (showRegisterCompany) {
      // 权限系统企业 同步/注册按钮请求
      const http$ = btn.click(this.base, row, this.xn, this.vcr);
  ...
  downAll() {
    const flowIds$ = this.rows.map((x) =>
      this.xn.api.post('/flow/main/detail', { mainFlowId: x.mainFlowId })
    );
  ...
  customClick(btn) {
    ...
    if (
      ...
      params.where._complex.appId &&
      params.where._complex.appId.length === 2
    ) {
    ...
    if (
      ...
      params.where &&
      params.where._complex &&
      params.where._complex.appName &&
      params.where._complex.appName.length === 2
    ) {
      param.appName = String(params.where._complex.appName[1]).replace(
        /[%]/g,
        ''
      );
    }
    if (
      ...
      params.where &&
      params.where._complex &&
      params.where._complex.orgLegalPerson &&
      params.where._complex.orgLegalPerson.length === 2
    ) {
      param.orgLegalPerson = String(
        params.where._complex.orgLegalPerson[1]
      ).replace(/[%]/g, '');
    }
    if (
      params &&
      params.where &&
      params.where._complex &&
      ['0', '1', '2'].includes(String(params.where._complex.status))
    ) {
      ...
      for (const search of this.searches) {
        if (!XnUtils.isEmpty(this.arrObjs[search.checkerId])) {
          params.where._complex[search.checkerId] =
            search.type && ['select', 'listingg'].includes(search.type)
  ...
  private selectFiles(paramAction, paramFileArr, params): Array<any> {
    ...
    params.logs.map((v) => {
      if (
        v &&
        v.flowId &&
        (v.flowId !== 'factoring_loan' || v.flowId !== 'factoring_repayment')
      ) {
  ...
  private async rexDownload(params, manflowId) {
    ...
    if (this.xn.user.orgType === 1) {
      ...
      if (params.logs.length) {
        logsData = params.logs.filter(
          (x) => !!x.appId && x.appId === this.supplierOperateAppId.appId
        );
      }
