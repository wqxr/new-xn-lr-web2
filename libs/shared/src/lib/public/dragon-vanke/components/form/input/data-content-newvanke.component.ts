...
export class NewVankeDataContentComponent
  implements OnInit, OnChanges, OnDestroy
{
  ...
  ngOnInit() {
    ...
    // 暂存的总部公司
    this.type =
      this.localStorageService.caCheMap.get('headquarters') ||
      HeadquartersTypeEnum[1];
  ...
  uploadExcel(e) {
     ...
     */
    this.xn.api.dragon
      .upload(this.heads[this.type].excel_up_url, fd)
      .subscribe((json) => {
        ...
        if (json.type === 'complete') {
          if (json.data.ret === 0) {
            ...
            if (msg.indexOf(';') !== -1) {
              const uniqueStr = XnUtils.distinctArray(
                msg.split(']')[1].replace(/\s*/g, '').split(';')
              );
