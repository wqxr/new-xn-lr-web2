...
import { IntelligenceStandardModalComponent } from '../../modal/intelligence-standard-modal.component';
  ...
  implements AfterViewChecked, OnInit, OnDestroy
{
  ...
  private showSubmit(recordId, fresh?) {
    ...
    this.xn.dragon
      ...
      .subscribe((json) => {
        ...
        //JSON.parse(JSON.stringify(this.svrConfig));
  ...
  private copyDraft(shows: any[], hides: any[], fresh: boolean): void {
    ...
    if (
      ['dragon_platform_verify'].includes(this.svrConfig.flow.flowId) &&
