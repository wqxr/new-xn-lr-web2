...
export class FlowDetailRecordComponent implements OnInit {
  ...
  public againSignCons(paramCons: any): void {
    ...
    contract = contract.filter(
      (x: any) =>
        x.label === '保理合同-国寿' || x.label.includes('应收账款转让登记协议')
    );
  ...
  public intermediaryHasOrCanViewDetail(paramsRow): boolean {
    return (
      [77, 102].includes(this.xn.user.orgType) &&
      paramsRow.flowId !== 'flowIdIsNotChild' &&
      paramsRow.flowId !== 'zichanchi' &&
      paramsRow.flowId !== 'wait_verification_500' &&
      paramsRow.flowId !== 'verificating_500' &&
  ...
  public showAuditingstandard(paramsRow): boolean {
    return (
      (paramsRow.appId === this.xn.user.appId || this.xn.user.orgType === 3) &&
  ...
  public checkSubprocess(paramRow: any) {
    this.xn.router.navigate([`/logan/record/view/${paramRow.recordId}`]);
...
*/
  ...
  downloadVerifyFiles() {
    ...
    this.xn.dragon
      .download('/list/main/download_platform_check_flie', {
        mainFlowId: this.mainFlowId,
      })
      .subscribe((v: any) => {
        this.loading.close();
