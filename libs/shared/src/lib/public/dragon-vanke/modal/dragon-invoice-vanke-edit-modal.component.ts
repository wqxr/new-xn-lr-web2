...
export class DragonInvoiceVankeEditModalComponent implements OnInit {
  ...
  open(params: any): Observable<any> {
    ...
    // 发票地址
    this.fileSrc = this.xn.file.dragonView(params || '');
    this.modal.open(ModalSize.XLarge);
    return this.observable;
  }
  disableExcelData(params) {
    if (params && params.invoiceCode && params.invoiceNum && params.invoiceDate) {
        ...
        if (json.ret === 0) {
          if (json.data.type === 'single') {  // 老发票验证
            ...
            this.xn.msgBox.open(false, '发票验证成功', () => {
              this.xn.dragon.post('/file/invoice_status', { invoiceNum, invoiceCode }).subscribe(x => {
                this.close({
                  action: 'ok',
                  invoiceCode,
                  invoiceNum,
                  ...
                  invoiceScreenshot: json.data?.invoiceScreenshot || '',
                  fileId,
                });
              });
            });
  ...
  imageVerification() {
    ...
    this.xn.dragon.postMap('/file/invoice_checkocr', { fileId, mainFlowId }).subscribe(x => {
