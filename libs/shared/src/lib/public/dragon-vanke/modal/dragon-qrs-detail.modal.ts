...
@Component({
    selector: 'app-dragon-qrs-detail-modal',
    templateUrl: './dragon-qrs-detail.modal.html',
    styles: [`
        .pdf-container {
            width: 100%;
            min-height: 100%;
...
export class DragonQrsDetailModalComponent {
    ...
    open(params: any): Observable<any> {
        ...
        this.total = this.params.payConfimFile.length;
        ...
        this.buildFormGroup();
        if (this.params.payConfimFile && this.params.payConfimFile.length > 0) {
            this.onPage(1);
        }
    ...
    public onPage(e) {
        ...
        const file = this.params.payConfimFile[e - 1];
        this.fileType = file.filePath.substr(-3).toLowerCase() === 'pdf' ? 'pdf' : 'img';
        if (this.fileType === 'img') {
            // this.fileSrc = this.xn.file.view(file);
            this.fileSrc = file.isAvenger ? this.xn.file.view(file) : this.xn.file.dragonView(file);
        } else {
            setTimeout(() => {
    ...
    public onDownLoad() {
        this.xn.api.AvengerDownload('/file/downFile', {
            files: this.params.payConfimFile,
        }).subscribe((v: any) => {
     ...
     */
    verifyClick(){
        if (this.params.verifyPayConfimStatus === 3){
            // 取消确认
            this.shows.map(checker => checker.verifyPayConfimStatus = 2);
            this.params.verifyPayConfimStatus = 2;
        }else if (this.params.verifyPayConfimStatus === 2){
            // 人工确认一致
            this.shows.map(checker => checker.verifyPayConfimStatus = 3);
            this.params.verifyPayConfimStatus = 3;
        }
        this.mainForm.get('verifyPayConfimStatus').patchValue(this.params.verifyPayConfimStatus);
    }
    ...
    public handleSubmit() {
        ...
        this.close({
            action: 'ok',
            verifyPayConfimStatus: Number(this.mainForm.get('verifyPayConfimStatus').value)
        });
    }
    // 计算应收账款转让金额
