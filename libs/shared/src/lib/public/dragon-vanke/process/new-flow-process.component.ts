...
import { FlowProcessStatusEnum } from '../../../common/dragon-vanke/public-enum';
import { XnService } from '../../../services/xn.service';

@Component({
    selector: 'xn-new-flow-process-component',
    template: `
    <div>
    <nz-steps [nzCurrent]="current-1" nzLabelPlacement="vertical">
    <ng-container *ngFor="let node of localFlowProcess as lists;let i=index">
      <nz-step [nzTitle]="subNode.name" *ngFor="let subNode of node.subList"></nz-step>
    </ng-container>
    </nz-steps>
   </div>
    `,
...
export class NewFlowProcessComponent implements OnInit, AfterViewInit {
    ...
    ngOnInit(): void {
        ...
        if (this.mainfromlist && this.mainfromlist.length > 0 &&
            ...
            this.xn.api.dragon.post('/flow/navigation', {
                ...
                this.current = this.localFlowProcess.findIndex(x => x.status=1);
            ...
            this.xn.api.dragon.post('/flow/navigation', {
                ...
                this.current = this.localFlowProcess.findIndex(x => x.status===1);
                console.log('current==》', this.localFlowProcess,this.current);
            });
