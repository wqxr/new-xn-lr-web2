...
export class PdfSignModalComponent implements OnInit {
  ...
  open(params: any): Observable<any> {
    ...
    // if (navigator.userAgent.indexOf('MSIE') > 0) {s
    ...
    // this.showOrHide();
    if (params.id === '') { // 针对没有id的说合同查看
      this.pdfViewService.m_init = true;
      this.title = this.params.fileName;
      console.log('图片资源有无数据', params.data)
  ...
  public onSave() {
    if (this.params.id === '') {
      if (this.title === '平台注册操作指引') {
        this.xn.api.getFileDownload('/user/get_protocol_file',
          { type: 'register', download: true }).subscribe(x => {
            const fileName = this.xn.api.getFileName(x._body.headers);
            this.xn.api.save(x._body.body, `${fileName}`);
          });
      } else {
        this.xn.api.getFileDownload('/user/get_auth_confirm_file', { type: 'blank', download: true }).subscribe(x => {
          const fileName = this.xn.api.getFileName(x._body.headers);
          this.xn.api.save(x._body.body, `${fileName}`);
        });
      }
